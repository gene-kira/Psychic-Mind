...
class BaseAgent:
    ...
    def detect_anomaly(
        self, memory: MemoryState, cpu: float, mem: float
    ) -> bool:
        if cpu < 0 or mem < 0:
            return False
...
class RegenerationChamber:
    ...
    def _too_many_anomalies(self, memory: MemoryState) -> bool:
        if not memory.anomalies:
            return False
        recent = [
            a for a in memory.anomalies if now() - a["timestamp"] < 120
        ]
...
class NodeServer:
    ...
    def start(self):
        ...
        t.start()
        threading.Thread(
            target=self.telemetry_sensor_loop, daemon=True
        ).start()
        threading.Thread(
            target=self.filesystem_sensor_loop, daemon=True
        ).start()
