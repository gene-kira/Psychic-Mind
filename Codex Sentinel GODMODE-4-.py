...
- Tiny CLI wrapper: `godmode`-style commands via argparse
"""
...
def cli():
    ...
    p_gui = sub.add_parser("gui", help="Launch GUI installer")
    ...
    if args.command == "run":
        ...
        app = create_fastapi_app(lambda: GODMODEOrchestrator(
            llm=HFLLMBackend(model_name=cfg["model_name"], device=None if cfg["use_gpu"] else "cpu")
        ))
