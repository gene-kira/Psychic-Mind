#!/usr/bin/env python3
# OmniQueen Swarm Guard - Full Platform
# - Swarm + Queen
# - PySide6 Cockpit
# - ML anomaly model (+ optional GPU)
# - Steam/Epic whitelist
# - Remote command API
# - ETW/kernel-sensor stub
...
class MLAnomalyModel:
    ...
    def add_sample(self, features):
        ...
        if len(self.buffer) >= 200 and not self.trained:
            try:
                if GPU_AVAILABLE:
                    # simple CPU fit; GPU could be used for transforms if needed
...
class BorgMemory:
    ...
    def _base_features(self, rarity, keyword_boost, env_boost, pressure_boost):
        return [rarity, keyword_boost, env_boost, pressure_boost]
    ...
    def score_process(self, name, cmdline, suspicious_keywords):
        ...
        base_score = min(1.0, rarity * 0.4 + keyword_boost + env_boost + pressure_boost)

        features = self._base_features(rarity, keyword_boost, env_boost, pressure_boost)
    ...
    def score_ip(self, ip):
        ...
        base_score = min(1.0, rarity * 0.5 + env_boost + pressure_boost)

        features = self._base_features(rarity, 0.0, env_boost, pressure_boost)
        self.ml_model.add_sample(features)
        ml_score = self.ml_model.score(features)
...
def etw_loop(borg: BorgMemory, stop_event):
    if platform.system().lower() != "windows":
        return
    # This is a placeholder for real ETW integration.
    # You can wire a library like 'etw' or custom bindings here.
    log_event("INFO", "etw", "ETW stub loop started (no real providers wired)", {})
    while not stop_event.is_set():
        # In a real implementation, consume ETW events and call:
        # borg.register_threat_event(weight=...)
        time.sleep(5)
...
class Cockpit(QWidget):
    def __init__(self, cfg_ref, borg: BorgMemory, state_ref, parent=None):
        ...
        self.timeline_list.setWindowTitle("Timeline")
