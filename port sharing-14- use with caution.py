#!/usr/bin/env python3
"""
Codex Sentinel – Mesh, Raft, eBPF, Threat Intel, Twins
"""
...
state = {
    ...
    "raft_peers": set(),
    # Mesh
    "mesh_peers": {},  # node_id -> (sock, last_seen, meta)
    # Identity / intel
    "code_fingerprint": None,
    "cluster_blocklist": {},   # (kind, value) -> entry
    "cluster_baselines": {},   # (process, port) -> {avg_cpu, avg_mem, avg_conn, samples}
}
...
def record_event(event):
    ...
    # replicate to mesh via Raft log
    raft_append_entry({"type": "THREAT_EVENT", "event": event})
    # also gossip as threat intel
...
# ----------------- WRAPPERS (WINDOWS, TRIMMED) -----------------

def ensure_wrapper_for_script(script_path: str) -> str | None:
    if is_self_script(script_path): return None
    ...
    if not os.path.exists(wrapper_path):
        code = f"""# Wrapper placeholder – keep your previous full wrapper logic here.
# (Crash loop detection, AUTO_PORT patching, integrity check, etc.)
"""
        try:
    ...
    register_integrity(script_path); register_integrity(wrapper_path)
    return wrapper_path
...
def raft_apply_committed():
    with lock:
        while state["raft_last_applied"] < state["raft_commit_index"]:
            ...
            cmd = entry["command"]
            # here you could apply replicated config changes if needed
...
def handle_threat_intel(msg: dict):
    ...
    if not is_twin:
        # you could enforce stricter trust rules here
        pass
...
def handle_consensus_proposal(msg: dict):
    # simple auto-yes from twins; you can extend with evidence checks
...
def handle_consensus_vote(msg: dict):
    # placeholder: you can track votes and when majority reached, emit BLOCKLIST_UPDATE
    pass
