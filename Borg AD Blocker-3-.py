...
# =========================
# Event Horizon (BORG) window
# =========================

class EventHorizonWindow(QtWidgets.QMainWindow):
    """
    Neon sci-fi BORG cockpit:
    - System health
    - Cognitive stream (updates every 3s)
    - Four panels: Allow, Block, Radioactive, Pending/Unknown
    - Click items -> popup (Allow/Block/Radioactive) -> move between lists
    """
    ...
    def __init__(self):
        ...
        self.uptime_label = QtWidgets.QLabel("UPTIME: 0s")
        self.uptime_label.setStyleSheet("color:#9ca3af;")
        ...
        health_group.setStyleSheet("QGroupBox { border: 1px solid #22c55e; margin-top: 6px; }"
        ...
        cog_group.setStyleSheet("QGroupBox { border: 1px solid #22c55e; margin-top: 6px; }"
                                "QGroupBox::title { subcontrol-origin: margin; left: 10px; padding: 0 3px; }")
        ...
        # Four panels in a horizontal row
        ...
        # Seed with some placeholder items (you can remove this)
        self.add_threat_item("chrome.exe", "142.250.190.78", "Overseas", 42, self.pending_list["list"])
        self.add_threat_item("steam.exe", "192.168.1.10", "Local", 15, self.allow_list["list"])
        self.add_threat_item("python.exe", "8.8.8.8", "New Country", 65, self.radio_list["list"])
    ...
    def add_threat_item(self, proc: str, remote: str, context: str, risk: int, target_list: QtWidgets.QListWidget):
        text = f"{proc} → {remote}  |  {context}  |  Risk {risk}"
    ...
    def update_cognitive_stream(self):
        lines = [
            "[THINK] Scanning new outbound patterns...",
            "[TRACK] Monitoring high-risk overseas flows...",
            "[BLOCK] Enforcing policy on radioactive endpoints...",
            "[PREDICT] Evaluating next likely deviation..."
        ]
    ...
    def show_item_menu(self, pos: QtCore.QPoint):
        ...
        menu = QtWidgets.QMenu(self)
        allow_action = menu.addAction("Allow")
        block_action = menu.addAction("Block")
        radio_action = menu.addAction("Radioactive")
        ...
        # Here is where you would sync with QueenGuard policies if desired.
...
def main():
    ...
    queen_win = QueenWindow(event_q, memory)
