...
CROSS-PLATFORM + LAN SWARM EDITION (Windows / Linux / macOS / most POSIX)
...
- Confidence Engine for detections (Bernoulli-style weighting)
- Queen Consensus Engine (REAL-TIME QUEEN, swarm-style risk aggregation)
...
- Behavioral Threat Scan (non-destructive, pattern-based)
- Integrity Self-Check (no self-rewriting)
- Fleet Orchestrator (multi-node view, local-only)
- LAN Swarm Mode (UDP broadcast/listen, anonymous node IDs, no IP/MAC/hostname logging)
- "Intelligent Water" Data Physics: treats metrics like fluid flow (bursts, turbulence)
- Altered States: explicit detection of high-stress/high-entropy states

SAFE DESIGN:
- No IP addresses, MAC addresses, usernames, hostnames, process names, PIDs, file paths in logs.
...
# =========================
# Global brain state (for swarm id / phantom)
# =========================
...
# =========================
# Network / Remote Control Detection Patterns
# (behavioral only, no logging of raw commands)
# =========================
...
# =========================
# Real-Time Identifiers (used only internally, not logged)
# =========================
...
class Queen:
    ...
    Aggregates risk across nodes based on behavioral entities.
    """
    def __init__(self):
...
class DataPhysicsEngine:
    ...
    Treats CPU/RAM/Disk/Net as flows with inertia, turbulence, and shape.
    """
    ...
    def confidence_score(self, stress, entropy, anomaly_flags):
        # Bernoulli-style: each anomaly contributes a "vote" to confidence
        base = 0.5
...
class POVUnifiedSecurityCore:
    def __init__(self, root, node_id=None, role="WORKSTATION"):
        ...
        # Use swarm id as base node id (not logged externally)
        ...
        # Behavioral counters from threat_scan_and_respond
        ...
        self.banner = tk.Label(
            self.main_frame,
            text=f"AUTONOMOUS • PRIVACY HARDENED • LOCAL + LAN SWARM • OS: {platform.system()}",
            fg="#A0FFCF",
            bg="#07120F",
        ...
        self.info_label = tk.Label(
            control_frame,
            text=f"Autonomous mode: monitoring only • Node: {self.node_id}",
            fg="#8888FF",
            bg="#111318"
        )
        ...
        self.queen_title = tk.Label(
            self.side_panel,
            text="QUEEN CONSENSUS (LOCAL + LAN)",
            fg="#A0C8FF",
            bg="#0A0C12",
            font=("Consolas", 10, "bold")
        )
        ...
        self.status_label.config(text="Status: Running (Autonomous, Monitoring Only, Privacy Hardened, LAN Swarm)")
    ...
    # =========================
    # Threat Detection + Response (behavioral, non-destructive)
    # =========================
    ...
    def threat_scan_and_respond(self):
        ...
        - Counts processes
        - Looks for remote-control / firewall / settings / shell patterns in cmdlines
        - Looks for suspicious names (keylogger/sniffer/injector/bot/miner) WITHOUT killing anything
        """
        try:
            try:
                conns = safe_net_connections()
                ...
                try:
                    if is_listening(conn):
                        ...
                        if key in seen:
                            continue
                        seen.add(key)
                        # We don't log ip/port externally; just count behavior
                    else:
                        continue
                except Exception:
                    continue
        ...
        while self.running:
            try:
                ...
                fleet_status = self.build_fleet_status(
                    avg_stress, avg_entropy, behavioral_state, anomaly_flags
                )
                payload = {
                ...
                sock.sendto(data, ("<broadcast>", LAN_UDP_PORT))
            except Exception:
                pass
        ...
        while self.running:
            try:
                ...
                if not isinstance(msg, dict):
                    continue
                if msg.get("type") != "sentinel_status":
                    continue
    ...
    def update_ui_and_interfaces(self, cpu, ram, disk_norm, net_kb, stress, entropy):
        ...
        if self.altered_state:
            status_text = "Status: ALTERED STATE (High Stress + High Entropy, LAN Swarm)"
            status_color = "#FF4BFF"
        else:
            status_text = f"Status: {behavioral_state} (Autonomous, Monitoring Only, LAN Swarm)"
            status_color = "#A0FFB0"
    ...
    def update_queen_preview(self, global_risk):
        ...
        lines = ["High-risk entities (local + LAN):"]
