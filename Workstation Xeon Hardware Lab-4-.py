...
"""
Workstation / Xeon Hardware Lab v5.0

Adds on top of v4.0:
- Real-time sensors (CPU load, temp where available)
- Thermal modeling (TDP vs cooling vs VRM, thermal headroom)
- GPU compute scoring (TFLOPs-style heuristic)
- AI-based build optimizer (rule-based "AI" optimizer on top of scoring)
- Keeps: PCIe/NUMA/memory/DMI/PSU/storage/multi-CPU/workload scoring/suggested builds
"""
    ...
    def read_sensors():
        ...
        try:
            ...
            for name, entries in temps.items():
                for e in entries:
                    if "cpu" in e.label.lower() or "package" in e.label.lower() or "core 0" in e.label.lower():
...
class PlatformModel:
    ...
    @staticmethod
    def dmi_bottleneck(board, gpu, storage):
        ...
        gpu_bw = 8.0 if gpu and gpu.get("pcie_gen", 4) >= 4 else 4.0
...
class PowerEngine:
    @staticmethod
    def estimate_system_power(cpu, board, gpu, storage):
    ...
    @staticmethod
    def evaluate_psu_margin(power_info, gpu):
        ...
        if gpu:
            ...
            if rec < gpu_rec:
                warnings.append(
                    f"Estimated PSU ({rec}W) is below GPU vendor recommendation ({gpu_rec}W)."
                )
        return rec, warnings

class ThermalModel:
    @staticmethod
    def estimate_thermal_headroom(cpu, board, sensors):
        cpu_tdp = cpu.get("tdp", 0)
...
class WorkloadScorer:
    @staticmethod
    def score(workload, cpu, board, gpu, mem_bw, dmi_info):
        score_delta = 0
        notes = []
...
class RuleEngine:
    def evaluate(self, cpu, board, gpu=None, mem=None, storage=None, workload=None, sensors=None):
        result = {
            "score": 100,
            "compatible": True,
            "warnings": [],
        ...
        if gpu:
            ...
            if not has_slot:
                result["score"] -= 25
                result["warnings"].append("No suitable PCIe slot for GPU")

            if board.get("pcie_gen", 3) < gpu.get("pcie_gen", 3):
                result["score"] -= 5
                result["warnings"].append("Board PCIe gen below GPU capability")
        ...
        if sensors is None:
            sensors = {}
        thermal_info = ThermalModel.estimate_thermal_headroom(cpu, board, sensors)
        result["thermal"] = thermal_info
        if thermal_info.get("temp_note"):
            result["warnings"].append(thermal_info["temp_note"])
        if thermal_info.get("vrm_note"):
            result["warnings"].append(thermal_info["vrm_note"])
        ...
        if workload:
            ...
            gpu_score, gpu_note = GPUComputeScorer.score_gpu_for_workload(gpu, workload)
...
class Simulator:
    ...
    def run(self, cpu_name, board_name, gpu_name=None, mem_name=None, storage_name=None, workload=None, sensors=None):
        ...
        result = self.engine.evaluate(cpu, board, gpu, mem, storage, workload, sensors)
        return {
            "cpu": cpu_name,
            "board": board_name,
            "gpu": gpu_name,
...
class AIBuildOptimizer:
    ...
    - Explores CPU/board/GPU/memory/storage combos
    - Uses RuleEngine + GPU compute + workload scoring
    - Adds heuristic penalties/bonuses for cost/overkill/efficiency
    """
    ...
    def optimize(self, workload: str) -> Optional[Dict[str, Any]]:
        ...
        for cpu_name, cpu in self.db.cpu_db.items():
            for board_name, board in self.db.board_db.items():
                ...
                for gpu_name, gpu in self.db.gpu_db.items():
                    ...
                    sensors = HardwareDetector.read_sensors()
                    result = self.engine.evaluate(cpu, board, gpu, mem, storage, workload, sensors)

                    score = result["score"]
                    score += self._efficiency_bonus(cpu, board, gpu, result)
                    score -= self._overkill_penalty(cpu, board, gpu, workload)
    ...
    def _efficiency_bonus(self, cpu, board, gpu, result):
        ...
        if 0.4 < ratio < 0.7:
            bonus += 5
        if gpu and gpu.get("tier") in ("pro", "high") and cpu.get("cores", 0) >= 8:
            bonus += 5
        return bonus

    def _overkill_penalty(self, cpu, board, gpu, workload):
        penalty = 0
        if workload in ("gaming", "unreal"):
            if "EPYC" in cpu.get("family", "") or "Xeon Scalable" in cpu.get("family", ""):
                penalty += 10
        if workload in ("balanced",) and gpu and gpu.get("tier") == "ultra":
            penalty += 5
        return penalty
    ...
    def suggest(self, workload):
        ...
        for cpu_name, cpu in self.db.cpu_db.items():
            ...
            for board_name, board in self.db.board_db.items():
                ...
                gpu = self.db.gpu_db.get(gpu_name)
...
class HardwareLab(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Workstation / Xeon Hardware Lab v5.0")
    ...
    def _build_panel_dashboard(self):
        ...
        title = QtWidgets.QLabel("Live System Fingerprint + Sensors")
        ...
        self.sensor_view.setFixedHeight(120)
    ...
    def _update_sensor_panel(self):
        ...
        lines.append(f"  CPU Load: {sensors.get('cpu_load_percent')} %")
        lines.append(f"  CPU Temp: {sensors.get('cpu_temp_c')} °C")
    ...
    def _build_panel_oem_sim(self):
        ...
        self.ai_opt_btn = QtWidgets.QPushButton("AI Optimize Build (v5)")
        ...
        left.addWidget(QtWidgets.QLabel("Upgrade Advisor / AI Notes"))
        ...
        right.addWidget(QtWidgets.QLabel("OEM Compatibility + Thermal + GPU Compute Report"))
    ...
    def run_simulation(self):
        ...
        sensors = HardwareDetector.read_sensors()
        report = self.sim.run(cpu_name, board_name, gpu_name, mem_name, storage_name, workload, sensors)
    ...
    def ai_optimize_build(self):
        ...
        self.advice_box.setText(
            f"AI-Optimized build (v5):\nScore={suggestion['score']}\n\n" + advice
        )

        self._log_event(
            f"AI-Optimized build: CPU={cpu_name}, Board={board_name}, GPU={gpu_name}, "
            f"MEM={mem_name}, Storage={storage_name}, Workload={workload}, Score={suggestion['score']}"
        )
