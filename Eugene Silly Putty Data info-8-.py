...
# =========================================================
# OBSERVATION LAYER (REAL SYSTEM SIGNALS)
# =========================================================
...
class SystemObserver:
    def observe(self) -> Dict[str, float]:
        ...
        return {
            ...
            "processes": len(psutil.pids()) / 500.0,
        }
...
class StateEncoder:
    def encode(self, o: Dict[str, float]) -> str:
        if o["cpu"] > 0.8 or o["memory"] > 0.8:
            return "HIGH_LOAD"
        if o["cpu"] > 0.5:
            return "MEDIUM_LOAD"
        if o["processes"] > 0.7:
            return "PROCESS_HEAVY"
        if (o["net_in"] + o["net_out"]) > 1e7:
...
ACTIONS = ["OBSERVE", "OPTIMIZE", "COOLDOWN", "THROTTLE"]
...
class QAgent:
    ...
    def learn(self, s, a, r, s2):
        best_next = max(self.q[s2].values()) if s2 in self.q else 0.0
...
# =========================================================
# SWARM CONSENSUS (SIMPLIFIED BUT REAL)
# =========================================================
...
# =========================================================
# ADAPTIVE CONTROL (SAFE "MUTATION")
# =========================================================
...
class Environment:
    def reward(self, state, action):
        ...
        if state == "HIGH_LOAD" and action == "COOLDOWN":
            return 1.0
        if state == "STABLE" and action == "OBSERVE":
            return 0.7 + base * 0.3
        if state == "MEDIUM_LOAD" and action == "OPTIMIZE":
            return 0.8 + base * 0.2
        if state == "PROCESS_HEAVY" and action == "THROTTLE":
            return 0.9
...
class CognitiveCore:
    ...
    def step(self):
        ...
        o2 = self.obs.observe()
        s2 = self.enc.encode(o2)

        self.agent.learn(s, a, r, s2)
        self.mem.store((s, a, r, s2))

        self.ctrl.adapt(r)

        self.swarm.submit("node", r)
        swarm_signal = self.swarm.consensus()

        self.reward_ema = 0.9 * self.reward_ema + 0.1 * r
...
class Persistence:
    def __init__(self, path="cais_state.json"):
        self.path = path

    def save(self, core: CognitiveCore):
        try:
            with open(self.path, "w") as f:
                json.dump({
                    "q": dict(core.agent.q),
                    "epsilon": core.ctrl.epsilon,
                    "lr": core.ctrl.learning_rate,
                }, f)
        except Exception:
            pass

    def load(self, core: CognitiveCore):
        try:
            if not os.path.exists(self.path):
                return
            with open(self.path) as f:
                data = json.load(f)
                core.agent.q.update(data.get("q", {}))
                core.ctrl.epsilon = data.get("epsilon", 0.2)
                core.ctrl.learning_rate = data.get("lr", 0.1)
        except Exception:
            pass
