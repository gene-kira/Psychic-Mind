...
(CodexModified + self-exclusion + original-script detection)
...
- Original-script detection (manual runs don’t trigger wrapper restarts)
...
def watchdog_loop():
    ...
    while True:
        for entry in MANAGED_PROGRAMS:
            # If original script is already running (manually started), do NOT start wrapper
            script_path = entry.get("script_path")
            if script_path and is_original_script_running(script_path):
                continue

            # Otherwise, ensure wrapper is running
