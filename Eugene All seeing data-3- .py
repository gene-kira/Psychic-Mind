...
# - IP ledger (IP -> hostname -> processes/directions)
# - Call-home beacon
...
class PersistentState:
    def __init__(self):
        ...
        self.state = {
            ...
            "ip_ledger": {},   # ip -> {hostname, first_seen, last_seen, directions, processes}
    ...
    def update_ip_ledger(self, ip, direction, pid, proc_name, ts=None):
        if not ip:
            return
        ts = ts or now_iso()
        with self.lock:
    ...
    def add_connection_event(self, ev):
        """
        Store full connection history and prune older than retention window.
        """
...
class NetProcessMonitor(threading.Thread):
    ...
    - Maintain IP ledger
    - Store full connection history (with 1-year retention)
    """
    ...
    def sample(self):
        ...
        # Per-process metadata + connection history
        for proc in psutil.process_iter(attrs=["pid", "name"]):
            ...
            for c in conns:
                ...
                # Update IP ledger
                if remote_ip:
                    self.state.update_ip_ledger(remote_ip, direction, pid, name, ts=ts)
...
class ConsoleHUD(threading.Thread):
    ...
    def render(self):
        ...
        print("-" * 100)
        print(" Top IPs in ledger (by process count):")
        for ip, entry, proc_count in ip_summary:
            hostname = entry.get("hostname") or "-"
            dirs = ",".join(entry.get("directions", []))
            print(f"  {ip:<15} | {hostname:<30} | dirs: {dirs:<15} | procs: {proc_count}")
        print("-" * 100)
