"""
Alien Cockpit + ReplicaNPU Swarm + MemryX NPU Integration
---------------------------------------------------------

Universal GPU/NPU cockpit with:

- ReplicaNPU (software NPU organ with plasticity, integrity, auto-tune)
- DummyGPUOrgan (simulated GPU backend)
- NPUSwarm (multi-node ReplicaNPU swarm)
- MemryXOrgan (hardware MemryX MX3 Cascade+ backend)
- MemryXNode (swarm node backed by MemryX)
- OS-level discovery for GPU + USB/PCIe NPU devices
- Tkinter dark-neon cockpit GUI

NOTE: You MUST fill in the MemryX model path and I/O shapes in the
      MEMRYX CONFIG section below before running on real hardware.
"""
...
# If MemryX Python driver is available, import it.
# Adjust the module name if your driver uses a different one.
try:
    import mxa  # MemryX Python driver
    HAS_MEMRYX = True
except ImportError:
    HAS_MEMRYX = False
...
# =============================================================
# MEMRYX CONFIG (FILL THESE FOR REAL HARDWARE)
# =============================================================

MEMRYX_MODEL_PATH = "path/to/your_model.dfp"  # TODO: set real DFP path
MEMRYX_MODEL_ID = 0
MEMRYX_GROUP_ID = 0
MEMRYX_CHIP_GEN = getattr(mxa, "MEMX_DEVICE_CASCADE_PLUS", 3) if HAS_MEMRYX else 3
MEMRYX_MODEL_IDX = 0
MEMRYX_IN_FLOW_ID = 0
MEMRYX_OUT_FLOW_ID = 0

# Input/output shapes for your MemryX model.
# You MUST set these to match your compiled model.
# Example: (1, 128) or (224, 224, 3), etc.
MEMRYX_INPUT_SHAPE = (1, 128)   # TODO: set real input shape
MEMRYX_OUTPUT_SHAPE = (1, 64)   # TODO: set real output shape


# =============================================================
# ReplicaNPU: Ultra-style organ
# =============================================================
...
class ReplicaNPU:
    ...
    def __init__(
        ...
        self.horizon = 0.5
        self.dampening = 0.5
        self.cache_behavior = 0.5
        self.thread_expansion = 0.5
...
class Brain:
    ...
    def update(self, prediction_bus: PredictionBus):
        ...
        npu0 = self.swarm.nodes[0].npu if self.swarm.nodes else ReplicaNPU()
...
def _probe_usb_macos() -> List[DiscoveredDevice]:
    ...
    for line in out.splitlines():
        ...
        desc = s.rstrip(":")
        is_npu = any(h in desc.lower() for h in NPU_USB_HINTS)
        backend_hint = "memryx" if "memryx" in desc.lower() else ("generic_usb_npu" if is_npu else None)
        devices.append(
            DiscoveredDevice(
                bus="USB",
                vendor_id="0000",
...
class MemryXOrgan:
    """
    MemryX NPU backend organ.

    Wraps the MemryX driver (mxa) and exposes run(ifmap) -> ofmap.
    """

    def __init__(
        self,
        name: str = "memryx_0",
        model_id: int = MEMRYX_MODEL_ID,
        group_id: int = MEMRYX_GROUP_ID,
        chip_gen: int = MEMRYX_CHIP_GEN,
        model_path: str = MEMRYX_MODEL_PATH,
        model_idx: int = MEMRYX_MODEL_IDX,
        download_type: Optional[int] = None,
        in_flow_id: int = MEMRYX_IN_FLOW_ID,
        out_flow_id: int = MEMRYX_OUT_FLOW_ID,
    ):
        if not HAS_MEMRYX:
            raise RuntimeError("MemryX driver (mxa + numpy) not available")
        ...
        self.download_type = (
            ...
            else getattr(mxa, "download_type_wtmem_and_model", 3)
        )
    ...
    def _open_and_init(self):
        mxa.lock(self.group_id)
        try:
            mxa.open(self.model_id, self.group_id, self.chip_gen)
            ...
            mxa.set_stream_enable(self.model_id, wait=1)

            self._opened = True
            self._stream_enabled = True
        finally:
            mxa.unlock(self.group_id)
    ...
    def close(self):
        ...
        try:
            if self._stream_enabled:
                mxa.set_stream_disable(self.model_id, wait=1)
        finally:
            mxa.close(self.model_id)
            self._opened = False
            self._stream_enabled = False

    def run(self, ifmap: "np.ndarray", timeout_ms: int = 0) -> "np.ndarray":
        if not self._opened:
            raise RuntimeError("MemryXOrgan not opened")

        flow_in = self.in_flow_id
        flow_out = self.out_flow_id
        ...
        try:
            ...
            ofmap = np.zeros(MEMRYX_OUTPUT_SHAPE, dtype=ifmap.dtype)
...
class MemryXNode:
    """
    Swarm node backed by a MemryX NPU.

    Uses MemryXOrgan for inference and a small ReplicaNPU shadow for
    confidence/integrity bookkeeping.
    """

    def __init__(self, name: str, memryx: MemryXOrgan):
        self.name = name
        self.memryx = memryx

        self.npu = ReplicaNPU(name=f"{name}_shadow", cores=4, frequency_ghz=1.0)
        self.npu.add_head("short", 3, lr=0.01, risk=1.0)
        self.npu.add_head("medium", 3, lr=0.01, risk=1.0)
        self.npu.add_head("long", 3, lr=0.01, risk=1.0)

        self.last_preds = {"short": 0.5, "medium": 0.5, "long": 0.5}

    def step(self, x_vec, targets=None, integrity=1.0):
        x_arr = np.array(x_vec, dtype=np.float32).reshape(MEMRYX_INPUT_SHAPE)

        ofmap = self.memryx.run(x_arr)

        flat = ofmap.flatten()
        if flat.size == 0:
            s = m = l = 0.5
        else:
            third = max(1, flat.size // 3)
            s = float(np.mean(flat[:third]))
            m = float(np.mean(flat[third:2 * third]))
            l = float(np.mean(flat[2 * third:]))
        ...
        self.npu.predict(x_vec)
        self.npu.learn(x_vec, pseudo_targets)
        self.npu.micro_recovery()
        self.npu.check_integrity(external_integrity=integrity)
        self.npu.tick(budget=16, last_step_success=True)

        self.last_preds = pseudo_targets
...
def create_npu_swarm_from_discovery(default_count: int = 2) -> NPUSwarm:
    ...
    for idx, dev in enumerate(npu_devs):
        ...
        if dev.backend_hint == "memryx" and HAS_MEMRYX:
            try:
                mem = MemryXOrgan(
                    name=f"memryx_{idx}",
                    model_id=MEMRYX_MODEL_ID,
                    group_id=MEMRYX_GROUP_ID,
                    chip_gen=MEMRYX_CHIP_GEN,
                    model_path=MEMRYX_MODEL_PATH,
                    model_idx=MEMRYX_MODEL_IDX,
                )
                node = MemryXNode(name=f"memryx_{idx}", memryx=mem)
            except Exception as e:
                print(f"[memryx] Failed to init MemryX organ: {e}, falling back to ReplicaNPU node")
                node = NPUNode(name=f"npu_fallback_{idx}")
        else:
...
class AlienCockpit:
    def __init__(self, root):
        self.root = root
        self.root.title("Alien Cockpit - ReplicaNPU Swarm + MemryX")
    ...
    def _build_ui(self):
        ...
        self.lbl_title = ttk.Label(top_frame, text="Alien Cockpit - ReplicaNPU Swarm + MemryX", style="AlienTitle.TLabel")
    ...
    def _tick(self):
        ...
        for node in self.swarm.nodes:
            if isinstance(node, MemryXNode):
                node.step(x, integrity=self.brain.model_integrity)
            else:
                node.step(x, targets, integrity=self.brain.model_integrity)
