# =========================
# MIGHTY MOUSE – FULL UPDATED CODE (PREDICTIVE UPGRADE, OPTION B)
# =========================
...
# =========================
# GAME COGNITION + PREDICTIVE ENGINE
# =========================
...
class GameCognition:
    def __init__(self, hud, memory, recorder, predictor):
        ...
        self.danger_history = []
        self.smoothed_danger = None
        self.trend = 0.0
        self.alpha = 0.3
        self.beta = 0.1
        self.future_horizons = [3, 5, 10]  # seconds-equivalent steps
    ...
    def _update_timeline_and_prediction(self):
        ...
        if len(self.danger_history) > 200:
            self.danger_history.pop(0)

        self.memory.record_danger_peak(base_score)

        # Holt-style exponential smoothing for level + trend
        if self.smoothed_danger is None:
            self.smoothed_danger = base_score
            self.trend = 0.0
        else:
            prev_level = self.smoothed_danger
            level = self.alpha * base_score + (1 - self.alpha) * (prev_level + self.trend)
            trend = self.beta * (level - prev_level) + (1 - self.beta) * self.trend
            self.smoothed_danger = level
            self.trend = trend

        # Multi-horizon forecast (in "steps" ~ tick windows)
        forecasts = {}
        for h in self.future_horizons:
            pred = self.smoothed_danger + self.trend * h
            pred = max(0, min(100, int(pred)))
            forecasts[str(h)] = pred

        # Use 5-step horizon as primary future danger
        future_5 = forecasts.get("5", base_score)
        self.memory.set_future_danger(future_5)
        self.memory.set_future_danger_multi(forecasts)

        # Alerts on rising trend and anomalies
        if future_5 >= 70 and base_score < 70:
            self.hud.push("predict", f"Danger spike expected in ~5s (trend rising)")

        if len(self.danger_history) >= 20:
            import statistics
            recent = self.danger_history[-20:]
...
# =========================
# WHAT-IF ENGINE (SIMULATED FUTURE STATES)
# =========================

class WhatIfEngine:
    @staticmethod
    def simulate_future(minimap, seconds=5, predictor=None):
        if not minimap:
            return {"danger": 0, "enemies": 0}
        enemies = minimap.get("enemies", [])
        if not enemies:
            return {"danger": 0, "enemies": 0}

        # Simple model: assume linear drift based on ML predictor if available
        projected_enemies = enemies
        if predictor is not None:
            pred = predictor.predict_future_positions({"enemies": enemies})
            if pred:
                projected_enemies = [{"x": p["x"], "y": p["y"]} for p in pred]

        danger = min(100, len(projected_enemies) * 10)
        return {
            "danger": danger,
            "enemies": len(projected_enemies)
        }
...
class DirectXOverlayBridge:
    ...
    def send_state(self, game_cog, mode, profile, threat_level, future_danger):
        ...
        state = {
            ...
            "future_danger_multi": game_cog.memory.data["game"].get("future_danger_multi", {}),
...
class Orchestrator:
    def __init__(self, bus, memory, cursor, hud, web, fs, game, profiles, overlay_bridge, predictor):
        self.bus = bus
        self.memory = memory
        self.cursor = cursor
        self.hud = hud
...
def start_whatif_server(game_cognition, predictor, orchestrator):
    class WhatIfHandler(BaseHTTPRequestHandler):
        def do_POST(self):
            ...
            try:
                ...
                result = WhatIfEngine.simulate_future(minimap, seconds=horizon, predictor=predictor)
                orchestrator.log(f"[WHATIF] horizon={horizon}s danger={result['danger']} enemies={result['enemies']}")
...
class GameHelperClient:
    ...
    def send_whatif(self, minimap, horizon=5):
        url = f"{self.base_url}:8791/whatif"
        self._post_json(url, {"minimap": minimap, "horizon": horizon})
...
def run_game_helper_sim():
    ...
    while True:
        ...
        helper.send_whatif(minimap, horizon=5)
        yaw = (yaw + 15) % 360
        time.sleep(1.0)
...
class CockpitGUI(QWidget):
    def __init__(self, orch, hud, web, fs, game, profiles, memory):
        ...
        self.future_multi_label = QLabel("Multi-horizon Danger (steps): {}")
    ...
    def update_gui(self):
        ...
        future_multi = self.memory.data["game"].get("future_danger_multi", {})
        self.future_multi_label.setText(f"Multi-horizon Danger (steps): {future_multi}")
...
def main():
    ...
    orch = Orchestrator(bus, memory, cursor, hud, web, fs, game, profiles, overlay_bridge, predictor)
    ...
    start_whatif_server(game, predictor, orch)
    ...
    orch.log("[SYSTEM] Mighty Mouse fully initialized (predictive upgrade, what-if ready)")
    sys.exit(app.exec_())
