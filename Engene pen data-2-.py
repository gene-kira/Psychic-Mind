...
# ============================================================
#  GLOBALS: REPLAY PROTECTION + LINEAGE
# ============================================================
...
def select_ring_for_payload_size(size: int) -> str:
    # Light load balancing: prefer ring with lower pressure among eligible
    candidates = []
...
# ============================================================
#  POLICY ENGINE (RING + SENSITIVITY + PRESSURE AWARE)
# ============================================================
...
def pre_decode_patterns(shards, pattern_id, seed=None, perm=None):
    # Integrity check: 4 shards required
    if len(shards) != 4:
        return shards
    ...
    # For deterministic patterns, we know the forward perm; invert it
    if pattern_id == "base":
        perm = [0, 2, 1, 3]
...
def adaptive_pre_profile(ring_id: str, frame_index: int, sensitivity: str, ring_state: RingState):
    # Swarm-ish: use pressure + sensitivity + frame index
...
def decode_frame_with_pre(frame: bytes, flow_id: str, ring_state: RingState,
    ...
    for s in original_order_shards:
        try:
            ...
            # PRE error recovery: fallback to raw shard
            x = b""
        x = organ_unreverse(x)
        restored.append(x)
...
def backbone_encode(raw: bytes, device_id: str, session_id: str, parent_flow_id: str | None = None):
...
def backbone_decode(capsule: dict):
    ...
    try:
        ...
        # If decompression fails, fallback to raw compressed bytes
        raw = compressed
    ...
    def update_loop(self):
        ...
        for e in reversed(self.telemetry.events):
            if e["kind"] == "flow" and e["data"].get("stage") == "ingress":
                policy = e["data"].get("sensitivity")
                break
        # Quick scan of last system_metrics event for active games
        for e in reversed(self.telemetry.events):
            if e["kind"] == "flow" and e["data"].get("stage") == "ingress":
                break
        # More direct: count last system_metrics event
        for e in reversed(self.telemetry.events):
            if e["kind"] == "flow":
                break
        # Instead, just approximate from last system_metrics capsule
        # (we don't store them separately, so we skip deep integration here)
        self.game_label.config(text=f"Active Games: ~")
...
# ============================================================
#  CONTINUOUS BACKBONE LOOP (DAEMON + HEARTBEAT + ISOLATION)
# ============================================================
        ...
        while True:
            ...
            try:
                ...
                if sys_evt is not None:
                    ...
                    capsule = backbone_encode(raw, device_id, session_id, parent_flow_id=None)
