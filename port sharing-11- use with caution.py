...
#!/usr/bin/env python3
"""
Codex Sentinel – Unified Autonomous Swarm Port Guardian
(Hardening + Smarter Detection + Swarm Security + Safety)

Adds:
- Integrity verification (SHA-256) for wrappers/scripts
- Encrypted configuration storage
- Privilege isolation for wrappers (best-effort)
- Secure logging with hash chain
- Behavioral fingerprinting + adaptive thresholds + threat correlation
- Swarm mutual auth (signed node identity)
- Retry back-off + graceful quarantine
"""
...
SWARM_NODE_SECRET = "KILLER666_NODE_SECRET"  # per-node auth secret (simplified)
...
state = {
    ...
    "behavior_history": {},  # (proc,port)->list of (cpu,mem,conn_count)
}
...
def load_config():
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            secure_log("info", "Loaded encrypted config.")
        except Exception as e:
...
def save_config():
    ...
    try:
        ...
        with open(CONFIG_FILE, "wb") as f:
            f.write(enc)
        secure_log("info", "Encrypted config saved.")
    except Exception as e:
...
def firewall_block_port(port, protocol="TCP"):
    ...
    try:
        ...
        secure_log("info", f"Firewall rule added for {protocol} port {port}")
    except Exception as e:
...
def firewall_unblock_port(port, protocol="TCP"):
    ...
    try:
        subprocess.run(cmd, capture_output=True, text=True, check=False)
        secure_log("info", f"Firewall rule removed for {protocol} port {port}")
    except Exception as e:
...
def adaptive_thresholds(process, port, count) -> bool:
    ...
    # adaptive: if variance is low, be more sensitive
    ...
    with lock:
        ...
        for conn in connections:
            ...
            if port not in ports:
                ...
                updated = True
        if updated:
            secure_log("info", "Learning: updated allowed ports.")
    if updated:
        with lock:
            state["last_new_port_time"] = datetime.utcnow()
        save_config()
    ...
    for conn in connections:
        ...
        if not learn_mode:
            if unknown_process or unauthorized_port:
                # threat correlation: check behavior + connections
                cpu = 0.0
                mem = 0
                try:
...
def launch_program(entry):
    ...
    try:
        # privilege isolation best-effort: no extra flags here, but daemon stays elevated
