...
#!/usr/bin/env python3
"""
Unified Autonomous Swarm Port Guardian with Universal Auto-Port Patching

Features:
- Auto-elevated (Admin)
- Port sharing allowed
- Auto-learning allowed ports per program
- Auto-flip from learning to enforcement after stability window
...
- Universal auto-port patching via auto-generated wrappers for Startup Python scripts
- Crash-loop detection (rate-limited restarts)
- Process-specific matching (name + command line path)
- Multi-node swarm sync (UDP heartbeat + rule sharing)
- Encrypted swarm packets (simple symmetric key)
- Distributed port baselines (merge from swarm)
...
- Basic process sandboxing (priority limiting + CPU affinity)
- Anomaly detection (simple statistical deviation)
- ML-style threat scoring (heuristic + anomaly + history)
"""
...
SWARM_KEY = "KILLER666_SWARM_KEY"  # simple shared secret
...
SANDBOX_CPU_PERCENT_LIMIT = 50  # soft target (placeholder)
SANDBOX_AFFINITY_LIMIT = 2      # limit to first N cores if sandboxed
...
state = {
    "allowed_ports": {},          # {process_name: [ports]}
    ...
    "swarm_peers": {},            # {node: last_seen}
    "port_usage_history": {},     # {(process, port): [counts]}
    "false_positive_counts": {},  # {(process, port): count}
}
...
def scanner_loop():
    ...
    while True:
        ...
        for conn in connections:
            ...
            anomaly = is_anomalous(name, port, count)
            if anomaly:
                event = {
                    "pid": pid,
                    "process": name,
                    ...
                    "unauthorized_port": False,
                    "anomaly": True,
                    "reason": "Anomalous port usage pattern",
                }
                record_event(event)
                sandbox_process(pid)
            snapshot.append({
...
# =========================
# UNIVERSAL AUTO-PORT PATCHING (WRAPPERS)
# =========================

def ensure_wrapper_for_script(script_path: str) -> str:
    """
    Given a Python script path in Startup, ensure a wrapper exists next to it.
    ...
    wrapper_path = os.path.join(folder, wrapper_name)

    if not os.path.exists(wrapper_path):
        console.info(f"[WRAPPER] Creating wrapper for {script_path}")
...
# =========================
# SWARM SYNC (HEARTBEAT + RULE MERGE)
# =========================
