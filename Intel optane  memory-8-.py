#!/usr/bin/env python3
"""
neurofabric_tier8_dappa.py

Tier‑8 NeuroFabric Organism (single file, DAPPA cockpit, tabbed GUI):

- Unified multi‑drive cache:
    * Detects all healthy drives
    * Uses them as a combined "Tier‑0" hot cache (striped small files)
    * Best drive as Tier‑1 hot store, fallback as Tier‑2 cold store
- GPU‑first vector intelligence (FAISS / CuPy / Torch if available)
- Autoencoder anomaly scoring + temporal window
- GNN‑style attack reasoning (NetworkX, PyTorch Geometric if available)
- Distributed mesh (UDP gossip + CRDT)
- Auto‑port FastAPI backend (no 8080 conflict)
- PySide6 DAPPA cockpit:
    * Tabbed interface (all panels as tabs)
    * System, Drives, Heat, Attack Graph, Anomalies, Telemetry, Swarm
    * Dark neon theme
"""
...
class UnifiedDriveCacheManager:
    """
    Treats all healthy drive roots as a combined Tier‑0 cache:
    - For small hot files, writes striped copies across all roots
    - Reads from the first available copy
    This leverages each drive's own hardware/OS cache and parallel I/O.
    """
...
# ---------------------------------------------------------------------------
# Tiered KV cache (integrated with unified drive cache)
# ---------------------------------------------------------------------------
...
class TieredKVCache:
    ...
    def put(self, key: str, value: Dict[str, Any]) -> None:
        ...
        # Tier‑0: write to unified drive cache
...
class DappaCockpit(QtWidgets.QMainWindow):
    def __init__(self, stats_worker: StatsWorker,
        ...
        self.setWindowTitle("NeuroFabric DAPPA Cockpit (Tier‑8)")
