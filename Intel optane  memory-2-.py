#!/usr/bin/env python3
"""
optane_neurofabric_tk_dual_hybrid_blacklist_autport.py
...
- Tkinter Tactical Cockpit (always-on):
    ...
    - Subsystem stats
    - Heat table
    - Shows active API port
- FastAPI HTTP service exposing KV + anomaly search
- Dual-mode in one process:
    - Tkinter main loop
    - FastAPI/uvicorn in background thread
- Auto-port selection:
    - Try preferred port (8080)
    - If busy, pick any free port in a range
    - Persist chosen port to service_port.txt
"""
...
def find_free_port(preferred: int = 8080, low: int = 20000, high: int = 40000) -> int:
    ...
    # Otherwise pick a random free port in range
    while True:
...
class NeuroFabricCockpitTk:
    def __init__(self,
        ...
        self.root.title("NeuroFabric Tactical Cockpit (Tk, Dual-Mode, Hybrid Auto-Blacklist + Auto-Port)")
