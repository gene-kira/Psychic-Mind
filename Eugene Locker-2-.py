...
# - Machine-bound via per-queen local secrets + OS/host
...
# -----------------------------
# Borg Queens (machine-bound keys)
# -----------------------------
...
class BorgQueen:
    ...
    All 3 Queens must be present (their secrets intact) to unwrap.
    """
    ...
    def get_key(self) -> bytes:
        ...
        h = hashlib.sha256()
        h.update(os_name + b"|" + host + b"|" + self.queen_id.encode("utf-8") + b"|" + local_secret)
        return h.digest()  # 32 bytes

    def lock_with_workers(self, data: bytes) -> bytes:
        # Workers are conceptual; we just log and do one AES-GCM
        key = self.get_key()
        return aes_gcm_encrypt(key, data)

    def unlock_with_workers(self, data: bytes) -> bytes:
        key = self.get_key()
        return aes_gcm_decrypt(key, data)


# -----------------------------
# Borg Collective wrapper (3 Queens)
# -----------------------------
...
class BorgCollectiveWrapper:
    ...
    If any Queen's key changes or secret is missing, unwrap fails.
    """
    ...
    def unwrap(self, blob: bytes) -> bytes:
        ...
        # Reverse order: Queen3, Queen2, Queen1
...
class App:
    def __init__(self, root):
        ...
        self.log_msg("[Borg] Three Queens online: alpha, beta, gamma. All required for lock/unlock.")
