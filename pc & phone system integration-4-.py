...
Cybernetic phone swarm organism with battle-ready organs:

- H.264 video streaming (Termux + ffmpeg -> WebSocket chunks)
- Bluetooth GATT control (IoT / BLE devices via bleak)
- Mesh networking between phones (UDP discovery + TCP links)
- Voice recognition organ (speech_recognition backend if installed)

MODES
-----

- MODE controller:
    * SwarmManager (USB + WiFi nodes, mesh-aware)
    * GUI cockpit (Tkinter):
        - Node list + status
        - Multi-node map (GPS)
        - Live sensor readout
        - Multi-camera grid (up to 4 nodes, JPEG snapshots)
        - Thermal / battery / CPU load indicators
        - Voice status
        - Bluetooth scan + GATT status
        - Simple graphs (CPU + battery history)
        - H.264 stream monitor (per-node last chunk size)

- MODE agent:
    * Termux-based phone agent:
        - WebSocket server on port 8765
        - Real sensors via termux-sensor + termux-location + termux-battery-status
        - Camera:
            - Snapshot via termux-camera-photo (JPEG)
            - H.264 streaming via ffmpeg (pipe over WebSocket)
        - AI inference via ONNX Runtime Mobile (real models if present)
        - Voice recognition organ (speech_recognition if installed)
        - Thermal / battery / CPU load organ
        - Bluetooth organ:
            - BLE scan (nearby devices)
            - GATT control (via bleak if installed)
        - Mesh networking:
            - UDP discovery
            - TCP peer links (basic message relay + info)
...
On Android (Termux agent):
    ...
    pkg install ffmpeg
    pip install websockets onnxruntime bleak speechrecognition
    python phone_swarm_organism.py --mode agent

Controller example commands (inside Python REPL or another script):

    # Start H.264 stream from a node
    await swarm.camera_start(node_id, h264=True)

    # Run BLE GATT read
    await swarm.bt_gatt(node_id, mac, service_uuid, char_uuid, "read")

    # Run BLE GATT write (hex data)
    await swarm.bt_gatt(node_id, mac, service_uuid, char_uuid, "write", data="010203")

    # Trigger voice recognition
    await swarm.run_task_on_node(node_id, "voice_recognize", {})

    # Get mesh info
    await swarm.mesh_info(node_id)
"""
...
class SwarmManager:
    def __init__(self) -> None:
        ...
        self.last_camera_frame: Dict[str, str] = {}
            ...
            try:
                ...
                if res.success:
                    ...
                    if cpu is not None:
                        ...
                        self.cpu_history[node.node_id] = self.cpu_history[node.node_id][-100:]
                    if batt is not None:
                        self.batt_history.setdefault(node.node_id, []).append((t, float(batt)))
                        self.batt_history[node.node_id] = self.batt_history[node.node_id][-100:]
            except Exception:
                pass
    ...
    async def run_task_on_node(
        ...
        try:
            ...
            if command == "camera_start" and ok:
                ...
                if frame:
                    self.last_camera_frame[node_id] = frame
    ...
    async def schedule_task(
        ...
        command: str,
        payload: Dict[str, Any],
        selector: Optional[Callable[[List[PhoneNode]], Optional[PhoneNode]]] = None,
    ) -> TaskResult:
        ...
        if selector is None:
            ...
            node = selector(candidates) or random.choice(candidates)
    ...
    async def camera_start(
        ...
        return await self.run_task_on_node(node_id, "camera_start", payload)

    async def camera_stop(self, node_id: str, stream_id: str) -> TaskResult:
        payload = {"stream_id": stream_id}
        return await self.run_task_on_node(node_id, "camera_stop", payload)
...
def onnx_infer_real(model_path: str, input_data: Any, params: Dict[str, Any]) -> Dict[str, Any]:
    if ort is None:
...
async def agent_h264_stream(ws, stream_id: str, resolution: List[int], fps: int) -> None:
    """
    H.264 streaming:
    - Uses ffmpeg to grab camera and encode H.264.
    - Sends TS chunks base64-encoded over WebSocket.
    Controller can monitor chunk sizes and optionally decode.
    """
...
# ---------------------------------------------------------------------------
# AGENT SIDE: Voice recognition
# ---------------------------------------------------------------------------

async def agent_voice_recognize() -> Dict[str, Any]:
...
# ---------------------------------------------------------------------------
# AGENT SIDE: Mesh networking (UDP discovery + TCP peers)
# ---------------------------------------------------------------------------
...
async def mesh_info() -> Dict[str, Any]:
    return {"peers": [{"id": k, "host": v[0], "port": v[1]} for k, v in MESH_PEERS.items()]}
...
async def agent_handle_request(ws, req: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if command == "ai_infer":
            ...
            result = onnx_infer_real(model, input_data, params)
            result["model"] = model
        ...
        if command == "camera_start":
            ...
            if h264:
                asyncio.create_task(agent_h264_stream(ws, stream_id, resolution, fps))
        ...
        if command == "voice_recognize":
            ...
            res = await agent_voice_recognize()
...
async def agent_main(host: str = "0.0.0.0", port: int = 8765) -> None:
    ...
    async with websockets.serve(agent_handle_client, host, port, ping_interval=20, ping_timeout=20):
...
# ---------------------------------------------------------------------------
# CONTROLLER SIDE: GUI (graphs + multi-camera grid + H.264 monitor)
# ---------------------------------------------------------------------------
...
class SwarmGUI:
    def __init__(self, swarm: SwarmManager) -> None:
        ...
        # Left: nodes + thermal + BT status + mesh + H.264
        ...
        # Center: sensors + voice + BT devices + graphs
        ...
        ttk.Label(center, text="CPU/Battery Graph", font=("Arial", 12, "bold")).pack(anchor="w")
        self.graph_canvas = tk.Canvas(center, width=400, height=150, bg="black")
        self.graph_canvas.pack(fill=tk.X)

        # Right: map + multi-camera grid
        right = ttk.Frame(self.main_frame)
        right.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        ttk.Label(right, text="Swarm Map", font=("Arial", 12, "bold")).pack(anchor="w")
        self.map_canvas = tk.Canvas(right, width=300, height=200, bg="black")
        self.map_canvas.pack()

        ttk.Label(right, text="Camera Grid (JPEG snapshots)", font=("Arial", 12, "bold")).pack(anchor="w")
    ...
    def update_gui(self) -> None:
        ...
        if nid and nid in self.swarm.nodes:
            ...
            if voice:
                self.voice_label.config(
                    text=f"Voice: listening={voice.get('listening')} last_text={voice.get('last_text')}"
                )
            else:
            ...
            if mesh:
                self.mesh_label.config(text=f"Mesh peers: {len(mesh.get('peers', []))}")
            else:
        ...
        for idx, lbl in enumerate(self.cam_labels):
            if idx < len(online_nodes):
                ...
                frame_b64 = self.swarm.last_camera_frame.get(nid2)
                if frame_b64:
                    try:
...
async def controller_async_main(swarm: SwarmManager, wifi_ip: Optional[str], wifi_port: int) -> None:
    ...
    for node in online[:4]:
        await swarm.camera_start(node.node_id, h264=False)
