"""
BORG-OS MEMORY BACKBONE ORGAN (v6.0 – Swarm, Telemetry, Water-Physics, Altered States)
...
- Lightweight Tkinter GUIs: MemoryVisualizer (GPU heatmap), CortexDashboard
- JSON IPC + TCP daemon
"""
...
class WaterPhysicsEngine:
    ...
    def flow_bias(self, energy: float, priority: float) -> int:
        # Positive bias (right) when high energy/priority and pressure is high
    ...
    def _run(self):
        while not self._stop:
            try:
                ...
                self.memory.debug_log.append(f"SystemTelemetry error: {e}")
            time.sleep(self.interval_sec)


class DiskTelemetryOrgan:
    ...
    def start(self):
        if self._thread is None and self._last is not None:
            self._thread = threading.Thread(target=self._run, daemon=True)
            self._thread.start()
    ...
    def _run(self):
        while not self._stop:
            try:
                ...
                self.memory.debug_log.append(f"DiskTelemetry error: {e}")
            time.sleep(self.interval_sec)


class ProcessBehaviorOrgan:
    ...
    def _run(self):
        while not self._stop:
            try:
                ...
                self.memory.debug_log.append(f"ProcessBehavior error: {e}")
            time.sleep(self.interval_sec)


class WindowsEventLogOrgan:
    ...
    def start(self):
        if platform.system() == "Windows" and self._thread is None:
            self._thread = threading.Thread(target=self._run, daemon=True)
            self._thread.start()
    ...
    def _run(self):
        # Stub: could use win32evtlog; here we just simulate a low-frequency health pulse
        while not self._stop:
            try:
                self.memory.inject(1, 2, 1, 0.8, {"metric": "eventlog_pulse"})
            except Exception as e:
                self.memory.debug_log.append(f"WindowsEventLog error: {e}")
            time.sleep(self.interval_sec)


class PacketSnifferOrgan:
    """
    Metadata-only: counts packets via psutil.net_io_counters.
    ...
    def _run(self):
        while not self._stop:
            try:
                ...
                self.memory.debug_log.append(f"PacketSniffer error: {e}")
            time.sleep(self.interval_sec)
    ...
    def _run(self):
        while not self._stop:
            try:
                ...
                self.memory.debug_log.append(f"BrowserTelemetry error: {e}")
            time.sleep(self.interval_sec)


class GameTelemetryOrgan:
    ...
    def _run(self):
        while not self._stop:
            try:
                ...
                self.memory.debug_log.append(f"GameTelemetry error: {e}")
            time.sleep(self.interval_sec)


class HardwareSensorsOrgan:
    def __init__(self, memory: BorgMemoryOrgan, interval_sec: float = 10.0):
        self.memory = memory
        self.interval_sec = interval_sec
        self._stop = False
    ...
    def _run(self):
        while not self._stop:
            try:
                ...
                self.memory.debug_log.append(f"HardwareSensors error: {e}")
            time.sleep(self.interval_sec)
    ...
    def _run(self):
        while not self._stop:
            try:
                ...
                self.memory.debug_log.append(f"CloudAPI error: {e}")
            time.sleep(self.interval_sec)
    ...
    def _run(self):
        while not self._stop:
            try:
                ...
                self.memory.debug_log.append(f"WeatherOrgan error: {e}")
            time.sleep(self.interval_sec)


class StockMarketOrgan:
    def __init__(self, memory: BorgMemoryOrgan, interval_sec: float = 60.0, url: str = ""):
        self.memory = memory
        self.interval_sec = interval_sec
        self.url = url
    ...
    def _run(self):
        while not self._stop:
            try:
                ...
                self.memory.debug_log.append(f"StockMarketOrgan error: {e}")
            time.sleep(self.interval_sec)
...
# =========================
# GUI: MemoryVisualizer + CortexDashboard
# =========================

class MemoryVisualizer:
    def __init__(self, organ: BorgMemoryOrgan, root=None, cell_size=25, tick_ms=300):
        if tkinter is None:
        ...
        title_suffix = f"{self.organ.personality.mode.upper()} | {self.organ.altered_state.state.upper()} | {'ADMIN' if self.organ.profile.is_admin else 'USER'}"
...
class CortexDashboard:
    def __init__(self, organ: BorgMemoryOrgan, root=None, tick_ms=500):
        ...
        self.lbl_disk = tkinter.Label(self.window, text="Disk R/W: 0/0 MB", fg="white", bg="black")
        self.lbl_disk.pack()
        ...
        self.lbl_state = tkinter.Label(self.window, text="State: focus", fg="magenta", bg="black")
        self.lbl_state.pack()
