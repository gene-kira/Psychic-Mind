"""
borg_kernel_omega_evolved_comfixed_v2.py

Universal user-space microkernel organism with:

- IPC fabric with basic capability enforcement
...
- Organs:
  ...
  - Constraint organ (negative-space rules with distributed logic, DSL-driven)
  - Prediction organ (simple forecasting)
  - Process watcher organ (new process detection)
  - Federation organ (multi-node topology exchange, TLS + HMAC-secured, persistent topology)
  - GPU inference organ (optional real model via PyTorch, with safe fallback)
  - UIAutomation organ (OS-aware hooks: Windows via pywinauto, COM-safe)
- Cluster topology (multi-node) with persistence
- Persistent state + rich reflection episodes
- Organ restart logic (self-healing)
- Distributed scheduling + organ migration hooks
- Constraint DSL with distributed evaluation
- Simple resource isolation hints + cgroup-based control (Linux-only, best-effort)
- Operator scripting language (mini-DSL) for constraints and actions
- PyQt5 cockpit GUI:
  ...
  - Processes
  - Topology
  - Controls (toggle organs, adjust CPU thresholds, run scripts)
  - Simple operator auth gate (password: admin)
"""
...
# We do NOT import pywinauto at module level to avoid COM initialization
_HAS_PYWIN = platform.system().lower().startswith("win")

# Optional COM support for Windows (for UIAutomation)
try:
    import pythoncom
    _HAS_PYTHONCOM = True
except Exception:
...
class IPCFabric:
    """
    Central IPC fabric: channels, endpoints, routing, backpressure, basic capability enforcement.
    """
    ...
    def send(self, msg: Message) -> List[str]:
        ...
        Enforces basic capability checks on receivers.
        ...
        with self._lock:
            ...
            for ep_id in recipients:
                ...
                if ep is None:
                    continue
                if not ep.can_recv(msg.dst, msg.type):
                    continue
        ...
        with self._lock:
            ...
            for nid, nd in data.get("nodes", {}).items():
                ...
                node_caps = NodeCapabilities(
                    ...
                    zone=caps.get("zone", "default"),
                )
                node = NodeInfo(
                    node_id=nid,
                    capabilities=node_caps,
                    organs=set(nd.get("organs", [])),
                    healthy=nd.get("healthy", True),
                    degraded=nd.get("degraded", False),
                )
                self.nodes[nid] = node
            for key, ld in data.get("links", {}).items():
                src = ld.get("src")
                dst = ld.get("dst")
                if src and dst:
                    link = LinkInfo(
                        src=src,
                        dst=dst,
                        latency_ms=ld.get("latency_ms", 0.0),
                        bandwidth_mbps=ld.get("bandwidth_mbps", 0.0),
                    )
                    self.links[(src, dst)] = link


# =========================
# Distributed scheduling + organ migration
# =========================

class DistributedScheduler:
    """
    Topology-aware placement hints for organs + migration decisions.
    """

    def __init__(self, topology: ClusterTopology):
        self.topology = topology

    def choose_node_for_organ(self, organ_name: str) -> str:
        best_node = None
        best_score = -1
        for node in self.topology.nodes.values():
            if not node.healthy:
                continue
            score = node.capabilities.cpu_cores
            if score > best_score:
                best_score = score
                best_node = node.node_id
        return best_node or "local"

    def should_migrate(self, current_node: str, organ_name: str) -> Optional[str]:
        best_node = None
        best_score = -1
        for node in self.topology.nodes.values():
            if not node.healthy or node.node_id == current_node:
                continue
            score = node.capabilities.cpu_cores
            if score > best_score:
                best_score = score
        ...
        if best_node is None:
            return None
        current_caps = self.topology.nodes.get(current_node)
        if current_caps and best_score > current_caps.capabilities.cpu_cores:
            return best_node
        return None
...
# =========================
# Constraint DSL (local + distributed)
# =========================
...
class ConstraintRule:
    ...
    op: ConstraintOp
    threshold: float
    target: str  # e.g. "background_organs", "node:node-2"
    action: str  # e.g. "throttle", "unthrottle", "migrate"


class ConstraintDSL:
    """
    Very simple DSL: rules over metrics -> actions.
    Supports local and distributed metrics (e.g. node.cpu_percent).
    """
...
# =========================
# Operator scripting language (mini DSL)
# =========================

class ScriptCommandType(enum.Enum):
...
class OperatorScriptEngine:
    """
    Tiny scripting language for operators.

    Grammar (line-based, very simple):

    ADD_CONSTRAINT name=<name> metric=<metric> op=<op> threshold=<float> target=<target> action=<action>
    REMOVE_CONSTRAINT name=<name>
    MIGRATE_ORGAN organ=<name> node=<node_id>
    """

    def __init__(self, kernel: "Kernel"):
        self.kernel = kernel

    def parse_line(self, line: str) -> Optional[ScriptCommand]:
        line = line.strip()
        ...
        if cmd == "ADD_CONSTRAINT":
            return ScriptCommand(ScriptCommandType.ADD_CONSTRAINT, kv)
        if cmd == "REMOVE_CONSTRAINT":
            return ScriptCommand(ScriptCommandType.REMOVE_CONSTRAINT, kv)
        if cmd == "MIGRATE_ORGAN":
            return ScriptCommand(ScriptCommandType.MIGRATE_ORGAN, kv)
        return None
    ...
    def execute(self, script: str) -> List[str]:
        ...
        for line in script.splitlines():
            ...
            if cmd is None:
                continue
            if cmd.type == ScriptCommandType.ADD_CONSTRAINT:
                try:
                    rule = ConstraintRule(
                        name=cmd.args["name"],
                        metric=cmd.args["metric"],
                        op=ConstraintOp(cmd.args["op"]),
                        threshold=float(cmd.args["threshold"]),
                        target=cmd.args["target"],
                        action=cmd.args["action"],
                    )
                    self.kernel.constraint_dsl.add_rule(rule)
                    logs.append(f"Added constraint {rule.name}")
                except Exception as e:
                    logs.append(f"Error adding constraint: {e}")
            elif cmd.type == ScriptCommandType.REMOVE_CONSTRAINT:
                name = cmd.args.get("name")
                if not name:
                    logs.append("Missing name for REMOVE_CONSTRAINT")
                    continue
                before = len(self.kernel.constraint_dsl.rules)
                self.kernel.constraint_dsl.rules = [r for r in self.kernel.constraint_dsl.rules if r.name != name]
                after = len(self.kernel.constraint_dsl.rules)
                logs.append(f"Removed {before - after} constraints named {name}")
            elif cmd.type == ScriptCommandType.MIGRATE_ORGAN:
                organ = cmd.args.get("organ")
                node = cmd.args.get("node")
                if not organ or not node:
                    logs.append("Missing organ or node for MIGRATE_ORGAN")
                    continue
                self.kernel.request_organ_migration(organ, node)
                logs.append(f"Requested migration of {organ} to {node}")
        return logs
...
class Cortex(Organ):
    ...
    def _handle_proposal(self, proposal: AIProposal) -> None:
        ...
        if proposal.proposal_type == ProposalType.MIGRATE_ORGAN:
            organ = proposal.payload.get("organ")
            target = proposal.payload.get("target_node")
            if organ and target:
                self.kernel.request_organ_migration(organ, target)
...
class SimpleAIPolicyOrgan(Organ):
    """
    Simple AI organ that periodically emits dummy proposals, including migration hints.
    """
    ...
    def tick(self) -> None:
        target = self.kernel.distributed_scheduler.should_migrate(self.kernel.node_id, "gpu_inference")
        if target and target != self.kernel.node_id:
            proposal = AIProposal(
                id=str(uuid.uuid4()),
                ts=time.time(),
                source_organ=self.name,
                proposal_type=ProposalType.MIGRATE_ORGAN,
                payload={"organ": "gpu_inference", "target_node": target},
                confidence=0.7,
                rationale=f"Heuristic: node {target} has more CPU cores; migrate gpu_inference.",
            )
...
class TelemetryOrgan(Organ):
    ...
    def _collect_snapshot(self) -> Dict[str, Any]:
        try:
            ...
            # Robust thread_count: skip processes that vanish between listing and inspection
            thread_count = 0
            for p in processes:
                try:
...
# =========================
# Constraint organ (local + distributed)
# =========================

class ConstraintOrgan(Organ):
    """
    Encodes negative-space rules via DSL and throttles background organs.
    Also supports distributed constraint logic using remote metrics.
    """
    ...
    def tick(self) -> None:
        ...
        fired_local = self.kernel.constraint_dsl.eval(snap)
        for rule in fired_local:
            self._apply_rule(rule, snap, local=True)

        for node_id, node in self.kernel.topology.nodes.items():
            metrics = {
                f"node.{node_id}.cpu_cores": node.capabilities.cpu_cores,
                f"node.{node_id}.memory_mb": node.capabilities.memory_mb,
            }
            fired_remote = self.kernel.constraint_dsl.eval(metrics)
            for rule in fired_remote:
                self._apply_rule(rule, metrics, local=False, node_id=node_id)

    def _apply_rule(self, rule: ConstraintRule, metrics: Dict[str, Any], local: bool, node_id: Optional[str] = None):
        if rule.target == "background_organs" and local:
            if rule.action == "throttle" and not self.kernel.throttle_background:
                self.kernel.throttle_background = True
                self._record(rule, metrics, "throttle_background_on")
            elif rule.action == "unthrottle" and self.kernel.throttle_background:
                self.kernel.throttle_background = False
                self._record(rule, metrics, "throttle_background_off")
        elif rule.target.startswith("node:") and not local:
            target_node = rule.target.split(":", 1)[1]
            if target_node == node_id and rule.action == "migrate":
                organ = "gpu_inference"
                self.kernel.request_organ_migration(organ, target_node)
                self._record(rule, metrics, f"migrate_{organ}_to_{target_node}")
    ...
    def _record(self, rule: ConstraintRule, snap: Dict[str, Any], action: str) -> None:
        ...
        self.kernel.record_episode(
            ...
            severity="warning" if "throttle" in action or "migrate" in action else "info",
...
# =========================
# GPU inference organ (real model if available)
# =========================

class GPUInferenceOrgan(Organ):
    """
    GPU inference organ. Uses a tiny dummy model if PyTorch is available, otherwise stub.
    """
    ...
    def init(self) -> None:
        if _HAS_TORCH:
            try:
                ...
                print(f"[GPUInference] Failed to init model: {e}")
                self.model = None
    ...
    def tick(self) -> None:
        ...
        if self.model is not None and _HAS_TORCH:
            try:
                x = torch.tensor(
                    [
                        [
                            float(snap.get("cpu_percent", 0.0)),
                            float(snap.get("memory_percent", 0.0)),
                            float(snap.get("process_count", 0.0)),
                            float(snap.get("thread_count", 0.0)),
                        ]
                    ],
                    dtype=torch.float32,
                    device=self.device,
                ...
                print(f"[GPUInference] Inference error: {e}")
                score = 0.0
        else:
            score = 0.0
        ...
        signal = {
            "ts": time.time(),
            "note": "GPU inference executed" if self.model is not None else "GPU inference stub executed",
            "score": score,
        }
        self.kernel.record_episode(
            ...
            metrics_before={},
            metrics_after={},
            lessons=["GPU inference organ ran"],
            followups=[]
        )


# =========================
# UIAutomation organ (OS-aware, COM-safe)
# =========================

class UIAutomationOrgan(Organ):
    """
    UIAutomation organ. On Windows, uses pywinauto to inspect top-level windows.
    COM is initialized per-tick in this worker thread to avoid conflicts with PyQt.
    On other OSes, acts as a stub.
    """
    ...
    def tick(self) -> None:
        ...
        # COM-safe: initialize COM in this worker thread only, not in the main GUI thread
        if _HAS_PYTHONCOM:
            pythoncom.CoInitialize()
        try:
            # Lazy import pywinauto here, after COM init, to avoid Qt COM mode conflicts
            try:
                from pywinauto import Desktop
            except Exception as e:
                print(f"[UIAutomation] pywinauto import failed: {e}")
                self.kernel.record_episode(
                    trigger="uiautomation_tick",
                    severity="warning",
                    tags=["uiautomation"],
                    ...
                    metrics_before={},
                    metrics_after={},
                    lessons=["UIAutomation import failed"],
                    followups=[]
                )
                return
            ...
            for w in windows[:20]:
                try:
                    info.append({"title": w.window_text(), "class_name": w.class_name()})
                except Exception:
                    continue
            self.kernel.record_episode(
                trigger="uiautomation_tick",
                severity="info",
                tags=["uiautomation", "windows"],
                context={"node_id": self.kernel.node_id, "ts_start": time.time()},
                signals={"windows": info},
                constraints_active={},
                decision={"action": "inspect_windows"},
                alternatives=[],
                outcome={"status": "ok"},
                metrics_before={},
                metrics_after={},
                lessons=["UIAutomation inspected top-level windows"],
                followups=[]
            )
        except Exception as e:
...
# =========================
# Federation organ (TLS + HMAC, persistent topology)
# =========================

class FederationOrgan(Organ):
    """
    Multi-node federation with TLS and HMAC authentication.
    """
    ...
    def _create_server_socket(self) -> socket.socket:
        ...
        s.listen(5)
        s.settimeout(1.0)
        if self.kernel.tls_enabled:
            context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
            context.load_cert_chain(self.kernel.tls_certfile, self.kernel.tls_keyfile)
    ...
    def _server_loop(self) -> None:
        try:
            ...
            print(f"[Federation] Listening on port {self.listen_port} (TLS={self.kernel.tls_enabled})")
    ...
    def _apply_remote_topology(self, body: Dict[str, Any]) -> None:
        try:
            ...
            self.kernel.persist_topology()
        except Exception as e:
            print(f"[Federation] Error applying remote topology: {e}")

    def _create_client_socket(self, host: str, port: int) -> socket.socket:
        s = socket.create_connection((host, port), timeout=1.0)
        if self.kernel.tls_enabled:
            context = ssl.create_default_context(ssl.Purpose.SERVER_AUTH)
            if self.kernel.tls_ca_file:
                context.load_verify_locations(self.kernel.tls_ca_file)
    ...
    def tick(self) -> None:
        ...
        for host, port in self.kernel.federation_peers:
            try:
                with self._create_client_socket(host, port) as c:
                    c.sendall(data)
            except Exception:
                continue
...
# =========================
# cgroup-based resource control (Linux-only, best-effort)
# =========================

class CGroupManager:
    """
    Very simple cgroup v1/v2 helper for CPU limits (best-effort, no guarantees).
    """

    def __init__(self, group_name: str = "borg_kernel"):
        self.group_name = group_name
        self.enabled = platform.system().lower() == "linux"
    ...
    def _init_cgroup(self):
        try:
            if os.path.exists(os.path.join(self.base_path, "cpu")):
                self.cpu_path = os.path.join(self.base_path, "cpu", self.group_name)
            else:
                self.cpu_path = os.path.join(self.base_path, self.group_name)
            os.makedirs(self.cpu_path, exist_ok=True)
        except Exception as e:
            print(f"[CGroup] Failed to init cgroup: {e}")
            self.enabled = False

    def set_cpu_quota(self, quota_percent: float) -> None:
        if not self.enabled or self.cpu_path is None:
            return
        try:
            period_file = os.path.join(self.cpu_path, "cpu.cfs_period_us")
            quota_file = os.path.join(self.cpu_path, "cpu.cfs_quota_us")
            if not os.path.exists(period_file) or not os.path.exists(quota_file):
                return
            with open(period_file, "r") as f:
                period = int(f.read().strip())
            quota = int(period * quota_percent / 100.0)
            with open(quota_file, "w") as f:
                f.write(str(quota))
        except Exception as e:
            print(f"[CGroup] Failed to set CPU quota: {e}")

    def add_pid(self, pid: int) -> None:
        if not self.enabled or self.cpu_path is None:
            return
        try:
            tasks_file = os.path.join(self.cpu_path, "tasks")
            if os.path.exists(tasks_file):
                with open(tasks_file, "a") as f:
                    f.write(str(pid) + "\n")
        except Exception as e:
            print(f"[CGroup] Failed to add pid: {e}")
...
class Kernel:
    ...
    def __init__(self,
        ...
        self.distributed_scheduler = DistributedScheduler(self.topology)
        self.script_engine = OperatorScriptEngine(self)
        ...
        self.pending_migrations: "queue.Queue[Tuple[str, str]]" = queue.Queue()
    ...
    def _restore_topology(self) -> None:
        topo_data = self.store.state.get("topology")
        if topo_data:
            self.topology.from_dict(topo_data)
    ...
    def _init_topology_self_node(self) -> None:
        ...
        caps = NodeCapabilities(
            ...
            has_gpu=_HAS_TORCH and torch.cuda.is_available(),
    ...
    def persist_topology(self) -> None:
        self.store.state["topology"] = self.topology.to_dict()
    ...
    def request_organ_migration(self, organ_name: str, target_node: str) -> None:
        self.pending_migrations.put((organ_name, target_node))

    def process_migrations(self) -> None:
        while not self.pending_migrations.empty():
            organ_name, target_node = self.pending_migrations.get()
            if target_node == self.node_id:
                continue
            if organ_name not in self.organs:
                continue
            self._migrate_organ(organ_name, target_node)

    def _migrate_organ(self, organ_name: str, target_node: str) -> None:
        organ = self.organs.get(organ_name)
        if not organ:
            return
        organ.stop()
        del self.organs[organ_name]
        self.topology.nodes[self.node_id].organs.discard(organ_name)
        self.persist_topology()
        self.record_episode(
            trigger="organ_migration",
            severity="info",
            tags=["organ", "migration"],
            context={"node_id": self.node_id, "ts_start": time.time(), "organ": organ_name},
            signals={"target_node": target_node},
            constraints_active={},
            decision={"action": "migrate_out"},
            alternatives=[],
            outcome={"status": "migrated_out"},
            metrics_before={},
            metrics_after={},
            lessons=[f"Organ {organ_name} migrated out to {target_node} (logical)"],
            followups=[]
        )

    def apply_cgroup_limits(self) -> None:
        if not self.cgroup_manager.enabled:
            return
        try:
            self.cgroup_manager.add_pid(os.getpid())
            if self.throttle_background:
                self.cgroup_manager.set_cpu_quota(50.0)
            else:
                self.cgroup_manager.set_cpu_quota(100.0)
        except Exception as e:
            print(f"[Kernel] cgroup limit error: {e}")
...
class AuthDialog(QtWidgets.QDialog):
    """
    Simple operator authentication dialog.
    """

    def __init__(self, expected_password: str, parent=None):
        super().__init__(parent)
        self.expected_password = expected_password
        self.setWindowTitle("Operator Authentication")
        self.setModal(True)
        layout = QtWidgets.QVBoxLayout()
        layout.addWidget(QtWidgets.QLabel("Enter operator password:"))
    ...
    def accept(self):
        if self.edit.text() == self.expected_password:
            super().accept()
        else:
...
class CockpitWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, kernel: Kernel):
        super().__init__()
        self.kernel = kernel
        self.setWindowTitle("Borg Kernel Cockpit (Omega Evolved)")
        ...
        self.tabs.addTab(self.script_tab, "Operator Script")
    ...
    def _init_script_tab(self):
        ...
        self.script_edit.setPlaceholderText(
            "Example:\n"
            "ADD_CONSTRAINT name=remote_migrate metric=node.node-2.cpu_cores op=>= threshold=8 target=node:node-2 action=migrate\n"
            "MIGRATE_ORGAN organ=gpu_inference node=node-2\n"
        )
        self.run_script_button = QtWidgets.QPushButton("Run script")
        self.run_script_button.clicked.connect(self._run_script)
        self.script_output = QtWidgets.QTextEdit()
        self.script_output.setReadOnly(True)
        layout.addWidget(QtWidgets.QLabel("Operator script:"))
    ...
    def _run_script(self):
        ...
        logs = self.kernel.script_engine.execute(script)
    ...
    def refresh_views(self):
        self.kernel.process_migrations()
        self.kernel.apply_cgroup_limits()
    ...
    def _refresh_status(self):
        self.status_label.setText(f"Kernel node: {self.kernel.node_id} (TLS={self.kernel.tls_enabled})")
...
def main():
    kernel = Kernel(
        ...
        tls_enabled=False,          # set True and provide cert/key/ca for real TLS
        tls_certfile=None,
        tls_keyfile=None,
        tls_ca_file=None,
    )
    ...
    auth = AuthDialog(kernel.operator_password)
