#!/usr/bin/env python3
"""
optane_neurofabric_tk_dual_hybrid_blacklist.py

NeuroFabric spine backed by Optane/NVMe with:

- Auto-detects best Optane/NVMe-like target
- Drive health scanner (psutil-based)
- Drive blacklist / whitelist
- Hybrid auto-blacklist:
    - Runtime auto-blacklist on first failure
    - Persistent auto-blacklist (auto_blacklist.json) on repeated failures
- Dynamic failover to local SSD if Optane/NVMe becomes unhealthy
- Tiered hot/cold storage:
    ...
    - Swarm node state
- Heat tracking + migration daemon (cold -> hot)
- LLM KV-cache adapter (per-model, per-session)
- Concrete KV-cache shard layout (mmap'd tensors on Optane)
- Anomaly embedding store with cosine nearest-neighbor search
- Tkinter Tactical Cockpit (always-on):
    - Multi-frame layout
    - Optane usage
    - Subsystem stats
    - Heat table
- FastAPI HTTP service exposing KV + anomaly search
- Dual-mode in one process:
    - Tkinter main loop
    - FastAPI/uvicorn in background thread
"""
...
def ensure_libs():
    ...
    for lib in REQUIRED_LIBS:
        try:
            ...
            missing.append(lib)
    if missing:
        print("[neurofabric] Missing libraries:", ", ".join(missing))
        print("Install with:")
...
# Manual blacklist (hard-coded)
DRIVE_BLACKLIST: set[str] = {
    "a:", "b:",
    # add more letters if needed, e.g. "g:"
}

# Manual whitelist (overrides any blacklist)
DRIVE_WHITELIST: set[str] = set()

# Runtime auto-blacklist (first failure in this run)
RUNTIME_BLACKLIST: set[str] = set()

# Persistent auto-blacklist (stored on disk)
PERSISTENT_BLACKLIST: set[str] = set()

# Failure counts per drive letter (for hybrid logic)
...
def note_drive_failure(letter: Optional[str], exc: BaseException):
    ...
    if DRIVE_WHITELIST and letter in DRIVE_WHITELIST:
        # Whitelist overrides auto-blacklist
        return
    # Increment failure count
    count = DRIVE_FAIL_COUNTS.get(letter, 0) + 1
    DRIVE_FAIL_COUNTS[letter] = count
    # First failure -> runtime blacklist
    if count == 1:
        RUNTIME_BLACKLIST.add(letter)
    # Second or more failures -> persistent blacklist
...
# ---------------------------------------------------------------------------
# Optane / NVMe auto-detection with health + failover
# ---------------------------------------------------------------------------
...
class LLMKVAdapter:
    ...
    def save_cache_meta(self,
        ...
        key = self._key(model_id, session_id)
        self.kv.put(key, meta)
...
# ---------------------------------------------------------------------------
# Concrete KV-cache layout: mmap'd tensor shards on Optane
# ---------------------------------------------------------------------------
...
class AnomalyEmbeddingStore:
    ...
    def get_anomaly(self,
                    anomaly_id: str) -> Optional[Tuple[np.ndarray, Dict[str, Any], str]]:
        vec_id = f"anomaly:{anomaly_id}"
        return self.vecdb.get(vec_id)

    def upsert_attack_node(self,
                           graph_id: str,
                           node_id: str,
                           embedding: np.ndarray,
                           meta: Dict[str, Any],
    ...
    def get_attack_node(self,
                        graph_id: str,
                        node_id: str) -> Optional[Tuple[np.ndarray, Dict[str, Any], str]]:
        vec_id = f"attack_node:{graph_id}:{node_id}"
        return self.vecdb.get(vec_id)
    ...
    def search_nearest(self,
        ...
        ids = [vid for vid in self.vecdb.all_ids() if vid.startswith(prefix)]
        ...
        for vid in ids:
            ...
            e_norm = np.linalg.norm(emb) + 1e-8
...
class NeuroFabricCockpitTk:
    def __init__(self,
        ...
        self.root.title("NeuroFabric Tactical Cockpit (Tk, Dual-Mode, Hybrid Auto-Blacklist)")
    ...
    def _build_ui(self):
        ...
        self.path_label = ttk.Label(top_frame, text=f"Optane root: {self.optane_root}")
        ...
        self.usage_bar = ttk.Progressbar(top_frame, orient="horizontal", mode="determinate", maximum=100)
        ...
        heat_scroll = ttk.Scrollbar(mid_right, orient="vertical", command=self.heat_list.yview)
        ...
        self.refresh_button = ttk.Button(bottom, text="Refresh now", command=self._refresh_from_snapshot)
    ...
    def _refresh_from_snapshot(self):
        ...
        self.path_label.config(text=f"Optane root: {root_str}")
        ...
        self.status_label.config(text=f"Status: updated @ {time.strftime('%H:%M:%S', time.localtime(ts))}")
...
def build_neurofabric_app(kv: TieredKVCache,
    ...
    def anomaly_search(req: SearchRequest):
        ...
        hits = [
            SearchHit(id=vid, score=score, meta=meta)
...
def start_service_in_thread(kv: TieredKVCache,
                            ...
                            port: int = 8080):
    import uvicorn
    app = build_neurofabric_app(kv, vecdb)
...
def main():
    ...
    service_thread = start_service_in_thread(kv, vecdb)
