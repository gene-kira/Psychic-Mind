"""
UNIFIED VIDEO AI PIPELINE (PERFORMANCE + TRACKING + NETWORK + VOICE + GPU HUD + PLUGINS + REMOTE API)

New features:
- Performance:
  - Threaded frame capture
  - Frame queue with skipping under load
- Object tracking:
  - Simple IOU-based tracker assigning stable IDs
- Multi-node networking:
  - UDP broadcast of state
  - UDP listener for remote control messages
- Voice control (optional):
  - Speech commands to toggle YOLO/RL, FPS, mode
- GPU-accelerated HUD (optional):
  - CUDA-based blending when available
- Plugin architecture:
  - Dynamic plugin loading from "plugins" directory
  - Hooks for state, actions, and drawing
- Remote control API:
  - HTTP server with JSON endpoints for status and control
- Existing features preserved:
  - YOLO detection, RL, physics, predictive reasoning
  - Persistent memory with drive/SMB/folder selection
  - Centered popup for save location
  - Dark HUD with green accents
"""
...
class PluginHub:
    ...
    def load_plugins(self, directory="plugins"):
        if not os.path.isdir(directory):
            print(f"[PLUGIN] No plugin directory '{directory}'.")
            return
        for fname in os.listdir(directory):
            if not fname.endswith(".py"):
                continue
            mod_name = fname[:-3]
            full_name = f"{directory}.{mod_name}"
            try:
    ...
    def on_state(self, state: Dict[str, Any]):
        for p in self.plugins:
            try:
                ...
                print(f"[PLUGIN] on_state error in {p.__name__}: {e}")
    ...
    def on_action(self, action: Dict[str, Any]):
        for p in self.plugins:
            try:
                ...
                print(f"[PLUGIN] on_action error in {p.__name__}: {e}")
    ...
    def on_draw(self, frame):
        for p in self.plugins:
            try:
                ...
                print(f"[PLUGIN] on_draw error in {p.__name__}: {e}")
...
def main():
    ...
    try:
        while True:
            ...
            # Apply network control commands
