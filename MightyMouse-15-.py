...
def lookup_country_for_ip(ip: str) -> str:
    ...
    Right now returns 'UNKNOWN'; you can wire in a real DB/API later.
    """
    return "UNKNOWN"
...
class WebBrain:
    ...
    def _update_domain_stats(self, domain, score, status):
        ...
        stats["samples"] += 1
        stats["avg_score"] = ((stats["avg_score"] * (stats["samples"] - 1)) + score) / stats["samples"]
        if status in ("GOOD", "LEARNED"):
            stats["good_sessions"] += 1
        elif status == "MISMATCH":
            stats["bad_sessions"] += 1
    ...
    def evaluate(self):
        ...
        if domain:
            self._update_domain_stats(domain, score, status)
            stats = self.domain_stats.get(domain, {})
            if stats.get("bad_sessions", 0) >= 3:
                self.hud.push("hazard", f"Domain {domain} has repeated mismatches – treat as HIGH RISK")
...
# =========================
# GAME COGNITION (PREDICTIVE + ANOMALY)
# =========================
...
class GameServerWatcher:
    """
    Watches active game processes and their remote server IPs.
    Auto-learns first IP per game, then enforces GOOD / LEARNED / MISMATCH.
    """
...
class NetworkAnomalyMonitor:
    """
    Watches all outbound connections, learns a baseline for 1 hour,
    then only records anomalies/threats and maps them into a 'network cone'.
    Only anomalies are shown in the cone.
    """
...
class CockpitGUI(QWidget):
    def __init__(self, orch, hud, web, fs, game, profiles, memory):
        ...
        self.server_detail_label = QLabel("Server Trust Details:")
        self.server_detail_box = QTextEdit()
        self.server_detail_box.setReadOnly(True)
        self.server_detail_box.setStyleSheet("background-color: white; color: black;")
        main_layout.addWidget(self.server_detail_label)
        main_layout.addWidget(self.server_detail_box)

        self.cone_label = QLabel("3D Cone Visualization (game enemies, textual):")
    ...
    def update_gui(self):
        ...
        if self.web.latest_url:
            ...
            if status_line:
                ...
                self.web_status_label.setText(f"Web IP Status: {status}")
            else:
                status = "UNKNOWN"
                self.web_status_label.setText("Web IP Status: (unknown)")
            ...
            self.web_status_label.setText("Web IP Status: (none)")
        ...
        if server_status == "GOOD":
            details += "Reason: Matches trusted baseline\n"
        elif server_status == "LEARNED":
            details += "Reason: First time seen, stored as trusted\n"
        elif server_status == "MISMATCH":
            details += "Reason: Does not match trusted baseline\n"
        else:
            details += "Reason: No server detected\n"

        self.server_detail_box.setPlainText(details)
...
def main():
    ...
    game_server_watcher = GameServerWatcher(memory, hud, orch, interval=2.0)
    game_server_watcher.start()

    network_monitor = NetworkAnomalyMonitor(memory, hud, orch)
