"""
BORG-OS MEMORY BACKBONE ORGAN (v9.0 – Encrypted Swarm + Faster GUI)
...
- Real telemetry organs (multi-threaded, auto-paced):
  - CPU/RAM, Disk, Process behavior, Windows Event Log, Network, Packet flow
...
- Swarm networking organ: encrypted UDP JSON messages (AES-GCM + ChaCha20-Poly1305 hybrid)
"""
...
class SwarmCrypto:
    ...
    - HMAC-SHA256 for extra integrity
    """
    ...
    def encrypt(self, payload: dict) -> bytes:
        ...
        if self.aes is not None:
            ...
            # fallback: no encryption, just HMAC
            ct = body
            mode = b"N"
...
class SwarmNetworkingOrgan(BaseOrganThread):
    """
    Encrypted UDP-based swarm networking:
    - Broadcasts local blacklist + threat level + anomaly score
    - Receives messages from peers and forwards to firewall/memory
    - Uses AES-GCM / ChaCha20-Poly1305 + HMAC-SHA256
    """
    def __init__(
        self,
        memory: BorgMemoryOrgan,
    ...
    def _default_key(self) -> bytes:
        # Simple deterministic key per user/host (for now)
...
# =========================
# GUI: CortexDashboard (Extended Mode, Faster)
# =========================

class CortexDashboard:
    def __init__(self, organ: BorgMemoryOrgan, base_tick_ms=80):
        if tkinter is None:
    ...
    def _update(self):
        ...
        self._set_label(self.lbl_disk, "disk", f"Disk R/W: {o.last_disk_read} / {o.last_disk_write} MB")
        self._set_label(self.lbl_gpu_util, "gpu_util", f"GPU Util: {o.last_gpu_util}%")
        self._set_label(self.lbl_gpu_temp, "gpu_temp", f"GPU Temp: {o.last_gpu_temp}°C")
        self._set_label(self.lbl_net_threat, "net_threat", f"Net Threat: {o.last_net_threat}")
        self._set_label(self.lbl_state, "state", f"State: {o.altered_state.state}")
        ...
        self._set_label(self.lbl_avg_cpu, "avg_cpu", f"Avg CPU: {avg_cpu:.1f}%")
        self._set_label(self.lbl_avg_mem, "avg_mem", f"Avg MEM: {avg_mem:.1f}%")
        self._set_label(self.lbl_avg_gpu, "avg_gpu", f"Avg GPU: {avg_gpu:.1f}%")
        self._set_label(self.lbl_avg_threat, "avg_threat", f"Avg Threat: {avg_threat:.1f}")

        # Graphs
        self._set_label(self.lbl_graph_cpu, "graph_cpu", "CPU Graph:    " + self._ascii_graph(o.history_cpu))
        self._set_label(self.lbl_graph_mem, "graph_mem", "MEM Graph:    " + self._ascii_graph(o.history_mem))
        self._set_label(self.lbl_graph_gpu, "graph_gpu", "GPU Graph:    " + self._ascii_graph(o.history_gpu))
        self._set_label(self.lbl_graph_threat, "graph_threat", "Threat Graph: " + self._ascii_graph(o.history_threat))

        # Threat lineage
        lines = []
        for entry in o.threat_lineage[-5:]:
            lines.append(f"{entry.get('origin_node')} → {entry.get('reason')}")
        lineage_text = "Threat Lineage:\n" + ("\n".join(lines) if lines else "(none)")
        ...
        if o.threat_lineage:
            ...
            fw_text = f"Firewall Reason: {reason}"
        else:
            fw_text = "Firewall Reason: n/a"
        self._set_label(self.lbl_fw_reason, "fw_reason", fw_text)
        ...
        anomaly_text = "Anomaly Events:\n" + ("\n".join(alines) if alines else "(none)")
        self._set_label(self.lbl_anomaly_events, "anomaly_events", anomaly_text)
        ...
        self._set_label(self.lbl_profiler, "profiler", profiler_text)
...
def main():
    ...
    if tkinter is not None:
        cortex = CortexDashboard(organ, base_tick_ms=80)
        cortex.run()
    else:
        while True:
            time.sleep(1.0)
