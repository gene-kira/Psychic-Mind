...
# -*- coding: utf-8 -*-
"""
Dual-Socket Simulator v4 – Real-Time Enterprise Cockpit

Features:
- Real-time structured workloads (web, db, ml, backup + custom)
- Two heterogeneous sockets: Desktop CPU + Xeon CPU
- Policy-based scheduler (Balanced / Latency / Throughput / Memory / Reliability)
- Industrial dual gauges (per-socket utilization) with needle + digital readout
- Per-workload timelines (rolling intensity graph)
- Pinning controls (force workload to a socket, or unpin)
- Selection-aware right-side panel (details + timeline + pin controls)
- Dark cockpit theme
- Smooth real-time updates via QTimer
...
class Workload:
    ...
    pinned: Optional[int] = None          # None, 0, or 1
    ...
    def describe(self) -> str:
        pin_str = "None" if self.pinned is None else str(self.pinned)
        return (
...
def assign_workloads(state: SimulationState) -> str:
    ...
    for w in state.workloads:
        if w.pinned is not None:
            w.assigned_socket = w.pinned
            lines.append(
                f"[{state.policy}] Workload '{w.name}' ({w.role}) pinned → "
                f"{state.sockets[w.pinned].name}"
            )
            continue
...
def compute_socket_utilization(state: SimulationState, socket_index: int) -> float:
    ...
    socket_weight = 0.0
    for w in state.workloads:
        w_weight = w.current_intensity()  # max 40
...
class WorkloadTableModel(QtCore.QAbstractTableModel):
    HEADERS = ["Name", "Role", "Latency", "Throughput", "Memory", "Reliability", "Pinned", "Assigned Socket"]
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        if role == QtCore.Qt.DisplayRole:
            if col == 0:
                ...
                return "-" if w.pinned is None else str(w.pinned)
            elif col == 7:
...
class TimelineWidget(QtWidgets.QWidget):
    ...
    def setHistory(self, history: List[int]):
...
class DualSocketSimulator(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Dual-Socket Simulator v4 – Real-Time Enterprise Cockpit")
    ...
    def _build_ui(self):
        ...
        # Middle: table + controls + right panel
        ...
        # Right-side panel: details + pinning + timeline
        ...
        self.pin0_btn = QtWidgets.QPushButton("Pin to Socket 0 (Desktop)")
        self.pin1_btn = QtWidgets.QPushButton("Pin to Socket 1 (Xeon)")
        self.unpin_btn = QtWidgets.QPushButton("Unpin")
        self.pin0_btn.clicked.connect(self._on_pin0)
        self.pin1_btn.clicked.connect(self._on_pin1)
        self.unpin_btn.clicked.connect(self._on_unpin)
        pin_layout.addWidget(self.pin0_btn)
        pin_layout.addWidget(self.pin1_btn)
        pin_layout.addWidget(self.unpin_btn)
        right_layout.addLayout(pin_layout)

        self.timeline_widget = TimelineWidget()
        right_layout.addWidget(self.timeline_widget, stretch=3)

        mid_layout.addWidget(right_panel, stretch=3)
    ...
    def _on_pin0(self):
        ...
        w.pinned = 0
        self.status_bar.showMessage(f"Workload '{w.name}' pinned to Socket 0.")
        self._run_scheduler_and_update_ui()
    ...
    def _on_pin1(self):
        ...
        w = self.state.workloads[self.selected_workload_index]
        w.pinned = 1
        self.status_bar.showMessage(f"Workload '{w.name}' pinned to Socket 1.")
        self._run_scheduler_and_update_ui()
    ...
    def _update_selected_panel(self):
        if self.selected_workload_index is None or self.selected_workload_index >= len(self.state.workloads):
            ...
            self.timeline_widget.setHistory([])
            return

        w = self.state.workloads[self.selected_workload_index]
        self.selected_details.setPlainText(w.describe())
        self.timeline_widget.setHistory(w.history)
