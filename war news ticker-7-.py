    ...
    def update(self, stress, sentiment_score, headline_count, ui_snapshot):
        ...
        if self.mlp_trained:
            try:
                ...
                predicted_stress_mlp = float(self.mlp.predict([[s, ds, sent, hc, inten]])[0])
                predicted_stress_mlp = max(0.0, min(predicted_stress_mlp, 100.0))
            except Exception as e:
                log(f"[MLP] Prediction error: {e}")
        combined_pred = 0.6 * predicted_stress_lin + 0.4 * predicted_stress_mlp
...
class HormuzCockpit(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Global Chokepoint Impact Cockpit (Deep + Predictive)")
    ...
    def _build_ui(self):
        ...
        self.title_label = tk.Label(
            top,
            text="Global Chokepoint Impact Monitor (Deep + Predictive + UI-Aware)",
            fg="#00ffcc",
            bg="#101018",
...
if __name__ == "__main__":
    log("=== Starting Global Chokepoint Cockpit (Deep + Predictive + UI-Aware) ===")
    app = HormuzCockpit()
    app.mainloop()
