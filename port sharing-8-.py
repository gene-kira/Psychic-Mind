...
#!/usr/bin/env python3
"""
Unified Autonomous Swarm Port Guardian with:
- Universal auto-port patching (wrappers)
- Heuristic + True-ML-style anomaly detection (model file if present)
...
ML_MODEL_FILE = "ml_anomaly_model.pkl"  # pre-trained IsolationForest / OneClassSVM, etc.
LEARN_MODE = True
SCAN_INTERVAL = 1.0
...
SWARM_KEY = "KILLER666_SWARM_KEY"          # symmetric key for swarm encryption
SWARM_COMMAND_KEY = "KILLER666_CMD_KEY"    # symmetric key for command signing
...
ml_model = None  # loaded if available
...
def sign_command(cmd: dict) -> str:
    """
    HMAC-SHA256 over JSON-serialized command (without 'sig').
    """
...
def verify_command(cmd: dict) -> bool:
    """
    Verify HMAC and that command type is allowed.
    """
...
def apply_swarm_command(cmd: dict):
    """
    Execute strictly limited, signed commands.
    """
    ctype = cmd.get("type")
    if ctype == "SET_MODE":
        value = cmd.get("value")
...
def extract_features_for_ml(conn, process_name: str) -> list[float]:
    """
    Simple feature vector for anomaly model.
    You can extend this offline and retrain.
    """
...
def ml_anomaly_flag(conn, process_name: str) -> bool:
    ...
    try:
        ...
        # For IsolationForest / OneClassSVM: decision_function or predict
        if hasattr(ml_model, "decision_function"):
            score = ml_model.decision_function([feats])[0]
            return score < 0  # negative = outlier
...
# =========================
# SWARM SYNC (HEARTBEAT + RULE MERGE + COMMANDS)
# =========================
        ...
        try:
            with lock:
                payload = {
                    ...
                    "allowed_ports": state["allowed_ports"],
                    "commands": []  # reserved for future outbound commands
                }
...
def swarm_listener_loop():
    ...
    while True:
        try:
            ...
            # handle signed commands if present
