"""
Unified Organism (Bernoulli + Distributed + Anomaly + HopGraph + GPU + ML + Consensus + Replay + Scripting + Autonomous Mode v2)
...
- AutonomousModeOrganV2 (state machine, dynamic profiles, risk budget, override ledger, outcome-based tuning)
- LifecycleOrgan
- build_organism()
"""
...
class ConfigOrgan:
    def __init__(self, config_path: Optional[str] = None):
        self.config: Dict[str, Any] = {
            ...
            # autonomous mode v2
            "AUTO_MODE_PROFILE": os.environ.get("ORG_AUTO_MODE_PROFILE", "adaptive"),
            "AUTO_MAX_ACTIONS_PER_MIN": int(os.environ.get("ORG_AUTO_MAX_ACTIONS_PER_MIN", "30")),
            "AUTO_REQUIRE_CONSENSUS_FOR_SEAL": os.environ.get("ORG_AUTO_REQUIRE_CONSENSUS_FOR_SEAL", "1") == "1",
            "AUTO_MAX_SEALS_PER_HOUR": int(os.environ.get("ORG_AUTO_MAX_SEALS_PER_HOUR", "10")),
            "AUTO_MAX_RESETS_PER_NODE_PER_HOUR": int(os.environ.get("ORG_AUTO_MAX_RESETS_PER_NODE_PER_HOUR", "20")),
            "AUTO_MAX_WORKER_ADJUST_PER_HOUR": int(os.environ.get("ORG_AUTO_MAX_WORKER_ADJUST_PER_HOUR", "30")),
            "AUTO_MAX_NODE_FRACTION_PER_CYCLE": float(os.environ.get("ORG_AUTO_MAX_NODE_FRACTION_PER_CYCLE", "0.4")),
        }
...
class PersistenceOrgan:
    ...
    def store_auto_override(self, ts: float, actor: str, action_kind: str,
                            target: str, reason: str, outcome: str, blocked_by: str):
        with self._lock, sqlite3.connect(self.db_path) as conn:
            c = conn.cursor()
            c.execute("""
                INSERT INTO auto_overrides (ts, actor, action_kind, target, reason, outcome, blocked_by)
                VALUES (?,?,?,?,?,?,?)
            """, (ts, actor, action_kind, target, reason, outcome, blocked_by))
            conn.commit()
    ...
    def apply_retention(self):
        ...
        with self._lock, sqlite3.connect(self.db_path) as conn:
            ...
            c.execute("DELETE FROM auto_overrides WHERE ts < ?", (cutoff,))
            conn.commit()
...
class AutoPersonality(Enum):
    AGGRESSIVE = "aggressive"
    CAUTIOUS = "cautious"
    ADAPTIVE = "adaptive"


class AutoNodeState(Enum):
    NORMAL = "NORMAL"
    ELEVATED = "ELEVATED"
    CRITICAL = "CRITICAL"
    RECOVERY = "RECOVERY"
...
class AutoProfile:
    name: str
    personality: AutoPersonality
    seal_threshold: int
    quarantine_threshold: int
    anomaly_sensitivity: float
    max_workers_factor: float
    min_workers_factor: float
    protection_axis: float
    performance_axis: float
    curiosity_axis: float


class AutonomousModeOrganV2:
    """
    Autonomous Mode v2:
    - Per-node state machine (NORMAL/ELEVATED/CRITICAL/RECOVERY)
    - Dynamic profiles (context-aware blending)
    - Risk budget + blast radius
    - Override ledger (who/what/why/blocked)
    - Outcome-based tuning (simple feedback)
    """
    ...
    def __init__(self,
        ...
        self.personality = self._load_personality(cfg.get("AUTO_MODE_PROFILE", "adaptive"))
        self.profiles = self._build_profiles()
        self.current_profile = self.profiles[self.personality]

        self._last_metrics: Dict[str, Any] = {}
        self._last_consensus: Dict[str, str] = {}
        self._recent_actions: Deque[float] = deque(maxlen=200)

        self.max_actions_per_min = cfg.get("AUTO_MAX_ACTIONS_PER_MIN", 30)
        self.require_consensus_for_seal = cfg.get("AUTO_REQUIRE_CONSENSUS_FOR_SEAL", True)
        self.max_seals_per_hour = cfg.get("AUTO_MAX_SEALS_PER_HOUR", 10)
        self.max_resets_per_node_per_hour = cfg.get("AUTO_MAX_RESETS_PER_NODE_PER_HOUR", 20)
        self.max_worker_adjust_per_hour = cfg.get("AUTO_MAX_WORKER_ADJUST_PER_HOUR", 30)
        self.max_node_fraction_per_cycle = cfg.get("AUTO_MAX_NODE_FRACTION_PER_CYCLE", 0.4)

        self._node_state_machine: Dict[str, AutoNodeState] = {}
        self._node_health_history: Dict[str, Deque[Tuple[float, int]]] = defaultdict(lambda: deque(maxlen=50))

        self._seals_history: Deque[float] = deque(maxlen=200)
        self._resets_history: Dict[str, Deque[float]] = defaultdict(lambda: deque(maxlen=200))
        self._worker_adjust_history: Deque[float] = deque(maxlen=200)

        self._override_ledger: Deque[Dict[str, Any]] = deque(maxlen=500)

        self._outcome_history: Deque[Dict[str, Any]] = deque(maxlen=200)

        self.bus.subscribe("flow_metrics", self._on_flow_metrics)
        self.bus.subscribe("consensus_view", self._on_consensus_view)
        self.bus.subscribe("anomaly_event", self._on_anomaly)
        self.bus.subscribe("cluster_anomaly_event", self._on_anomaly)

    def _load_personality(self, name: str) -> AutoPersonality:
        name = name.lower()
        if name == "aggressive":
            return AutoPersonality.AGGRESSIVE
        if name == "cautious":
            return AutoPersonality.CAUTIOUS
        return AutoPersonality.ADAPTIVE

    def _build_profiles(self) -> Dict[AutoPersonality, AutoProfile]:
        return {
            AutoPersonality.AGGRESSIVE: AutoProfile(
                name="aggressive",
                personality=AutoPersonality.AGGRESSIVE,
                seal_threshold=150,
                quarantine_threshold=80,
                anomaly_sensitivity=2.0,
                max_workers_factor=1.5,
                min_workers_factor=1.0,
                protection_axis=0.9,
                performance_axis=0.7,
                curiosity_axis=0.4,
            ),
            AutoPersonality.CAUTIOUS: AutoProfile(
                name="cautious",
                personality=AutoPersonality.CAUTIOUS,
                seal_threshold=250,
                quarantine_threshold=150,
                anomaly_sensitivity=3.5,
                max_workers_factor=1.2,
                min_workers_factor=0.8,
                protection_axis=0.6,
                performance_axis=0.5,
                curiosity_axis=0.2,
            ),
            AutoPersonality.ADAPTIVE: AutoProfile(
                name="adaptive",
                personality=AutoPersonality.ADAPTIVE,
                seal_threshold=200,
                quarantine_threshold=120,
                anomaly_sensitivity=3.0,
                max_workers_factor=1.3,
                min_workers_factor=0.9,
                protection_axis=0.75,
                performance_axis=0.75,
                curiosity_axis=0.6,
            ),
        }

    def _on_flow_metrics(self, event: FlowEvent):
        self._last_metrics = event.payload["metrics"]

    def _on_consensus_view(self, event: FlowEvent):
        self._last_consensus = event.payload["consensus"]

    def _on_anomaly(self, event: FlowEvent):
        anomaly = event.payload["anomaly"]
        self._record_action("anomaly_seen")
        self._decision_tree_on_anomaly(anomaly)

    def _record_action(self, reason: str):
        now = time.time()
        self._recent_actions.append(now)
        self.logger.log("INFO", "Autonomous action recorded", reason=reason)

    def _actions_in_last_minute(self) -> int:
        now = time.time()
        while self._recent_actions and self._recent_actions[0] < now - 60:
            self._recent_actions.popleft()
        return len(self._recent_actions)

    def _update_risk_budgets(self):
        now = time.time()
        while self._seals_history and self._seals_history[0] < now - 3600:
            self._seals_history.popleft()
        for node, dq in list(self._resets_history.items()):
            while dq and dq[0] < now - 3600:
                dq.popleft()
        while self._worker_adjust_history and self._worker_adjust_history[0] < now - 3600:
            self._worker_adjust_history.popleft()

    def _safety_rails_allow(self, action_kind: str, target: str) -> Tuple[bool, str]:
        self._update_risk_budgets()
        now = time.time()

        if self._actions_in_last_minute() >= self.max_actions_per_min:
            return False, "max_actions_per_min"

        if action_kind == "SEAL_NODE":
            if len(self._seals_history) >= self.max_seals_per_hour:
                return False, "max_seals_per_hour"
            if self.require_consensus_for_seal and self._last_consensus:
                state = self._last_consensus.get(target, "OPEN")
                if state != "SEALED":
                    return False, f"consensus_disagrees({state})"

        if action_kind == "RESET_NODE_HEALTH":
            dq = self._resets_history[target]
            if len(dq) >= self.max_resets_per_node_per_hour:
                return False, "max_resets_per_node_per_hour"

        if action_kind == "ADJUST_WORKERS":
            if len(self._worker_adjust_history) >= self.max_worker_adjust_per_hour:
                return False, "max_worker_adjust_per_hour"

        node_state, node_health, _ = self.policy.snapshot()
        sealed_nodes = [n for n, s in node_state.items() if s == NodeState.SEALED]
        if action_kind == "SEAL_NODE" and len(sealed_nodes) >= max(1, len(node_state) - 1):
            return False, "last_open_node"

        if action_kind in ("SEAL_NODE", "RESET_NODE_HEALTH"):
            total_nodes = max(1, len(node_state))
            max_nodes = max(1, int(total_nodes * self.max_node_fraction_per_cycle))
            # we approximate: if too many nodes already in CRITICAL/ELEVATED, limit
            elevated_or_critical = sum(
                1 for n in node_state.keys()
                if self._node_state_machine.get(n, AutoNodeState.NORMAL) in
                (AutoNodeState.ELEVATED, AutoNodeState.CRITICAL)
            )
            if elevated_or_critical >= max_nodes:
                return False, "blast_radius"

        return True, ""

    def _log_override(self, actor: str, action_kind: str, target: str,
                      reason: str, outcome: str, blocked_by: str):
        ts = time.time()
        entry = {
            "ts": ts,
            "actor": actor,
        ...
        self._override_ledger.append(entry)
        self.persistence.store_auto_override(ts, actor, action_kind, target, reason, outcome, blocked_by)

    def _apply_autonomous_policy_action(self, kind: str, target: str, reason: str):
        allowed, blocked_by = self._safety_rails_allow(kind, target)
        if not allowed:
            self.logger.log("WARN", "Autonomous safety rail blocked action",
                            kind=kind, target=target, blocked_by=blocked_by)
            self._log_override("auto", kind, target, reason, "blocked", blocked_by)
            return

        if kind in ("SEAL_NODE", "RESET_NODE_HEALTH", "UNSEAL_NODE"):
            self.bus.publish(FlowEvent("operator_command", {
                "cmd": kind,
                "target": target,
            }))
        elif kind == "ADJUST_WORKERS":
            pass

        now = time.time()
        if kind == "SEAL_NODE":
            self._seals_history.append(now)
        elif kind == "RESET_NODE_HEALTH":
            self._resets_history[target].append(now)
        elif kind == "ADJUST_WORKERS":
            self._worker_adjust_history.append(now)

        self._record_action(reason)
        self._log_override("auto", kind, target, reason, "applied", "")

    def _decision_tree_on_anomaly(self, anomaly: Dict[str, Any]):
        severity = anomaly.get("severity", 10)
        ring = anomaly.get("ring", "unknown")
        src = anomaly.get("source", "stat")

        node_state, node_health, _ = self.policy.snapshot()
        worst_node = None
        worst_health = -1
        for n, h in node_health.items():
            if h > worst_health:
                worst_health = h
                worst_node = n
        ...
        if severity >= 80:
            if worst_node:
                self._apply_autonomous_policy_action("SEAL_NODE", worst_node,
                                                     f"high severity anomaly ({src}) on ring {ring}")
        elif severity >= 40:
            if worst_node:
                self._apply_autonomous_policy_action("RESET_NODE_HEALTH", worst_node,
                                                     f"medium severity anomaly ({src}) on ring {ring}")

        if self._last_metrics:
            rings = self._last_metrics.get("rings", {})
            avg_pressure = 0.0
            if rings:
                avg_pressure = sum(m["pressure"] for m in rings.values()) / len(rings)
            current_workers = self.workers.worker_count
            max_workers = int(self.cfg.get("WORKERS_MAX", 16) * profile.max_workers_factor)
            min_workers = max(1, int(self.cfg.get("WORKERS_MIN", 1) * profile.min_workers_factor))

            if avg_pressure > 0.95 and current_workers < max_workers:
                self.workers.adjust_workers(current_workers + 1)
                self._apply_autonomous_policy_action("ADJUST_WORKERS", "global",
                                                     "increase_workers_on_anomaly")
            elif avg_pressure < 0.2 and current_workers > min_workers:
                self.workers.adjust_workers(current_workers - 1)
                self._apply_autonomous_policy_action("ADJUST_WORKERS", "global",
                                                     "decrease_workers_on_low_pressure")

    def _update_node_state_machine(self):
        node_state, node_health, _ = self.policy.snapshot()
        now = time.time()
        for node, health in node_health.items():
            hist = self._node_health_history[node]
            hist.append((now, health))
            if node not in self._node_state_machine:
                self._node_state_machine[node] = AutoNodeState.NORMAL

            state = self._node_state_machine[node]
            profile = self.current_profile

            slope = 0.0
            if len(hist) >= 2:
                (t0, h0), (t1, h1) = hist[0], hist[-1]
                dt = max(1.0, t1 - t0)
                slope = (h1 - h0) / dt

            if state == AutoNodeState.NORMAL:
                if health >= profile.quarantine_threshold or slope > 1.0:
                    self._node_state_machine[node] = AutoNodeState.ELEVATED
            elif state == AutoNodeState.ELEVATED:
                if health >= profile.seal_threshold or slope > 2.0:
                    self._node_state_machine[node] = AutoNodeState.CRITICAL
                elif health < profile.quarantine_threshold * 0.5 and slope <= 0:
                    self._node_state_machine[node] = AutoNodeState.RECOVERY
            elif state == AutoNodeState.CRITICAL:
                if health < profile.quarantine_threshold * 0.5 and slope <= 0:
                    self._node_state_machine[node] = AutoNodeState.RECOVERY
            elif state == AutoNodeState.RECOVERY:
                if health < profile.quarantine_threshold * 0.3 and slope <= 0:
                    self._node_state_machine[node] = AutoNodeState.NORMAL
                elif health >= profile.quarantine_threshold:
                    self._node_state_machine[node] = AutoNodeState.ELEVATED

    def _apply_state_machine_actions(self):
        node_state, node_health, _ = self.policy.snapshot()
        profile = self.current_profile
        for node, health in node_health.items():
            state = self._node_state_machine.get(node, AutoNodeState.NORMAL)
            if state == AutoNodeState.NORMAL:
                continue
            elif state == AutoNodeState.ELEVATED:
                if health >= profile.quarantine_threshold:
                    self._apply_autonomous_policy_action("RESET_NODE_HEALTH", node,
                                                         "elevated_state_reset")
            elif state == AutoNodeState.CRITICAL:
                if health >= profile.seal_threshold:
                    self._apply_autonomous_policy_action("SEAL_NODE", node,
                                                         "critical_state_seal")
            elif state == AutoNodeState.RECOVERY:
                pass

    def _dynamic_profile_blend(self):
        ...
        avg_health = 0.0
        if node_health:
            avg_health = sum(node_health.values()) / max(1, len(node_health))

        load_factor = min(1.0, avg_pressure)
        health_factor = min(1.0, avg_health / 200.0)

        base = self.profiles[AutoPersonality.ADAPTIVE]
        agg = self.profiles[AutoPersonality.AGGRESSIVE]
        cau = self.profiles[AutoPersonality.CAUTIOUS]

        w_agg = 0.5 * load_factor + 0.3 * health_factor
        w_cau = 0.5 * (1.0 - load_factor) + 0.3 * (1.0 - health_factor)
        w_adp = 1.0 - (w_agg + w_cau)
        if w_adp < 0:
            w_adp = 0.0
        total = w_agg + w_cau + w_adp
        if total <= 0:
            w_agg = w_cau = 0.0
            w_adp = 1.0
            total = 1.0
        ...
        def blend(attr):
            return (w_agg * getattr(agg, attr) +
                    w_cau * getattr(cau, attr) +
                    w_adp * getattr(base, attr))

        blended = AutoProfile(
            name="dynamic",
            personality=AutoPersonality.ADAPTIVE,
            seal_threshold=int(blend("seal_threshold")),
            quarantine_threshold=int(blend("quarantine_threshold")),
            anomaly_sensitivity=blend("anomaly_sensitivity"),
            max_workers_factor=blend("max_workers_factor"),
            min_workers_factor=blend("min_workers_factor"),
            protection_axis=blend("protection_axis"),
            performance_axis=blend("performance_axis"),
            curiosity_axis=blend("curiosity_axis"),
        )
    ...
    def _record_outcome(self):
        ...
        entry = {
            "ts": time.time(),
            "avg_pressure": sum(m["pressure"] for m in metrics.get("rings", {}).values()) /
                            max(1, len(metrics.get("rings", {}))),
            "avg_health": (sum(node_health.values()) / max(1, len(node_health))) if node_health else 0.0,
            "sealed_nodes": sum(1 for s in node_state.values() if s == NodeState.SEALED),
        }
        self._outcome_history.append(entry)

    def _outcome_based_tuning(self):
        if len(self._outcome_history) < 5:
            return
        recent = list(self._outcome_history)[-5:]
        avg_pressure = sum(e["avg_pressure"] for e in recent) / 5.0
        avg_health = sum(e["avg_health"] for e in recent) / 5.0

        profile = self.current_profile
        if avg_pressure < 0.3 and avg_health < 50:
            profile.seal_threshold = int(profile.seal_threshold * 1.05)
            profile.quarantine_threshold = int(profile.quarantine_threshold * 1.05)
        elif avg_pressure > 0.8 or avg_health > 150:
            profile.seal_threshold = int(profile.seal_threshold * 0.95)
            profile.quarantine_threshold = int(profile.quarantine_threshold * 0.95)
    ...
    def start(self):
        self.thread = Thread(target=self._loop, daemon=True)
        self.thread.start()
        self.logger.log("INFO", "AutonomousModeOrganV2 started",
    ...
    def _loop(self):
        while not self._stop:
            time.sleep(5.0)
            self._dynamic_profile_blend()
            self._update_node_state_machine()
            self._apply_state_machine_actions()
            self._record_outcome()
            self._outcome_based_tuning()
...
class LifecycleOrgan:
    def __init__(self, cockpit: TkinterCockpit, workers: WorkerPool,
                 ...
                 auto_mode: AutonomousModeOrganV2):
...
def build_organism(config_path: Optional[str] = None):
    ...
    auto_mode = AutonomousModeOrganV2(cfg, policy, flow_metrics, bus, logger, workers, rings, persistence)

    lifecycle = LifecycleOrgan(cockpit, workers, persistence, policy, bus, logger,
                               tuning, cluster, consensus, gpu_cockpit, auto_mode)
