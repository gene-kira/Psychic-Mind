"""
UNIFIED VIDEO AI PIPELINE + OVERLAY HUD (CLICKABLE, CUDA-OPTIONAL)
...
- Overlay HUD drawn on video with clickable controls:
  - Toggle YOLO
  - Toggle RL
...
MEMORY_SMB_PATH = r"\\MY-NAS\AI-Memory\reboot_memory.json"  # adjust if needed
...
USE_YOLO = True          # base capability
USE_RL = True            # base capability
USE_KEYBOARD_LOGGER = True

FPS_TARGET = 15.0
RL_MODE = "normal"       # "normal" or "altered"

# Runtime toggles (HUD controls)
ENABLE_YOLO_RUNTIME = True
ENABLE_RL_RUNTIME = True
ALTERED_MANUAL_OVERRIDE = False  # if True, RL_MODE is forced by HUD
...
class ConfigHUD:
    ...
    def draw(self, frame, yolo_on: bool, rl_on: bool, altered: bool, fps: float, ghost_count: int):
        if self.last_frame_size != (frame.shape[1], frame.shape[0]) or not self.buttons:
            self.layout(frame)
        ...
        info_lines = [
            ...
            "Click buttons in bottom-right to control."
        ]
        x0, y0 = 10, 20
