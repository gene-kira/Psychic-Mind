...
def compute_hormuz_stress(prices):
    """
    Simple stress index:
    - Higher Brent and VIX => higher stress
    - Roughly normalized into 0–100
    """
    brent = prices.get("Brent")
    vix = prices.get("VIX")

    if brent is None or vix is None:
        return 0.0

    brent_component = max(0.0, min((brent - 60) / 2.0, 60))  # 60–120 -> 0–30
    vix_component = max(0.0, min((vix - 15) * 2.0, 40))      # 15–35 -> 0–40

    stress = brent_component + vix_component
    return max(0.0, min(stress, 100.0))


# ---------------- WAR NEWS (IRAN / REGION) ---------------- #
...
def fetch_war_news(max_items=50):
    """
...
class HormuzCockpit(tk.Tk):
    ...
    def refresh_news(self):
        ...
        headlines = fetch_war_news(max_items=80)
    ...
    def update_ticker(self):
        ...
        # Show one headline at a time, cycling
