# =========================
# MIGHTY MOUSE – FULL UPDATED CODE (WITH PREDICTIVE MODULES)
# =========================
...
# =========================
# GAME COGNITION – ADD MISSING METHOD
# =========================
...
class PredictiveEngine:
    def __init__(self, alpha=0.4, max_len=180):
        self.alpha = alpha
        self.ewma = None
        self.history = []
    ...
    def update(self, value):
        ...
        if self.ewma is None:
            ...
            self.ewma = self.alpha * v + (1.0 - self.alpha) * self.ewma
        now = time.time()
        self.history.append((now, v))
        if len(self.history) > self.max_len:
            self.history.pop(0)

    def _linear_forecast(self, horizon_sec):
        if self.ewma is None:
            return 0.0
        if len(self.history) < 3:
            return self.ewma
        t0 = self.history[0][0]
        xs = [h[0] - t0 for h in self.history]
        ...
        a = (sum_y - b * sum_x) / n
        t_future = xs[-1] + horizon_sec
        y_future = a + b * t_future
        return max(0.0, min(100.0, y_future))

    def forecast_multi(self):
        return {
            1: self._linear_forecast(1.0),
            3: self._linear_forecast(3.0),
            5: self._linear_forecast(5.0),
            10: self._linear_forecast(10.0),
        }

    def get_volatility(self):
        if len(self.history) < 5:
            return 0.0
        vals = [h[1] for h in self.history[-30:]]
        m = sum(vals) / len(vals)
...
class WhatIfSimulator:
    def simulate_danger_push(self, base_danger, extra_enemies):
        if base_danger is None:
            base_danger = 0
        return max(0, min(100, int(base_danger + extra_enemies * 10)))

    def simulate_danger_retreat(self, base_danger, reduction_factor=0.5):
        if base_danger is None:
            base_danger = 0
        return max(0, int(base_danger * reduction_factor))

    def simulate_rotation_effect(self, game_cog, delta_yaw):
        if not game_cog.minimap:
            return 0
        player = dict(game_cog.minimap.get("player", {"x": 0, "y": 0, "yaw": 0}))
        player["yaw"] = (player.get("yaw", 0) + delta_yaw) % 360
        enemies = game_cog.minimap.get("enemies", [])
        cones = {"FRONT": 0, "RIGHT": 0, "BACK": 0, "LEFT": 0}
        for e in enemies:
            dx = e.get("x", 0) - player["x"]
            dy = e.get("y", 0) - player["y"]
...
class PredictiveManager:
    def __init__(self, memory, hud, game, cursor, web, bus):
        self.memory = memory
        self.hud = hud
        self.game = game
        self.cursor = cursor
        ...
        self.last_whatif_ts = 0
        self.last_regime = None
        bus.subscribe("tick", self.on_tick)

    def on_tick(self, _):
        if not ENABLE_PREDICTIVE:
            return
        danger = self.game.get_danger_score()
        self.engine.update(danger)
        multi = self.engine.forecast_multi()
        future5 = int(multi[5])
        self.memory.set_future_danger(future5)
        self.memory.data["game"]["future_danger_multi"] = {k: int(v) for k, v in multi.items()}
        regime = self.engine.get_regime()
        conf = self.engine.get_confidence()
        self.memory.data["game"]["predictive_regime"] = regime
        self.memory.data["game"]["predictive_confidence"] = conf
        if regime != self.last_regime:
            self.last_regime = regime
            self.hud.push("predict", f"Regime: {regime} (conf {int(conf*100)}%)")
        now = time.time()
        if ENABLE_WHATIF and now - self.last_whatif_ts > 5.0:
            self.last_whatif_ts = now
            push_danger = self.whatif.simulate_danger_push(danger, 2)
            retreat_danger = self.whatif.simulate_danger_retreat(danger)
            self.memory.data.setdefault("whatif", {})
            self.memory.data["whatif"]["push"] = push_danger
            self.memory.data["whatif"]["retreat"] = retreat_danger
            self.hud.push("predict", f"What-if push: {push_danger}, retreat: {retreat_danger}")
            if self.game.minimap:
                cones_rot = self.whatif.simulate_rotation_effect(self.game, 45)
                self.memory.data["whatif"]["cones_rot_45"] = cones_rot

def attach_predictive(bus, orch, game, memory, hud, cursor, web):
    if not ENABLE_PREDICTIVE:
        orch.log("[PREDICTIVE] Disabled")
        return None
    pm = PredictiveManager(memory, hud, game, cursor, web, bus)
    orch.log("[PREDICTIVE] Predictive engine attached")
    return pm
...
def main():
    ...
    # PREDICTIVE ATTACH
    attach_predictive(bus, orch, game, memory, hud, cursor, web)
    ...
    orch.log("[SYSTEM] Mighty Mouse fully initialized")
    sys.exit(app.exec_())
