#!/usr/bin/env python3
# borg_glyph_vault_unified_v9.py
#
# Concept demo ONLY:
# - Glyph codec
# - RSA + AES-GCM crypto
...
#       * PySide6 tactical cockpit (initial real layout, if available)
# - Safety:
...
class PolicyEngine:
    """
    Tightened rules:
    - High anomaly => ESCALATE
    - Very high anomaly => LOCK_ORG
    - Multiple unlock failures => LOCK_ORG
    - Frequent confirmation mismatches => ESCALATE
    """
    ...
    def _build_default_rules(self):
        self.rules.append({
            ...
            "condition": lambda ctx: ctx.get("anomaly_score", 0) > 0.7,
            "action": "ESCALATE",
            "message": "High anomaly score detected. Recommend lock-down.",
        })
        self.rules.append({
            "name": "critical_anomaly_lock",
            "condition": lambda ctx: ctx.get("anomaly_score", 0) > 0.9,
            "action": "LOCK_ORG",
...
# -----------------------------
# PySide6 tactical cockpit (initial real layout)
# -----------------------------
...
class PySideCockpit(QtWidgets.QMainWindow):
    def __init__(self, app_ref):
        ...
        self.setWindowTitle("Borg Glyph Vault – PySide6 Tactical Cockpit (Concept)")
        self.resize(900, 600)
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Borg Glyph Vault – Tk Tactical Cockpit (Concept Demo v9)")
