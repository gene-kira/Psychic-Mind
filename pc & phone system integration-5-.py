...
Cybernetic phone swarm organism with upgraded organs:
...
2) Bluetooth GATT (full BLE control)
   - Service discovery
   - Notifications subscribe/unsubscribe
   - Multi-device sessions per phone

3) Mesh networking (real distributed network)
   - UDP discovery
   - TCP peer links
   - Peer routing table
   - Distributed tasks (forward to peers)
   - Gossip protocol for state sharing

4) Voice recognition (async)
   - Streaming microphone (agent-side)
   - Wake-word detection (stub hook)
   - Real-time transcription (speech_recognition / Whisper hook)

5) AI pipelines
   - Preprocessing (image/audio/text)
   - Postprocessing (detections, labels)
   - Model registry: YOLO, MobileNet, Whisper (paths configurable)
   - ONNX Runtime Mobile on agent

6) GUI cockpit (redesigned)
   - Node list + status
   - Real-ish map panel (lat/lon projection)
...
On Android (Termux agent):
    ...
    # Optional for better AI / audio:
    # pip install opencv-python numpy

    python phone_swarm_organism.py --mode agent

NOTE:
- Some advanced features (Whisper, YOLO, real map tiles) are wired as hooks/stubs
  with clear TODOs where you can plug in real models or APIs.
"""
...
def decode_h264_ts_chunk(chunk_b64: str) -> Optional["Image.Image"]:
    """
    Decode a base64-encoded MPEG-TS H.264 chunk into a PIL Image.
    Requires OpenCV + numpy + Pillow.
    This is a best-effort low-latency decoder; not a full player.
    """
    if cv2 is None or np is None or Image is None:
        return None
    try:
        raw = base64.b64decode(chunk_b64)
        # Heuristic: treat chunk as raw H.264 stream and let OpenCV try to decode
        # We create a temporary in-memory buffer and use VideoCapture on it.
        # NOTE: This is not perfect; for production you'd want a persistent decoder.
...
# ---------------------------------------------------------------------------
# AGENT SIDE: AI pipelines (pre/post + model registry)
# ---------------------------------------------------------------------------
...
def ai_preprocess(input_type: str, input_data: Any, params: Dict[str, Any]) -> Any:
    """
    Basic preprocessing:
    - image: base64 JPEG -> resized float32 tensor
    - audio: raw PCM or base64 -> float32
    - text: tokenization stub
    """
    if np is None:
        return input_data
    ...
    if input_type == "audio_pcm":
        # TODO: implement real audio preprocessing
        return input_data

    if input_type == "text":
        # TODO: implement tokenization for Whisper or text models
        return input_data
...
def ai_postprocess(model: str, raw_outputs: Any, params: Dict[str, Any]) -> Any:
    ...
    - YOLO: stub bounding boxes
    - MobileNet: top-1 class stub
    - Whisper: text stub
    """
    if isinstance(raw_outputs, dict) and raw_outputs.get("stub"):
        return raw_outputs

    # TODO: implement real postprocessing per model
    return {"raw": raw_outputs, "model": model}
...
async def agent_voice_recognize_streaming() -> Dict[str, Any]:
    """
    Streaming-ish voice recognition:
    - Uses speech_recognition as backend (blocking internally)
    - Wraps it in an async task
    - Wake-word detection is stubbed: checks if wake_word in text
    """
...
async def agent_bt_notify(payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    Subscribe/unsubscribe to notifications on a characteristic.
    For simplicity, this is a stub that returns success; implementing
    persistent notification streaming would require a long-lived BLE client.
    """
...
async def agent_h264_stream(ws, stream_id: str, resolution: List[int], fps: int, low_latency: bool) -> None:
    ...
    - low_latency flag can be used to tweak ffmpeg params.
    """
...
# ---------------------------------------------------------------------------
# AGENT SIDE: Mesh networking (UDP discovery + TCP peers + routing + gossip)
# ---------------------------------------------------------------------------
...
async def mesh_gossip_loop() -> None:
    """
    Simple gossip: periodically send our state to peers.
    """
    while True:
    ...
    async def handle(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        try:
            ...
            if msg.get("type") == "gossip":
                ...
                # Distributed task: echo stub
                # TODO: implement real distributed task execution
                pass
        except Exception:
            pass
        finally:
...
async def mesh_task_forward(target_peer_id: str, inner_command: str, inner_payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    Forward a task to another peer via TCP.
    This is a stub: it just sends the task and does not wait for response.
    """
    ...
    try:
        ...
        if command == "ai_infer":
            ...
            result = {
                "model": model,
                "input_type": input_type,
                "preprocessed": isinstance(pre, (list, dict)),
                "output": post,
            }
            return {"ok": True, "result": result, "error": None, "task_id": task_id}
...
# ---------------------------------------------------------------------------
# CONTROLLER SIDE: GUI (maps + multi-camera grid + health charts)
# ---------------------------------------------------------------------------
...
class SwarmGUI:
    def __init__(self, swarm: SwarmManager) -> None:
        ...
        ttk.Label(right, text="Camera Grid (JPEG + H.264)", font=("Arial", 12, "bold")).pack(anchor="w")
    ...
    def update_gui(self) -> None:
        ...
        if nid and nid in self.swarm.nodes:
            ...
            if ai:
                self.ai_label.config(text=f"AI: model={ai.get('model')} ok")
            else:
