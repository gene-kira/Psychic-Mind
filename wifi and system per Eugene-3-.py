#!/usr/bin/env python3
"""
Universal Guardian Cockpit v5 (Operator-Grade + Enforcement)
...
- Self-tuning agent weights (in-memory)
...
# ---------- Self-Rewriting Agent (in-memory only) ----------

# [wifi_weight, conn_weight, proc_weight]
    ...
    def evaluate(self, state: "GuardianState") -> None:
        ...
        for p in procs:
            ...
            if score >= 5:
                ...
                self.enforce_on_process(p, conns, score)

    def enforce_on_process(self, proc: Dict[str, Any], conns: List[Dict[str, Any]], score: int) -> None:
        pid = proc.get("pid")
        exe = proc.get("exe") or ""
...
class GuardianGUI:
    def __init__(self, root: tk.Tk, state: GuardianState):
        ...
        root.title("Universal Guardian Cockpit v5")
...
def main() -> None:
    log_event("INFO", "Universal Guardian Cockpit v5 starting", {})
    state = GuardianState()
    ...
    wifi_mon.stop()
    time.sleep(1)
    log_event("INFO", "Universal Guardian Cockpit v5 exited", {})
