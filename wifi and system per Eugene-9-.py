#!/usr/bin/env python3
"""
Universal Guardian Cockpit v12 – Swarm RL, Temporal Patterns, Adaptive Stance

Features:
- Persistent brain (agent weights, policy params, reward log, long-term stats, enforcement count, mutation log)
- Config system (thresholds, policies, swarm, trust, role, swarm memory)
...
- v12 additions:
  - Swarm-wide reinforcement learning (reward log, policy params)
  - Temporal pattern tracking (event sequences)
  - Adaptive policy tuning (thresholds, weights)
  - Per-node stance profiles (hardened/balanced/observational/high-value)
"""
...
_default_brain = {
    ...
    "created_at": None,
    "updated_at": None,
    # v12: swarm RL + policy params + reward log + stance
    "policy_params": {
        "network_high_conn_base": 150.0,
        "network_spike_hard_limit": 300.0,
        "score_enforce_strict": 5.0,
        "score_sandbox_candidate": 8.0,
        "stance_bias_strict": 0.0,
        "stance_bias_relaxed": 0.0,
    },
    "reward_log": [],
    "max_reward_log": 500,
}
...
def load_brain() -> Dict[str, Any]:
    ...
    if "policy_params" not in state:
        state["policy_params"] = dict(_default_brain["policy_params"])
    if "reward_log" not in state:
        state["reward_log"] = []
    if "max_reward_log" not in state:
        state["max_reward_log"] = 500
...
def get_policy_params() -> Dict[str, float]:
    state = get_brain_state()
    params = state.get("policy_params")
    if not isinstance(params, dict):
        params = dict(_default_brain["policy_params"])
        state["policy_params"] = params
        save_brain()
    return dict(params)


def set_policy_params(params: Dict[str, float]) -> None:
    state = get_brain_state()
    state["policy_params"] = dict(params)
    save_brain()


def add_reward_entry(node_id: str, value: float, reason: str) -> None:
    state = get_brain_state()
    log = state.get("reward_log")
    if not isinstance(log, list):
        log = []
    entry = {
        "ts": _brain_now_iso(),
    ...
    log.append(entry)
    max_len = int(state.get("max_reward_log", 500))
    if len(log) > max_len:
        log = log[-max_len:]
    state["reward_log"] = log
    save_brain()


def get_recent_rewards(window: int = 100) -> List[Dict[str, Any]]:
    state = get_brain_state()
    log = state.get("reward_log")
    if not isinstance(log, list):
        return []
...
def ai_select_mode(
    ...
    base_high_conn = float(policy_params.get("network_high_conn_base", 150.0))
    high_conn_min = get_threshold("network.high_conn_min", 50.0)
    high_conn_threshold = int(base_high_conn + conn_weight * -50)
    ...
    stance_bias_strict = float(policy_params.get("stance_bias_strict", 0.0))
    stance_bias_relaxed = float(policy_params.get("stance_bias_relaxed", 0.0))

    if stance == "hardened" or stance == "high_value":
        stance_bias_strict += 0.2
    elif stance == "observational":
        stance_bias_relaxed += 0.2

    def biased_choice(strict_reason: str, relaxed_reason: str):
        if stance_bias_strict > stance_bias_relaxed:
            return ("STRICT", strict_reason + predictive_reason, False)
        else:
            return ("RELAXED", relaxed_reason + predictive_reason, False)
    ...
    if level == "OK":
        if high_conn or predictive_strict:
            return biased_choice(
                f"Trusted Wi-Fi but high connection count: {connection_count}",
                f"Trusted Wi-Fi, but high connections (relaxed stance)"
            )
        return ("RELAXED", f"Trusted Wi-Fi, normal activity{predictive_reason}", False)

    if high_conn or predictive_strict:
        return biased_choice(
            f"No Wi-Fi trust + high connections",
            f"No Wi-Fi trust but relaxed stance"
        )
...
class RuleEngine:
    ...
    def add_alert(self, kind: str, msg: str, data: Dict[str, Any]) -> None:
        ...
        self.state_ref.record_temporal_event(kind, data)
    ...
    def evaluate(self, state: "GuardianState") -> None:
        ...
        policy_params = get_policy_params()
        hard_limit = int(policy_params.get("network_spike_hard_limit", get_threshold("network.network_spike_hard_limit", 300.0)))
        ...
        score_enforce = int(policy_params.get("score_enforce_strict", get_threshold("threat.score_enforce_strict", 5.0)))
        score_sandbox = int(policy_params.get("score_sandbox_candidate", get_threshold("threat.score_sandbox_candidate", 8.0)))
    ...
    def enforce_on_process(
        ...
        add_reward_entry(self.state_ref.node_id, +1.0, "enforcement_event")
...
class RLThread(threading.Thread):
    def __init__(self, state: "GuardianState"):
        super().__init__(daemon=True)
        self.state = state
        self._stop_flag = threading.Event()
    ...
    def run(self) -> None:
        ...
        interval = float(rl_cfg.get("update_interval_seconds", 60))
        log_event("INFO", "RL thread started", {"enabled": enabled, "interval": interval})
        while not self._stop_flag.is_set():
            try:
                if enabled and self.state.is_coordinator:
                    self.state.rl_update()
            except Exception as e:
                log_event("ERROR", "RL loop error", {"error": str(e)})
            time.sleep(interval)
...
class GuardianState:
    def __init__(self):
        ...
        self.max_temporal_events = 500

        self.stance_profiles: Dict[str, str] = {}
        self.default_stance = get_config().get("stance", {}).get("default", "balanced")
    ...
    def handle_swarm_alert(self, data: Dict[str, Any]) -> None:
        ...
        self.record_temporal_event(alert.get("kind"), alert.get("data", {}))
    ...
    def update_processes(self) -> None:
        ...
        for p in added:
            ...
            self.record_temporal_event("PROCESS_START", p)
        for p in removed:
            log_event("INFO", "Process ended", p)
            self.record_temporal_event("PROCESS_END", p)
    ...
    def update_drives(self) -> None:
        ...
        for d in added:
            ...
            self.record_temporal_event("DRIVE_MOUNT", d)
        for d in removed:
            log_event("WARN", "Drive unmounted", d)
            self.record_temporal_event("DRIVE_UNMOUNT", d)
    ...
    def update_network(self) -> None:
        ...
        if anomaly["anomaly"]:
            ...
            self.record_temporal_event("NETWORK_ANOMALY", anomaly)

    def get_node_stance(self) -> str:
        cfg = get_config()
        stance_cfg = cfg.get("stance", {})
        overrides = stance_cfg.get("node_overrides", {})
        with self.lock:
            if self.node_id in overrides:
                return overrides[self.node_id]
            if self.node_id in self.stance_profiles:
                return self.stance_profiles[self.node_id]
        return stance_cfg.get("default", "balanced")
    ...
    def update_wifi_and_ai(self) -> None:
        ...
        stance = self.get_node_stance()
        mode, reason, learn_now = ai_select_mode(wifi_eval, connection_count, predictive_hint, stance)
    ...
    def record_temporal_event(self, kind: Optional[str], meta: Dict[str, Any]) -> None:
        if not kind:
            return
        ev = {
            "ts": datetime.utcnow().isoformat() + "Z",
        ...
        with self.lock:
            self.temporal_events.append(ev)
            if len(self.temporal_events) > self.max_temporal_events:
                self.temporal_events = self.temporal_events[-self.max_temporal_events:]

    def rl_update(self) -> None:
        cfg = get_config()
        rl_cfg = cfg.get("rl", {})
        reward_window = int(rl_cfg.get("reward_window", 100))
        strict_step = float(rl_cfg.get("strict_bias_step", 0.05))
        threshold_step = float(rl_cfg.get("threshold_step", 5.0))

        rewards = get_recent_rewards(reward_window)
        if not rewards:
            return
        ...
        if avg_reward > 0.1:
            policy_params["stance_bias_relaxed"] = min(1.0, policy_params.get("stance_bias_relaxed", 0.0) + strict_step)
            policy_params["stance_bias_strict"] = max(0.0, policy_params.get("stance_bias_strict", 0.0) - strict_step)
            policy_params["network_high_conn_base"] = max(
                50.0, policy_params.get("network_high_conn_base", 150.0) + threshold_step
            )
            log_event("INFO", "RL update: positive reward, relaxing slightly", {
                "avg_reward": avg_reward,
                "policy_params": policy_params
            })
        elif avg_reward < -0.1:
            policy_params["stance_bias_strict"] = min(1.0, policy_params.get("stance_bias_strict", 0.0) + strict_step)
            policy_params["stance_bias_relaxed"] = max(0.0, policy_params.get("stance_bias_relaxed", 0.0) - strict_step)
            policy_params["network_high_conn_base"] = max(
                50.0, policy_params.get("network_high_conn_base", 150.0) - threshold_step
            )
            log_event("WARN", "RL update: negative reward, tightening", {
                "avg_reward": avg_reward,
                "policy_params": policy_params
            })
        else:
            log_event("INFO", "RL update: neutral reward, no change", {"avg_reward": avg_reward})
            return

        set_policy_params(policy_params)
...
class GuardianGUI:
    def __init__(self, root: tk.Tk, state: GuardianState):
        ...
        root.title("Universal Guardian Cockpit v12 (Swarm RL & Temporal Patterns)")
...
def main() -> None:
    ...
    log_event("INFO", "Universal Guardian Cockpit v12 starting", {
    ...
    rl_thread.stop()
    time.sleep(1)
    log_event("INFO", "Universal Guardian Cockpit v12 exited", {})
