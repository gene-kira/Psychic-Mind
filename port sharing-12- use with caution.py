...
(Optimized + Plugins + Heatmap + Forensics + Cross‑Platform)

Major capabilities:
- Auto-elevation on Windows (best-effort), normal user elsewhere
- Cross‑platform core (Linux/macOS/Windows) – Windows-only parts are guarded
...
state = {
    ...
    "behavior_history": {},
    "heatmap": {},  # (process,port)->aggregated threat score
}
...
def forensic_dump(event):
    ...
    dump = {"event": event, "process_info": {}, "connections": []}
    try:
        p = psutil.Process(pid)
        dump["process_info"] = {
            "pid": p.pid,
...
class PortEnforcerGUI:
    def __init__(self, root):
        ...
        self.hud.geometry("260x120+20+20")
