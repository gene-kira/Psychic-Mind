...
class AdaptiveCodexMutation:
    ...
    def export_state(self) -> Dict[str, Any]:
        return {
            ...
            "last_ghost_detected": self.last_ghost_detected,
        }
    ...
    def load_from_state(self, state: Dict[str, Any]):
        ...
        ml = state.get("mutation_log")
        if isinstance(ml, list):
            self.mutation_log = ml[-200:]
        lg = state.get("last_ghost_detected")
        if isinstance(lg, (int, float)):
            self.last_ghost_detected = float(lg)
...
class SelfRewritingAgent:
    ...
    def mutate_weights(self, risk: float, health: float, ghost: bool):
        delta = self.learning_rate * (risk - (1.0 - health))
        self.weights["risk"] = max(0.1, min(3.0, self.weights["risk"] + delta))
        self.weights["health"] = max(0.1, min(3.0, self.weights["health"] - delta * 0.5))
        if ghost:
            self.weights["ghost"] = min(5.0, self.weights["ghost"] + self.learning_rate)
        self.mutation_log.append({
            "ts": time.time(),
            "risk": risk,
            "health": health,
    ...
    def export_state(self) -> Dict[str, Any]:
        return {
            ...
            "mutation_log": self.mutation_log[-200:],
...
class AutonomousCipherEngine:
    def __init__(self):
        self.posture = "RELAXED"  # RELAXED, GUARDED, LOCKDOWN
    ...
    def snapshot(self) -> Dict[str, Any]:
        return {
            ...
            "seconds_since_rotation": time.time() - self.last_rotation,
        }
...
class BrainCore:
    ...
    def tick(self, putty: SillyPuttyDataGatherer, organs: List[BaseOrgan], swarm_view: Dict[str, float]):
        ...
        organ_vector = {o.name: o.risk for o in organs} if organs else {}
        max_drive_risk = max((o.risk for o in organs), default=0.0)
        avg_health = sum((o.health for o in organs), 0.0) / len(organs) if organs else 1.0

        swarm_risk = swarm_view.get("swarm_risk", 0.0)

        base_risk = max(anomaly_density, max_drive_risk, swarm_risk)
        ...
        baseline_dev = abs(pressure - flow_rate)
        ...
        self.opportunity_score = max(0.0, min(1.0, self.opportunity_bias * (1.0 - best_guess)))

        self.anomaly_risk = anomaly_density
        self.drive_risk = max_drive_risk
        self.hive_risk = swarm_risk

        prev_health = self.collective_health
        self.collective_health = max(0.0, min(1.0, avg_health))
        self.health_trend = 0.8 * self.health_trend + 0.2 * (self.collective_health - prev_health)
        ...
        self.codex.mutate_rules("brain_tick")

        self.agent.mutate_weights(self.risk_score, self.collective_health, ghost)
        self.cipher.tick(self.risk_score)

        self.autonomy.auto_tune()
        self.autonomy.auto_calibrate_if_due(anomaly_density, pressure)

        self.agvra.tick(self.risk_score)
        self.scion.tick(self.risk_score)
        self.asi_arch.tick(self.risk_score)

        self._update_meta_state_from_performance()

        if self.predictions["30s"] > self.autonomy.threshold_risk:
            self.anticipation = "High risk in 30s window"
        elif self.predictions["5s"] > self.autonomy.threshold_risk:
            self.anticipation = "Short spike incoming"
        else:
            self.anticipation = "Stable window"

        if self._tick_count % 20 == 0:
            self._log_internal(
                f"Regime={self.regime}, Risk={self.risk_score:.3f}, Health={self.collective_health:.3f}, Meta={self.meta_state}"
            )
    ...
    def codex_snapshot(self) -> Dict[str, Any]:
        state = self.codex.export_state()
        return {
            "retention_seconds": state["retention_seconds"],
            "ghost_sensitive": state["ghost_sensitive"],
            "phantom_node_enabled": state["phantom_node_enabled"],
            "mutations": len(state.get("mutation_log", [])),
        }
    ...
    def reasoning_snapshot(self) -> Dict[str, float]:
        return dict(self.reasoning_heatmap)
    ...
    class BorgCockpit(QWidget):
        ...
        def _tick(self):
            ...
            self.label_cipher.setText(
                f"Cipher - Posture: {cipher['posture']} | KeyID: {cipher['current_key_id']} | RotInt: {cipher['rotation_interval']}s | SinceRot: {cipher['seconds_since_rotation']:.1f}s"
            )
