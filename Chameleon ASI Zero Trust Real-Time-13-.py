...
def compute_event_metrics(window_sec=300):
    with event_history_lock:
        ...
        # Turbulence: variance of inter-arrival times
        t_sorted = xp.sort(t_arr)
        diffs = t_sorted[:-1] - t_sorted[1:]
        turbulence = float((diffs ** 2).mean())
        # Trend: linear trend of severity vs time
        x = t_arr
        y = s_arr
        x_mean = x.mean()
        y_mean = y.mean()
...
# 🧮 Multi-Horizon Prediction (EWMA + trend)
def multi_horizon_prediction(metrics, anomaly_score):
    ...
    # EWMA of anomaly as base
    alpha = 0.3
    base = anomaly_score

    preds = {}
    for name, horizon in horizons.items():
        # Simple linear projection with trend and turbulence penalty
        projected = base + trend * horizon * 10.0 + turbulence * 50.0
        # Variance pushes risk up
...
# 🧠 Meta-Confidence Fusion / Reasoning Heatmap / Best-Guess
def fuse_predictions(preds, regime, anomaly_score, metrics):
    # Confidence based on regime and turbulence
    turb = metrics["turbulence"]
    variance = metrics["variance"]
    if regime == "stable":
        model_conf = 0.8
        ...
        model_conf = 0.4
    # Turbulence reduces confidence
    model_conf *= max(0.2, 1.0 - min(1.0, turb))

    # EWMA fallback confidence
    ewma_conf = 1.0 - model_conf

    # Blend horizons: micro gets more weight in chaos, macro in stability
    if regime == "chaotic":
    ...
    # EWMA baseline is just current anomaly
...
# 🧠 Meta-State Machine (Hyper-Flow, Deep-Dream, Sentinel, Recovery-Flow)
...
def self_integrity_organ():
    ...
    with brain_lock:
        ...
        # Simple drift check: if anomaly is NaN or extreme
...
def micro_recovery_loops():
    ...
    with brain_lock:
        ...
        # Thermal → reduce appetite + slow ingestion
        if cpu > 80:
            appetite["ingest"] *= 0.9
...
# 🧠 Auto-Tuning (based on long-term success)
def auto_tuning():
    with brain_lock:
        ...
        # If health high, allow more aggressive stance
        if health > 0.8:
...
# 🧠 Auto-Calibration (daily; shortened here for demo)
def auto_calibration():
    ...
    with brain_lock:
        stance = brain_state["stance_thresholds"]
        # Recompute stance thresholds around mean/std
    ...
    root.after(3600000, auto_calibration)  # 1h; conceptually 24h
...
log_text = tk.Text(root, height=10, width=110, bg="#2e2e3f", fg="#00ffcc", font=("Consolas", 10))
log_text.pack()
...
# 🧠 Hybrid Brain Loop: prediction, regimes, health, meta-state
def hybrid_brain_loop():
    ...
    # 6) Health score (hybrid)
    with brain_lock:
        integrity = brain_state.get("integrity_score", 1.0)
        # Health: high when risk low and integrity high
...
root.after(5000, check_destruction)
root.after(10000, threat_scan_and_respond)
root.after(12000, hybrid_brain_loop)
root.after(13000, swarm_heartbeat)
root.after(14000, periodic_brain_save)
root.after(15000, self_check)
root.after(16000, watchdog_loop)
root.after(17000, self_integrity_organ)
root.after(18000, micro_recovery_loops)
root.after(19000, auto_tuning)
root.after(20000, auto_calibration)
root.after(200, overlay_anim_loop)
