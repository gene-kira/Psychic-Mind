...
#  - Network transport: TCP/UDP capsule send/receive
# ============================================================
    ...
    def _start_tcp_server(self):
        def _tcp_server():
            ...
            try:
                ...
                TELEMETRY.record_error("net_tcp_server", "network", "BindError", str(e))
                srv.close()
                return
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    TELEMETRY.record_error("net_tcp_server", "network", "AcceptError", str(e))
                    continue
            srv.close()

        self.tcp_server_thread = threading.Thread(target=_tcp_server, daemon=True)
    ...
    def _handle_tcp_client(self, conn: socket.socket, addr):
        with conn:
            try:
                ...
                raw, env = backbone_decode(capsule)
                TELEMETRY.record_event("network_rx", {
                    "proto": "tcp",
                    "from": str(addr),
                    "size": len(capsule.get("payload", "")),
                    "decoded_size": len(raw),
                    "ring_id": env.get("ring_id"),
                })
            except Exception as e:
                TELEMETRY.record_error("net_tcp_server", "network", "TCPClientError", str(e))

    def send_capsule_tcp(self, capsule: dict, host: str | None = None, port: int | None = None):
        host = host or self.host
        port = port or self.tcp_port
        try:
            data = json.dumps(capsule, sort_keys=True).encode("utf-8")
            length = len(data)
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.settimeout(0.5)
            s.connect((host, port))
            s.sendall(length.to_bytes(4, "big") + data)
            s.close()
            TELEMETRY.record_event("network_tx", {
                "proto": "tcp",
                "to": f"{host}:{port}",
                "size": len(capsule.get("payload", "")),
            })
        except Exception as e:
            TELEMETRY.record_error("net_tcp_client", "network", "TCPSendError", str(e))
    ...
    def _start_udp_server(self):
        def _udp_server():
            ...
            try:
                ...
                TELEMETRY.record_error("net_udp_server", "network", "BindError", str(e))
                s.close()
                return
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    TELEMETRY.record_error("net_udp_server", "network", "UDPRecvError", str(e))
                    continue
            s.close()

        self.udp_server_thread = threading.Thread(target=_udp_server, daemon=True)
        self.udp_server_thread.start()

    def send_capsule_udp(self, capsule: dict, host: str | None = None, port: int | None = None):
        host = host or self.host
        ...
        try:
            ...
            TELEMETRY.record_error("net_udp_client", "network", "UDPSendError", str(e))

    def stop(self):
        self.stop_event.set()
        # Threads are daemon; they will exit when stop_event is set and process ends.
...
class FlowWorkerPool:
    ...
    def _process_job(self, job: dict):
        ...
        if kind == "system_metrics":
            ...
            # Send over network (UDP by default)
            if NETWORK_TRANSPORT is not None:
                NETWORK_TRANSPORT.send_capsule_udp(capsule)
...
def run_backbone():
    global NETWORK_TRANSPORT

    device_id = "trusted-device-123"
    session_id = "session-abc"

    # Initialize network transport (TCP + UDP)
    NETWORK_TRANSPORT = NetworkTransport(
        host=NET_HOST,
        tcp_port=NET_TCP_PORT,
        udp_port=NET_UDP_PORT,
        device_id=device_id,
        session_id=session_id,
    )
