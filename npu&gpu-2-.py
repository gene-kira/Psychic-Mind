...
if platform.system() == "Windows" and not is_admin():
    ...
    try:
        ...
        pass  # fall through to other variants
...
# Apply auto-elevation on Windows once at startup
...
class NPUOrgan:
    ...
    def infer(self, gpu_util, mem_util, mem_frac, temp, power, power_limit):
        ...
        self.confidence = 0.5 + 0.5 * (1.0 - abs(perf - self.target_util))
...
class NPURaidEnsemble:
    ...
    @property
    def confidence(self):
        return float(np.mean([n.confidence for n in self.npus]))
