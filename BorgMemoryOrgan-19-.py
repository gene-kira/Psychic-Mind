"""
BORG-OS MEMORY BACKBONE ORGAN (v11.4 – User Activity Governor + Tuned Context-Aware Priority + Recall Timeline HUD + Persistent Memory + Encrypted Swarm + UIAutomation)

This is the fully unified file with:
- Encrypted swarm networking
- Adaptive AI HUD (auto-expands/collapses)
- Full Recall Timeline
- Persistent memory (save/load brain state)
- UIAutomation (comtypes + UIAutomationCore.dll)
- Predictive behavior + missing-details detection
- User Activity Governor with tuned data-flow priority
- AI Timing Manager for smoother, faster GUI response
"""
...
class BorgMemoryOrgan:
    ...
    def priority_boost(self, metric: Optional[str]) -> float:
        """
        Tuned context-aware priority boost based on user_mode and metric.
        Higher boosts for what the user is actually doing, slight suppression for irrelevant streams.
        """
        if not metric:
            return 1.0
        ...
        # Global: anomaly + threat + recall always matter more
        if m in ("anomaly_score", "net_threat", "recall"):
            boost *= 1.3
        ...
        if u == "gaming":
            if m in ("gpu_util", "gpu_temp", "gpu_power", "gpu_fan", "game_load", "net_threat", "net_flow"):
                boost *= 2.2
            elif m in ("ui_context", "missing_details", "browser_load"):
                boost *= 0.6

        elif u == "typing":
            if m in ("ui_context", "missing_details", "prediction"):
                boost *= 2.3
            elif m in ("gpu_util", "game_load"):
                boost *= 0.6

        elif u == "browsing":
            if m in ("browser_load", "net_flow", "net_threat", "ui_context"):
                boost *= 1.9
            elif m in ("game_load", "gpu_util"):
                boost *= 0.7

        elif u == "coding":
            if m in ("ui_context", "cpu", "mem", "prediction"):
                boost *= 1.8
            elif m in ("game_load", "gpu_util"):
                boost *= 0.7

        elif u == "configuring":
            if m in ("ui_context", "missing_details", "anomaly_score", "eventlog_pulse"):
                boost *= 2.0
...
class UserActivityGovernorOrgan(BaseOrganThread):
    ...
    Drives tuned context-aware priority in BorgMemoryOrgan.priority_boost().
    """
    def __init__(self, memory: BorgMemoryOrgan, interval_sec: float = 2.0):
        super().__init__(memory, interval_sec)
...
def main():
    organ = BorgMemoryOrgan(
        organ_id="memory_backbone_v11_4",
