#!/usr/bin/env python3
# unified_telemetry_daemon_pyside6.py
# Single-file: telemetry daemon + anomaly engine + firewall + watchdog + PySide6 operator cockpit
...
class OperatorDashboard(QtWidgets.QMainWindow):
    ...
    def _build_ui(self):
        ...
        ip_lan_split = QtWidgets.QSplitter()
        ip_lan_split.setOrientation(QtCore.Qt.Horizontal)
        ip_lan_split.addWidget(self._wrap_group("Top Risky IPs", self.table_ips))
        ip_lan_split.addWidget(self._wrap_group("LAN Hosts", self.table_lan))
        layout.addWidget(ip_lan_split, 2)
        ...
        proc_anom_split = QtWidgets.QSplitter()
        proc_anom_split.setOrientation(QtCore.Qt.Horizontal)
        proc_anom_split.addWidget(self._wrap_group("Top Processes by Outbound Bytes", self.table_procs))
        proc_anom_split.addWidget(self._wrap_group("Recent Anomalies (last 10 minutes)", self.table_anom))
        layout.addWidget(proc_anom_split, 3)
