# =========================
# MIGHTY MOUSE – FULL UPDATED CODE (PART 1 OF 2)
# =========================
    ...
    def _update_cones(self):
        if not self.minimap:
            ...
            for e in self.minimap.get("enemies", []):
                dx = e.get("x, 0") - player["x"] if "x" in e else e.get("x", 0) - player["x"]
                dy = e.get("y, 0") - player["y"] if "y" in e else e.get("y", 0) - player["y"]
...
# =========================
# WEB SURFER (DUAL-MODE URL CAPTURE, INTEGRATED)
# =========================
...
class WebSurfer:
    def __init__(self, orchestrator):
        ...
        try:
            ...
            self.selenium_driver = webdriver.Chrome(options=chrome_opts)
            self.orch.log("[WEBSURFER] Selenium mode enabled")
        except Exception:
            self.selenium_driver = None
            self.orch.log("[WEBSURFER] Selenium not available, using window-title mode")
...
# =========================
# GUI COCKPIT (END OF CLASS)
# =========================

    # End of CockpitGUI class
    # (Part 1 ended inside update_gui — this closes the class cleanly)
    pass
...
def game_get_danger_score(self):
    """Compute danger score based on enemy count."""
    if not self.minimap:
        return 0
    enemies = self.minimap.get("enemies", [])
...
def main():
    ...
    # =========================
    # START WEBSURFER (DUAL-MODE URL CAPTURE)
    # =========================
