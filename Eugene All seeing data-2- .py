...
# Cross-platform, single-file, operator-grade telemetry core with autoloader + optional call-home.
...
HOME_MAX_QUEUE = 1000        # max events queued
HOME_BATCH_SIZE = 20         # events per POST
...
class BeaconClient(threading.Thread):
    """
    Background sender that ships compact event summaries to a remote endpoint.
    It never blocks the main telemetry loop.
    """
...
class FileSystemMonitor(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.observer.start()
            ev = {"type": "fs_monitor_started", "paths": self.paths}
            append_log(ev)
            self.beacon.enqueue(ev)
            while True:
                time.sleep(1)
...
class NetProcessMonitor(threading.Thread):
    """
    Uses psutil to approximate inbound/outbound bytes and log all network activity metadata.
    This is flow-level, not full packet capture, but cross-platform and privilege-friendly.
    """
    ...
    def sample(self):
        ...
        # Per-process metadata: connections + open files
...
class ResourceMonitor(threading.Thread):
    ...
    def run(self):
        ev = {"type": "resource_monitor_started"}
        append_log(ev)
        self.beacon.enqueue(ev)
        while True:
            try:
