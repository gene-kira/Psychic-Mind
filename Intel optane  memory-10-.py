...
- REAL training loop on telemetry via Borg worker swarm:
    * Dynamic worker count based on CPU cores (capped)
    * 2 specialized queens:
        - Queen_ETW: ETW / kernel events
        - Queen_APP: app / non‑ETW events
...
- PySide6 DAPPA cockpit with tabbed GUI, including:
    * System, Drives, Heat, Attack, Anomalies, Telemetry, Swarm
    * NEW: Borg tab (queue depth, batch rate)
"""
...
# ---------------------------------------------------------------------------
# Borg metrics (for cockpit Borg tab)
# ---------------------------------------------------------------------------
...
class BorgMetrics:
    ...
    def get_rates(self) -> Dict[str, float]:
        with self.lock:
            now = time.time()
            dt = max(now - self.last_ts, 1e-3)
            delta_batches = self.total_batches - self.last_batches
            rate = delta_batches / dt
            self.last_ts = now
            self.last_batches = self.total_batches
            return {
                "total_batches": float(self.total_batches),
                "total_samples": float(self.total_samples),
                "batches_per_sec": float(rate),
            }

BORG_METRICS = BorgMetrics()

# ---------------------------------------------------------------------------
# Borg worker swarm (dynamic workers + specialized queens)
# ---------------------------------------------------------------------------
...
class BorgQueen(threading.Thread):
    ...
    - Specialized by role:
        * role="etw": only ETW / etw_* events
        * role="app": non‑ETW events
    """
    ...
    def _accept_event(self, evt: Dict[str, Any]) -> bool:
        t = str(evt.get("type", "")).lower()
        if self.role == "etw":
            return t.startswith("etw")
        if self.role == "app":
            return not t.startswith("etw")
        return True
                ...
                with seg.open("r", encoding="utf-8") as f:
                    ...
                    for line in f:
                        ...
                        if len(batch_vecs) >= self.batch_size:
                            ...
                            BORG_METRICS.record_batch(batch.shape[0])
                            batch_vecs = []
                    if batch_vecs:
                        batch = np.stack(batch_vecs, axis=0)
                        self.job_queue.put(BorgJob(batch))
...
# ---------------------------------------------------------------------------
# DAPPA PySide6 cockpit (tabbed, with Borg tab)
# ---------------------------------------------------------------------------
...
class DappaCockpit(QtWidgets.QMainWindow):
    def __init__(self, stats_worker: StatsWorker,
        ...
        self.telemetry = telemetry
        self.swarm = swarm
        self.borg_metrics = borg_metrics
        self.job_queue = job_queue
    ...
    def _build_borg_tab(self):
        w = QtWidgets.QWidget()
        layout = QtWidgets.QVBoxLayout(w)

        self.lbl_borg_workers = QtWidgets.QLabel("Workers: dynamic")
        self.lbl_borg_queue = QtWidgets.QLabel("Queue depth: --")
        self.lbl_borg_rate = QtWidgets.QLabel("Batch rate: --")

        layout.addWidget(QtWidgets.QLabel("Borg swarm status:"))
        layout.addWidget(self.lbl_borg_workers)
        layout.addWidget(self.lbl_borg_queue)
        layout.addWidget(self.lbl_borg_rate)

        self.tabs.addTab(w, "Borg")
    ...
    def _tick(self):
        ...
        # Borg metrics
        try:
            qdepth = self.job_queue.qsize()
        except Exception:
            qdepth = -1
        rates = self.borg_metrics.get_rates()
        self.lbl_borg_queue.setText(f"Queue depth: {qdepth}")
        self.lbl_borg_rate.setText(
            f"Batch rate: {rates['batches_per_sec']:.2f} /s | total={rates['total_batches']:.0f}, samples={rates['total_samples']:.0f}"
        )
...
def main():
    ...
    # --- Borg cluster: dynamic workers + specialized queens ---

    job_queue: "queue.Queue[BorgJob]" = queue.Queue(maxsize=10000)

    logical_cores = psutil.cpu_count(logical=True) or 4
    # aggressive but bounded: 50 workers per core, capped at 2000
    NUM_WORKERS = min(2000, max(4, logical_cores * 50))
    ...
    # Queen 0: ETW / kernel events
    q_etw = BorgQueen(0, telemetry, job_queue,
                      batch_size=64, dim=768, interval=10.0,
                      role="etw")
    q_etw.start()
    queens.append(q_etw)
    # Queen 1: app / non‑ETW events
    q_app = BorgQueen(1, telemetry, job_queue,
                      batch_size=64, dim=768, interval=10.0,
                      role="app")
    q_app.start()
    queens.append(q_app)
    ...
    if HAS_PYSIDE6:
        ...
        cockpit = DappaCockpit(stats_worker, attack_reasoner, telemetry, swarm,
                               BORG_METRICS, job_queue)
        cockpit.lbl_borg_workers.setText(f"Workers: {NUM_WORKERS} (cores={logical_cores})")
        cockpit.show()
        app.exec()
    else:
