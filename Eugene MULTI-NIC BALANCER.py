...
# -*- coding: utf-8 -*-
"""
network_organism_full.py

Unified network organism:
- Autoloader
- Cross-platform HAL (Windows/Linux/macOS)
- NIC sensors
- GPU/NPU anomaly bridge
- AI cortex (pluggable PyTorch/CuPy-style backends)
- Swarm mesh (queen/worker)
- NetworkBrain (global / in-out / priority split)
- Gaming QoS hooks (DSCP/tc/WFP placeholders)
...
OPTIONAL_LIBS = ["PySide6", "torch", "cupy"]
...
class NetworkHAL:
    """
    Cross-platform abstraction for routing and QoS.
    Real implementations can be extended per environment.
    """
    ...
    def set_default_route(self, nic):
        ...
        try:
            if self.os == "Windows":
                ...
                # Simplified; real macOS routing may need more detail
                subprocess.run(
    ...
    def apply_in_out_split(self, in_nic, out_nic):
        if self.os == "Windows":
            ...
            # Hook: implement policy routing per OS
            if in_nic:
    ...
    def apply_priority_split(self, high_prio_nic, low_prio_nic):
        if self.os == "Windows":
            ...
            # Hook: implement tc/qdisc or policy routing
            if high_prio_nic:
    ...
    def apply_gaming_qos(self, game_process_names, high_prio_nic):
        ...
        Placeholder:
        - Windows: WFP/DSCP
        - Linux: tc + iptables marks
        - macOS: PF rules
        """
        # This is intentionally left as a hook; environment-specific.
        pass
...
class GPUAnomalyBridge:
    """
    Bridge from external GPU/NPU anomaly engines into ThreatHeatmap.
    """
...
class AICortex:
    """
    Simple wrapper that can use:
    - Pure Python heuristic
    - PyTorch model (if available)
    - CuPy accelerated scoring (if available)
    """
    ...
    def score(self, nic_features: dict):
        """
        nic_features: {nic: feature_vector}
        Returns: {nic: model_score}
        """
    ...
    def _score_heuristic(self, nic_features: dict):
        # Identity: just sum features as a placeholder
        out = {}
...
class NetworkBrain:
    ...
    def snapshot_state(self):
        ...
        best, second = self._decide(merged, peers)
    ...
    def _decide(self, merged_scores: dict, peers_snapshot: dict):
        if not merged_scores:
            return None, None

        # AI cortex hook: build feature vectors
        nic_features = {nic: [score] for nic, score in merged_scores.items()}
        ai_scores = self.ai_cortex.score(nic_features)

        # Combine AI scores with merged scores
...
if HAVE_QT:
    class NetworkCockpit(QtWidgets.QWidget):
        def __init__(self, brain: NetworkBrain):
            super().__init__()
            self.brain = brain
            self.setWindowTitle("Network Brain Cockpit")
            self.resize(900, 600)
        ...
        def refresh(self):
            ...
            self.info_label.setText(
                f"Node: {state.get('node_id')} | Role: {state.get('role')} | "
                f"Mode: {state.get('routing_mode')} | Best: {best} | Second: {second}"
            )
            ...
            for row, nic in enumerate(sorted(nic_health.keys())):
                ...
                self.table.setItem(row, 1, QtWidgets.QTableWidgetItem(f"{nic_health[nic]:.2f}"))
                self.table.setItem(row, 2, QtWidgets.QTableWidgetItem(f"{nic_threat.get(nic, 0.0):.2f}"))
                self.table.setItem(row, 3, QtWidgets.QTableWidgetItem(f"{merged.get(nic, 0.0):.2f}"))
