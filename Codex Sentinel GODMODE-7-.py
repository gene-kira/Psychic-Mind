"""
Codex Sentinel GODMODE — Cluster Edition (Async + GPU Monitor + Persistent Config)
"""
...
# ============================================================
# GODMODE Orchestrator (with GPU monitor + async hooks)
# ============================================================
...
# ============================================================
# Async Orchestration Supervisor
# ============================================================
...
class AsyncOrchestratorSupervisor:
    ...
    async def _gpu_task(self):
        while not self._stop:
            ...
            await asyncio.sleep(CONFIG.get("gpu_poll_interval", 10))
    ...
    async def _telemetry_task(self):
        while not self._stop:
            ...
            await asyncio.sleep(CONFIG.get("telemetry_poll_interval", 5))
    ...
    async def _self_update_task(self):
        while not self._stop:
            ...
            await asyncio.sleep(CONFIG.get("update_check_interval", 3600))
...
# ============================================================
# First‑Run Wizard (writes persistent config)
# ============================================================
...
def run_pyside6_cockpit(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    class CockpitWindow(QMainWindow):
        def __init__(self, orch: GODMODEOrchestrator, sup: AsyncOrchestratorSupervisor):
            ...
            header = QLabel("Codex Sentinel GODMODE — Cluster Cockpit (Async + GPU Monitor)")
