...
class EmergentSwarmBrain:
    ...
    def update(self, drone_reports: List[Dict]):
        ...
        # simple predictive routing: amplify current vector
        self.predicted_vector = (self.collective_vector[0] * 1.5,
                                 self.collective_vector[1] * 1.5)
...
class SwarmCockpit:
    def __init__(self, swarm: Swarm, telemetry: SystemTelemetry):
        self.swarm = swarm
...
def main():
    ...
    ui_driver = UIDriver(enabled=False)

    drones = [
        DataDrone("drone_1", physics, ui_driver, (200, 200), DroneRole.SCOUT, DronePersonality.AGGRESSIVE),
        DataDrone("drone_2", physics, ui_driver, (400, 200), DroneRole.OPERATOR, DronePersonality.BALANCED),
        DataDrone("drone_3", physics, ui_driver, (600, 200), DroneRole.EXECUTOR, DronePersonality.CAUTIOUS),
    ]
    ...
    cockpit = SwarmCockpit(swarm, telemetry)
    cockpit.run()
