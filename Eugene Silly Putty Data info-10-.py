#!/usr/bin/env python3
# Full Borg Organism – Next-Level Autonomous ASI Core (Upgraded v3)
# Prediction, Learning, Defense, Autonomy, GUI, Swarm, Aliveness
# Hybrid Reboot Memory (SMB + Local Fallback) + Episodic Memory + Cross-Organ Fusion
# Upgrades: richer anomaly physics, better BrainCore, self-rewriting, codex mutation, swarm, GUI, daemon
...
# =========================
# Silly Putty Organism (Anomaly Physics)
# =========================
...
class BorgQueen:
    ...
    def export_swarm_view(self) -> Dict[str, Any]:
        with self.lock:
            return {
...
# =========================
# Adaptive Codex Mutation (Upgraded)
# =========================
...
class SelfRewritingAgent:
    def __init__(self, learning_rate: float = 0.05):
        ...
        self.backend = "local_evo"

    def mutate(self, risk: float, health: float, turbulence: float, swarm_risk: float):
        before = dict(self.weights)
        score = risk * 1.2 + turbulence * 0.8 - health * 0.5 + swarm_risk
        delta = self.learning_rate * (score - 1.0)

        self.weights["risk"] = max(0.5, min(2.0, self.weights["risk"] + delta * 0.3))
        self.weights["health"] = max(0.5, min(2.0, self.weights["health"] - delta * 0.2))
        self.weights["turbulence"] = max(0.5, min(2.0, self.weights["turbulence"] + delta * 0.4))
        self.weights["swarm_risk"] = max(0.5, min(2.0, self.weights["swarm_risk"] + delta * 0.3))
        ...
        if len(self.mutation_history) > 50:
...
# =========================
# Autonomous Cipher Engine (Upgraded)
# =========================
...
class AutonomousCipherEngine:
    ...
    def tick(self, risk: float, entropy: float):
        now = time.time()
        base_interval = 600.0
        if risk > 0.7 or entropy > 0.7:
            self.posture = "HARDENED"
            self.rotation_interval = max(60.0, base_interval * 0.3)
        elif risk > 0.4 or entropy > 0.5:
            self.posture = "ELEVATED"
            self.rotation_interval = max(120.0, base_interval * 0.6)
        else:
            self.posture = "NORMAL"
            self.rotation_interval = base_interval
    ...
    def snapshot(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "seconds_since_rotation": int(now - self.last_rotation),
        }
...
class BrainCore:
    ...
    def __init__(self, chain_engine: AttackChainEngine, learning_rate: float = 0.05):
        ...
        self.internal_dialogue: deque = deque(maxlen=50)
    ...
    def tick(self, putty: SillyPuttyDataGatherer, organs: List[BaseOrgan], swarm_view: Dict[str, float]):
        self._tick_count += 1
        ...
        if organs:
            avg_health = sum(o.health for o in organs) / len(organs)
        else:
            avg_health = 1.0

        swarm_risk = swarm_view.get("swarm_risk", 0.0)

        base_risk = anomaly_density * self.anomaly_sensitivity
        baseline_dev = abs(base_risk - self.risk_score)
        variance = 0.0
        trend = 0.0
        ...
        best_guess = self._multi_engine_vote(base_risk, variance, trend, turbulence, baseline_dev, organ_vector)

        ewma_alpha = 0.2
        self.risk_score = (1 - ewma_alpha) * self.risk_score + ewma_alpha * best_guess
        self._risk_history.append(self.risk_score)

        self._update_regime()
        self._multi_horizon_forecast(self.risk_score, turbulence)

        self.collective_health = (self.collective_health * 0.9) + (avg_health * 0.1)
        if len(self._risk_history) > 5:
            self.health_trend = self._risk_history[-1] - self._risk_history[0]
        else:
            self.health_trend = 0.0
        ...
        if self.risk_score > self.risk_tolerance:
            self.anticipation = "Spike likely"
        elif self.predictions["30s"] > self.risk_tolerance:
            self.anticipation = "Rising soon"
        else:
            self.anticipation = "Stable window"

        ghost = self.codex.detect_ghost_sync(self.risk_score, self.collective_health, turbulence)
        if ghost:
            self.codex.mutate_rules("ghost_sync")
            self._log_internal("Ghost-sync detected; codex mutated.")
        elif self._tick_count % 50 == 0:
            self.codex.mutate_rules("periodic")
            self._log_internal("Periodic codex mutation.")

        self.agent.mutate(self.risk_score, self.collective_health, turbulence, swarm_risk)

        entropy_proxy = min(1.0, anomaly_density + turbulence * 0.5)
        self.cipher.tick(self.risk_score, entropy_proxy)
        ...
        if self.mission == "AUTO":
            if self.environment == "DANGER":
                ...
                self.anomaly_sensitivity = min(1.0, self.anomaly_sensitivity + 0.01)
            elif self.environment == "CALM":
                self.risk_tolerance = min(0.8, self.risk_tolerance + 0.005)
                self.anomaly_sensitivity = max(0.3, self.anomaly_sensitivity - 0.005)
        ...
        if self._tick_count % 30 == 0:
            self._log_internal(
                f"Tick={self._tick_count} Regime={self.regime} Meta={self.meta_state} Risk={self.risk_score:.3f} Health={self.collective_health:.3f}"
            )
...
if GUI_AVAILABLE:
    class BorgCockpit(QWidget):
        def __init__(self,
            ...
            self.setWindowTitle("Borg Organism Cockpit – Full Brain v3")
        ...
        def _tick(self):
            ...
            reasoning = self.brain.reasoning_heatmap
    ...
    while True:
        ...
        if now - last_snapshot >= snapshot_interval:
            snapshot = {
                ...
                "reasoning": brain.reasoning_heatmap,
...
def main():
    ...
    swarm_consensus = SwarmConsensusEngine()
    chain_engine = AttackChainEngine()
    brain = BrainCore(chain_engine=chain_engine, learning_rate=0.05)
    reboot_mgr = RebootMemoryManager()
    ...
    if GUI_AVAILABLE:
        ...
        daemon_thread = threading.Thread(
            ...
            args=(autoloader, putty, organs, self_integrity, queen, rt_queen, swarm_consensus, chain_engine, brain, reboot_mgr),
            daemon=True,
        )
        daemon_thread.start()
        ...
        autonomous_loop(
            autoloader, putty, organs, self_integrity, queen, rt_queen, swarm_consensus, chain_engine, brain, reboot_mgr
        )
