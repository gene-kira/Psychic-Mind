#!/usr/bin/env python3
# OmniQueen Swarm Guard - Full Platform (Upgraded ML + ETW)
# - Swarm + Queen
# - PySide6 Cockpit
# - ML anomaly model (+ optional GPU, richer features)
# - Steam/Epic whitelist
# - Remote command API
# - Real ETW skeleton (Windows, etw lib)
...
REQUIRED_LIBS = ["psutil", "yaml", "PySide6", "sklearn"]
...
if platform.system().lower() == "windows":
    ...
    def ensure_admin():
        try:
            ...
            print(f"[OmniQueen] Elevation failed: {e}")
            sys.exit()
...
COMMON_SHELL_PARENTS = {
    "cmd.exe", "powershell.exe", "pwsh.exe", "bash.exe",
    "sh.exe", "zsh", "fish", "wsl.exe"
}
COMMON_SYSTEM_PARENTS = {
    "services.exe", "wininit.exe", "winlogon.exe", "svchost.exe",
    "systemd", "launchd"
}
...
def string_entropy(s: str) -> float:
    ...
    for c in counts.values():
        p = c / total
        ent -= p * math.log2(p)
    return ent / 8.0  # light normalization
...
def get_parent_info(pid):
    try:
        ...
        if not parent:
            return None, None
        return parent.pid, (parent.name() or "").lower()
    except Exception:
        return None, None
...
class MLAnomalyModel:
    ...
    def add_sample(self, features):
        ...
        if len(self.buffer) >= 200 and not self.trained:
            try:
                self.model.fit(self.buffer)
                self.trained = True
                log_event("INFO", "ml", "ML model trained", {"samples": len(self.buffer)})
            except Exception as e:
                log_event("ERROR", "ml", "Training failed", {"error": str(e)})
    ...
    def score(self, features):
        ...
        try:
            if GPU_AVAILABLE:
                arr = cupy.array([features], dtype=cupy.float32)
                X = cupy.asnumpy(arr)
            else:
            ...
            log_event("ERROR", "ml", "Scoring failed", {"error": str(e)})
            return 0.0
...
class BorgMemory:
    ...
    def score_process(self, name, cmdline, suspicious_keywords, pid=None):
        rec = self.proc_stats.get(name, {"count": 0, "last_seen": 0})
        count = rec["count"]
        ...
        for kw in suspicious_keywords:
            if kw in low_name or kw in low_cmd:
                keyword_hits += 1
        keyword_hits_norm = normalize(keyword_hits, 0, 5)

        name_ent = string_entropy(low_name)

        cmd_tokens = low_cmd.split()
        cmd_len = len(cmd_tokens)
        cmd_len_norm = normalize(cmd_len, 0, 50)

        parent_is_shell = 0.0
        parent_is_system = 0.0
        if pid is not None:
            _, parent_name = get_parent_info(pid)
            if parent_name:
                if parent_name in COMMON_SHELL_PARENTS:
                    parent_is_shell = 1.0
                if parent_name in COMMON_SYSTEM_PARENTS:
                    parent_is_system = 1.0

        env_sigma_norm = 0.0
        if self.proc_count_stats and self.proc_count_stats["count"] > 5:
            sigma = stats_sigma(self.proc_count_stats)
            env_sigma_norm = normalize(sigma, 0.0, self.ai_cfg.get("sigma_multiplier", 2.0) * 10.0)
        ...
        pressure_norm = normalize(pressure, 0.0, 0.5)
        ...
        if pid is not None:
            try:
                ...
                cpu_burst_norm = normalize(cpu, 0.0, 100.0)
                total_mem = psutil.virtual_memory().total or 1
                ram_norm = normalize(mem / total_mem, 0.0, 0.1)
            except Exception:
                pass

        spawn_rate_norm = 1.0 - rarity

        base_score = min(1.0,
                         rarity * 0.3 +
                         keyword_hits_norm * 0.3 +
                         env_sigma_norm * 0.2 +
                         pressure_norm * 0.2)
        ...
        self.ml_model.add_sample(features)
        ml_score = self.ml_model.score(features)

        final_score = min(1.0, 0.5 * base_score + 0.5 * ml_score)
        return final_score
    ...
    def score_ip(self, ip):
        ...
        port_entropy = 0.0  # placeholder for future per-IP port tracking
...
def process_watch_loop(cfg_ref, state, borg: BorgMemory, stop_event):
    ...
    while not stop_event.is_set():
        ...
        for p in procs:
            ...
            score = borg.score_process(name, cmdline, suspicious_keywords, pid=pid)
            if score >= 0.85:
...
# ---------------- ETW / kernel sensor (real skeleton) ----------------

def etw_loop(borg: BorgMemory, stop_event):
    if not ETW_AVAILABLE:
        log_event("INFO", "etw", "ETW not available (library missing or non-Windows)", {})
        return

    providers = [
        ProviderInfo("Microsoft-Windows-Kernel-Process"),
        ProviderInfo("Microsoft-Windows-Kernel-Network"),
    ]

    def callback(event):
        try:
            pid = event.get("ProcessId") or event.get("PID")
            opcode = event.get("EventName") or event.get("OpcodeName") or ""
            opcode_l = opcode.lower()

            if "start" in opcode_l or "create" in opcode_l:
                image = (event.get("ImageFileName") or "").lower()
                cmdline = (event.get("CommandLine") or "")
                borg.register_threat_event(weight=0.1)
                log_event("INFO", "etw_proc", "Process start (ETW)", {
                    "pid": pid,
                    "image": image,
                    "cmdline": cmdline[:200]
                })

            if "connect" in opcode_l:
                dst_ip = event.get("daddr") or event.get("DestIp") or ""
                borg.register_threat_event(weight=0.1)
                if dst_ip:
                    borg.mark_suspect_ip(dst_ip)
                    log_event("INFO", "etw_net", "Network connect (ETW)", {
                        "dst_ip": dst_ip
                    })

        except Exception as e:
            log_event("ERROR", "etw", "ETW callback error", {"error": str(e)})

    try:
        etw = ETW(providers=providers, event_callback=callback)
        log_event("INFO", "etw", "ETW loop starting with real providers", {})
        for _ in etw:
            if stop_event.is_set():
                break
    except Exception as e:
        log_event("ERROR", "etw", "ETW loop crashed", {"error": str(e)})
...
class CommandHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        borg = REMOTE_API_CONTEXT["borg"]
        cfg_ref = REMOTE_API_CONTEXT["cfg_ref"]
        if borg is None or cfg_ref is None:
            self._send_json(500, {"error": "context not ready"})
            return
        ...
        if cmd == "get_status":
            ...
            cfg = cfg_ref["cfg"]
            manual_cfg = cfg.get("manual_override", {})
            manual_cfg["panic_mode"] = val
            cfg["manual_override"] = manual_cfg
            save_config(cfg)
            cfg_ref["cfg"] = load_config()
            if val:
                borg.altered_state = "CALM"
            self._send_json(200, {"panic_mode": val})
        elif cmd == "set_state":
            target = data.get("state", "").upper()
            if target in ALTERED_STATES:
                borg.altered_state = target
                self._send_json(200, {"altered_state": borg.altered_state})
            else:
                self._send_json(400, {"error": "invalid state"})
        else:
            self._send_json(400, {"error": "unknown cmd"})

    def log_message(self, format, *args):
        return

def start_command_api(borg: BorgMemory, cfg_ref):
    cfg = cfg_ref["cfg"]
    api_cfg = cfg.get("remote_api", {})
    if not api_cfg.get("enabled", True):
        return
    host = api_cfg.get("host", "127.0.0.1")
    port = api_cfg.get("port", 8088)

    REMOTE_API_CONTEXT["borg"] = borg
    REMOTE_API_CONTEXT["cfg_ref"] = cfg_ref

    def run():
        try:
            server = HTTPServer((host, port), CommandHandler)
            log_event("INFO", "remote_api", "Remote API started", {"host": host, "port": port})
            server.serve_forever()
        except Exception as e:
            log_event("ERROR", "remote_api", "Remote API failed", {"error": str(e)})
...
class Cockpit(QWidget):
    def __init__(self, cfg_ref, borg: BorgMemory, state_ref, parent=None):
        ...
        lists_group = QGroupBox("Suspects & Timeline")
        ...
        lists_layout.addWidget(self.timeline_list)
...
def main():
    ...
    t_etw = threading.Thread(target=etw_loop, args=(borg, stop_event), daemon=True)
    ...
    start_command_api(borg, cfg_ref)
