...
class WebBrain:
    ...
    def _check_ip_trust(self, domain, ip_str):
        ...
        learned = self.trusted_ips.get(domain, [])
        if not learned:
            self.trusted_ips.setdefault(domain, []).append(ip_str)
            self.memory.save()
            return ("LEARNED", f"{domain} → {ip_str} learned as trusted")

        if ip_str in learned:
            return ("GOOD", f"{domain} → {ip_str} matches learned trusted IP")
        else:
            return ("MISMATCH", f"{domain} → {ip_str} differs from learned trusted IPs")
...
class CockpitGUI(QWidget):
    ...
    def update_gui(self):
        ...
        if self.web.latest_url:
            ...
            if status_line:
                self.web_status_label.setText(status_line.replace("IP status:", "Web IP Status:"))
            else:
