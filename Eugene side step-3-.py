...
DEFAULT_ORGANS = ["telemetry", "filesystem", "browser", "game", "external_api",
                  "uiautomation", "heartbeat", "ml_inference", "gpu_inference"]
DEFAULT_LANES = ["system", "io", "activity", "telemetry", "signal", "focus", "control", "analysis"]
DEFAULT_RITUALS = ["sample", "fs_event", "url_visit", "tick", "ping",
                   "window_change", "alert", "classify", "gpu_score"]

WATCH_DIR = os.path.abspath(".")
ENCRYPTION_KEY = b"mythic_shared_key"  # simple shared key for XOR
...
def encrypt_payload(obj):
    ...
    enc = xor_encrypt(raw, ENCRYPTION_KEY)
    return base64.b64encode(enc).decode("utf-8")

def decrypt_payload(s):
    try:
        enc = base64.b64decode(s.encode("utf-8"))
        raw = xor_encrypt(enc, ENCRYPTION_KEY)
...
def log_swarm(node_id, direction, peer, msg):
    ...
    c = conn.cursor()
    c.execute("""
        INSERT INTO swarm_messages(node_id, direction, peer, payload, priority, organ, lane, ritual, ts)
        VALUES (?,?,?,?,?,?,?,?,?)
    """, (
        node_id,
        direction,
...
def query_swarm(limit=100, priority=None, tag=None):
    conn = sqlite3.connect(SWARM_DB)
    c = conn.cursor()
    base = "SELECT node_id, direction, peer, payload, priority, organ, lane, ritual, ts FROM swarm_messages"
    params = []
    where = []
    if priority:
...
class NodeServer:
    ...
    def send_to_neighbor(self, neighbor, msg):
        ...
        data = json.dumps({"enc": enc}).encode("utf-8")
    ...
    def filesystem_sensor_loop(self):
        ...
        class FSHandler(FileSystemEventHandler):
            ...
            def on_any_event(self, event):
                ...
                neighbors = get_neighbors(self.node.node_id)
                if neighbors:
                    target = random.choice(neighbors)
                    self.node.send_to_neighbor(target, msg)
...
class SwarmGUI:
    def __init__(self, root, node_procs):
        ...
        root.title("Mythic Federated Sidestep Swarm (A–G, Live, Encrypted, ML, GPU)")
        ...
        ttk.Button(root, text="Run Query (Swarm)", command=self.run_query_swarm).grid(row=9, column=2, columnspan=2, pady=5)
    ...
    def send_message(self):
        ...
        enc = encrypt_payload(msg)
        try:
            http_post(host, port, "/message", {"enc": enc})
