...
def main():
    ...
    from PySide6.QtGui import QPainter, QColor, QFont, QMouseEvent, QFontMetrics
    ...
    class AIInterfacePanel(QWidget):
        def __init__(self, parent=None):
            ...
            self.border_color = QColor(0, 200, 255, 180)
            ...
            self.resize(self.max_width, 140)

        def update_ai_state(self, engine, tag, prompt, response):
            self.engine_name = engine or "Unknown"
            self.reason_tag = tag or ""
            self.timestamp = time.strftime("%H:%M:%S")

            # Trim prompt to first line only
            self.prompt_trimmed = (prompt or "").split("\n")[0][:80]

            # Full response
            self.response_full = (response or "").strip()
        ...
        def sizeHint(self):
            if self.collapsed:
                return QSize(self.max_width, self.header_height)

            fm = QFontMetrics(QFont("Segoe UI", 9))
            resp_height = fm.boundingRect(
                0, 0,
                self.max_width - 20,
                9999,
                Qt.TextWordWrap,
            ...
            total = self.header_height + 60 + resp_height
            return QSize(self.max_width, min(total, 300))
        ...
        def paintEvent(self, event):
            ...
            header = f"AI: {self.engine_name}   [{self.reason_tag}]   {self.timestamp}"
            painter.drawText(10, 18, header)
            ...
            painter.drawText(10, 40, f"Prompt: {self.prompt_trimmed}")

            painter.setPen(QColor(255, 255, 255))
            resp_rect = QRect(10, 60, self.width() - 20, self.height() - 70)
            painter.drawText(resp_rect, Qt.TextWordWrap, self.response_full)
    ...
    class OverlayWindow(QWidget):
        def __init__(self):
            ...
            # AI interface panel (top-right)
            self.ai_panel = AIInterfacePanel(self)
            self.position_ai_panel()
            self.ai_panel.show()
        ...
        def position_ai_panel(self):
            if hasattr(self, "ai_panel") and self.ai_panel is not None:
                x = self.width() - self.ai_panel.width() - 10
                y = 10
                self.ai_panel.move(x, y)

        # ---------- AI PANEL UPDATE ----------

        def update_ai_panel(self, tag, prompt, response):
            try:
                eng = self.ai.get_best_engine()
                name = eng.get("name") if eng else "Unknown"
            except Exception:
                name = "Unknown"
            clean_tag = tag.strip("[]") if tag else ""
            self.ai_panel.update_ai_state(name, clean_tag, prompt, response)
        ...
        def translate_under_cursor(self):
            ...
            try:
                ...
                resp = self.ai.ask(prompt)
                self.translation_text = resp
                self.update_ai_panel("TRANSLATE", prompt, resp)
            except Exception as e:
        ...
        def explain_under_cursor(self):
            ...
            try:
                ...
                resp = self.ai.ask(prompt)
                self.explanation_text = resp
                self.update_ai_panel("EXPLAIN", prompt, resp)
            except Exception as e:
        ...
        def update_cursor_commentary(self):
            try:
                ...
                timestamp = time.strftime("%H:%M:%S")
                tag = "[CURSOR]"
                line = f"{timestamp} {tag} {comment}"
                self.hint_lines.append(line)
                self.trim_hints_to_fit()
                self.update_ai_panel(tag, prompt, comment)
                self.update()
            except Exception:
                pass
