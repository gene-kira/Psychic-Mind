#!/usr/bin/env python3
"""
Codex Sentinel – Unified Daemon + Mesh + Wrappers
"""
...
def ensure_wrapper_for_script(script_path: str) -> str | None:
    ...
    - Performs health-check (10–20s + port binding)
    - Detects crash-loops and backs off
    """
...
def raft_apply_committed():
    with lock:
        while state["raft_last_applied"] < state["raft_commit_index"]:
            ...
            cmd = entry["command"]
            # apply replicated commands here if needed
...
def handle_consensus_vote(msg: dict):
    # placeholder: track votes and when majority reached, emit BLOCKLIST_UPDATE
    pass
