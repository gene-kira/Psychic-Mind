...
def query_swarm(limit: int = 100, priority: Optional[str] = None, tag: Optional[str] = None) -> List[Dict[str, Any]]:
    conn = sqlite3.connect(SWARM_DB)
    c = conn.cursor()
    base = (
...
def add_automation_rule(name: str, condition: Dict[str, Any], action: Dict[str, Any], enabled: bool = True):
    conn = sqlite3.connect(SWARM_DB)
    c = conn.cursor()
    c.execute("""
...
def get_neighbors(node_id: str, include_unhealthy: bool = False) -> List[Dict[str, Any]]:
    ...
    conn.close()
    return [
        {"id": r[0], "host": r[1], "port": r[2], "healthy": r[3], "last_fail": r[4]}
        for r in rows
    ]
...
def add_constraint(node_id: str, organ: Optional[str], lane: Optional[str],
                   ritual: Optional[str], priority: str):
    conn = sqlite3.connect(db_path(node_id))
    c = conn.cursor()
    c.execute(
...
def get_q_value(node_id: str, neighbor_id: str, state: str = "default", action: str = "send") -> float:
    conn = sqlite3.connect(db_path(node_id))
    c = conn.cursor()
    c.execute(
...
def set_q_value(node_id: str, neighbor_id: str, state: str, action: str, q: float):
    conn = sqlite3.connect(db_path(node_id))
    c = conn.cursor()
    c.execute(
...
def update_q(node_id: str, neighbor_id: str, reward: float,
             state: str = "default", action: str = "send",
...
def detect_gpu(loader: AutoLoader) -> bool:
    for mod_name, attr in [("torch", "cuda"), ("cupy", None)]:
        ...
        if mod:
            try:
                if mod_name == "torch" and hasattr(mod, "cuda") and mod.cuda.is_available():
...
class Policy:
    ...
    def adapt_from_predictions(
        ...
        if predicted_cpu < 40 and predicted_mem < 60 and cycle_errors == 0 and not swarm_anomaly_vote:
            new_interval -= 1
...
class WindowsOrgans:
    ...
    def registry_sample(self):
        if not self.winreg:
            ...
            return
        try:
            key = self.winreg.OpenKey(self.winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE", 0, self.winreg.KEY_READ)
            subkey_count, _, _ = self.winreg.QueryInfoKey(key)
            print(f"[WinOrgans] HKLM\\SOFTWARE has ~{subkey_count} subkeys (read-only sample)")
    ...
    def list_services(self):
        ...
        try:
            ...
            for svc in services():
                if count >= 5:
                    break
                print(f"[WinOrgans] Service: {svc.name()} (status={svc.status()})")
                count += 1
        except Exception as e:
    ...
    def ui_automation_sample(self):
        ...
        try:
            ...
            print(f"[WinOrgans] Top-level windows detected: {len(windows)} (showing up to 5)")
            for w in windows[:5]:
...
class BaseAgent:
    def __init__(self, name: str, role: str, loader: AutoLoader, env: EnvironmentProfile, policy: Policy):
        self.name = name
        self.role = role
        self.loader = loader
        self.env = env
        self.policy = policy
        self.mode = "ant"
        self.windows_organs = WindowsOrgans(loader) if policy.can_use_windows_organs(env) else None
        self.linux_organs = LinuxOrgans(loader) if policy.can_use_linux_organs(env) else None
    ...
    def run_giant(self, memory: MemoryState) -> Tuple[bool, float, float, bool]:
        ...
        if self.role == "analyst" and numpy:
            ...
            print(f"[{self.name}][{self.role}] Mean={data.mean():.4f}, Std={data.std():.4f}")
    ...
    def detect_anomaly(self, memory: MemoryState, cpu: float, mem: float) -> bool:
        ...
        if is_anomaly:
            ...
            print(f"[{self.name}][{self.role}] Anomaly detected: score={score:.2f}, cpu={cpu}, mem={mem}")
...
class Swarm:
    def __init__(self, loader: AutoLoader, env: EnvironmentProfile, policy: Policy, node_id: str = "A"):
        self.loader = loader
        self.env = env
        self.policy = policy
    ...
    def _build_swarm(self):
        ...
        print(f"[Swarm] Built swarm with {len(self.agents)} nodes for logical node {self.node_id}")
    ...
    def run_all(self, memory: MemoryState) -> Tuple[int, float, float, bool]:
        ...
        for agent in self.agents:
            ...
            update_agent_stats(self.node_id, agent.role, msg["intent"], success=success and not anomaly)
        ...
        if neighbor:
            ...
            print(f"[Swarm] RL updated neighbor {neighbor['id']} with reward={reward}")
...
def main():
    ...
    while True:
        ...
        if elapsed >= memory.loop_interval_seconds:
            print("==================================================")
            print(f"[Main] Swarm cycle starting (interval={memory.loop_interval_seconds}s, elapsed={elapsed:.2f}s)")
            ...
            print(f"[Main] Predictions: next_cpu={pred_cpu:.1f}, next_mem={pred_mem:.1f}")

            policy.adapt_from_predictions(memory, pred_cpu, pred_mem, errors, swarm_anomaly)
