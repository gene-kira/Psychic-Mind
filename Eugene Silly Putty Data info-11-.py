...
# + Real telemetry hooks (psutil, optional ETW), emotional/meta states, auto-defense,
#   swarm sync over HTTP, and enhanced cockpit.
...
# =========================
# Telemetry: psutil + optional ETW + simple net hooks
# =========================
...
class WindowsETWListener:
    def __init__(self):
        ...
        if platform.system().lower().startswith("win"):
            try:
                # Placeholder: real ETW integration would go here
                self.available = True
            except Exception:
                self.available = False
    ...
    def poll_events(self) -> List[Dict[str, Any]]:
        if not self.available:
            return []
        # Placeholder synthetic ETW-like events
        events = []
        now = time.time()
...
def autonomous_loop(autoloader: BorgAutoloader,
    ...
    while True:
        ...
        # psutil metrics influence putty physics slightly
        metrics = telemetry.collect_psutil_metrics()
        # we could inject metrics into putty via extra bytes, but for now they just inform brain via swarm_view

        telemetry.scan_rogue_hosts()
        for ip in list(telemetry.rogue_hosts):
            # defensive auto-block
