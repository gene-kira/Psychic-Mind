...
class UIContextOrgan:
    ...
    - Feeds predictive routing and session manager
    """
    def __init__(self):
    ...
    def _get_with_pywinauto(self) -> Optional[UIContext]:
        ...
        try:
            ...
            win = Desktop(backend="uia").window(handle=Desktop(backend="uia").get_active()[0].handle)
...
# ============================================================
#  HYBRID ROUTER (PREDICTIVE + OPERATOR-DEFINED + UI CONTEXT)
# ============================================================
...
class HybridRouter:
    ...
    def __init__(self, cpu: CPUOrgan, gpu: GPUOrgan, npu: Optional[NPUOrgan]):
        ...
        # Learned UI context preferences: context_key -> preferred organ kind
    ...
    def learn_ui_preference(self, ctx: Optional[UIContext], organ: ComputeOrgan):
        ...
        if prev is None:
            ...
            # simple reinforcement: if same organ kind repeats, keep it
            if prev != organ.kind:
                # allow mutation: occasionally switch if load suggests better organ
    ...
    def _apply_ui_context_bias(self, ctx: Optional[UIContext], base: ComputeOrgan) -> ComputeOrgan:
        ...
        if pref_kind is None:
            return base
        # If preferred kind exists and isn't overloaded, bias toward it
    ...
    def route(self, task: ComputeTask) -> ComputeOrgan:
        ...
        if pref == RoutingPreference.ALWAYS_NPU and self.npu is not None:
            ...
            organ = self._apply_ui_context_bias(task.ui_context, base)

        # Learn from this routing decision for this UI context
...
class SessionProfile:
    def __init__(self, name: str):
        ...
        # UI context frequency map: context_key -> count
...
class SessionManager:
    ...
    - Uses UI context frequencies as part of behavioral memory
...
class Organism:
    ...
    - Dual-stack UI context organ
    - Hybrid router (predictive + operator + UI context)
