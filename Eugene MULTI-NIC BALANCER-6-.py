...
# -*- coding: utf-8 -*-
"""
network_organism_soc_tier6.py

Tier-6 unified hardened network organism with:
...
- DualPersonalityBot (guardian/rogge) deeply integrated
- Per-flow anomaly tagging + attack narrative reconstruction
- Timeline event bus + SOC timeline visualization
- ETW kernel-level hooks (Windows stub, events fed into flows/timeline)
...
# ETW (Windows-only, stubbed if not available)
HAVE_ETW = False
if platform.system() == "Windows":
    try:
...
class AttackNarrative:
    """
    Very lightweight narrative builder:
    - Watches suspicious flows + threat spikes + ETW alerts
    - Builds chains like: "Suspicious DNS -> High Threat on NIC -> ETW Firewall Drop"
    """
    ...
    def ingest(self, event: dict):
        ...
        # Simple heuristic: chain suspicious flow + threat spike + ETW
        if kind in ("flow_suspicious", "threat_spike", "etw_alert"):
            with self.lock:
                self.chains.append({
                    "ts": event["ts"],
                    "kind": kind,
...
# =========================
#  PER-FLOW CLASSIFIER (BASIC)
# =========================

class FlowClassifier:
    """
    Very lightweight per-flow table:
    - Key: (src_ip, dst_ip, src_port, dst_port, proto)
    - Stats: bytes, packets, last_seen, nic, simple health/anomaly flag
    - Feeds suspicious flows into timeline + narrative
    """
    ...
    def observe(self, meta: dict):
        ...
        with self.lock:
            ...
            # Simple suspicious heuristic: high bytes + short time + unusual port
...
class GPUAnomalyBridge:
    """
    Bridge for GPU/NPU anomaly models:
    - Accepts per-NIC feature vectors
    - Uses PyTorch or CuPy if available
    - Writes anomaly scores into ThreatHeatmap
    """
    ...
    def _score_torch(self, nic_features: dict):
        ...
        for nic, feats in nic_features.items():
            ...
            # simple "autoencoder-like" anomaly: distance from mean
            mean = x.mean()
...
class SwarmMesh:
    """
    Swarm with distributed trust decay:
    - Each peer has trust score [0,1]
    - Trust decays over time
    - Good data (consistent scores) slowly increases trust
    """
    ...
    def listen_loop(self):
        ...
        while not self.stop_event.is_set():
            ...
            with self.lock:
                ...
                # trust update: if scores are "reasonable", increase slightly
...
# =========================
#  ETW HOOKS (WINDOWS STUB)
# =========================

class ETWHook:
    """
    Minimal ETW-like hook:
    - On Windows, can be extended to read firewall / security logs
    - Here we simulate / stub and feed events into timeline + narrative
    """
    def __init__(self, timeline: TimelineBus, narrative: AttackNarrative):
        self.timeline = timeline
        self.narrative = narrative
    ...
    def _loop(self):
        # Stub: periodically emit a synthetic alert
        while not self.stop_event.is_set():
            time.sleep(45)
            evt = {
                "ts": time.time(),
                "kind": "etw_alert",
                "message": "Kernel firewall drop (simulated ETW)",
                "meta": {"source": "ETW_stub", "severity": "info"},
            }
            self.timeline.emit(evt["kind"], evt["message"], evt["meta"])
            self.narrative.ingest(evt)
...
class NetworkBrain:
    def __init__(self, node_id=None, role=ROLE_WORKER,
        ...
        # ETW
        self.etw = ETWHook(self.timeline, self.narrative)
...
if HAVE_QT:
    class NetworkCockpit(QtWidgets.QWidget):
        def __init__(self, brain: NetworkBrain):
            ...
            self.setWindowTitle("Network Brain SOC Console - Tier 6")
        ...
        def _build_ui(self):
            ...
            self.lbl_sec_info = QtWidgets.QLabel(
                ...
                "- ETW: stubbed kernel alerts into timeline"
            )
