#!/usr/bin/env python3
"""
CHAMELEON SENTINEL T9/T10 — Unified Defensive Organism
...
- Tkinter Tactical Cockpit GUI (redesigned around threat posture & narrative):
    ...
    - Location monitor panel (with stability)
...
def save_reboot_memory(data: Dict[str, Any]):
    data = dict(data)
    data["threat_log"] = threat_log[-100:]
...
# ============================================================
#  LOCATION GUARDIAN (WEBSITES + GAMES + STABILITY)
# ============================================================
...
# ============================================================
#  GUI TACTICAL COCKPIT (REDESIGNED)
# ============================================================

def start_gui():
    root = tk.Tk()
    root.title("Chameleon Sentinel T10 Cockpit")
    ...
    # LEFT: Threat posture + status + timeline
    ...
    lbl_factors.grid(row=2, column=0, sticky="w")
    ...
    for i, (text, key) in enumerate(fields):
        ...
        labels[key].grid(row=i+1, column=0, sticky="w")
        values[key] = ttk.Label(left, text="")
        values[key].grid(row=i+1, column=1, sticky="w")

    sep = ttk.Separator(left, orient="horizontal")
    sep.grid(row=len(fields)+1, column=0, columnspan=2, sticky="ew", pady=(5, 3))

    lbl_timeline = ttk.Label(left, text="Threat narrative timeline:")
    lbl_timeline.grid(row=len(fields)+2, column=0, columnspan=2, sticky="w")

    timeline = tk.Listbox(left, height=14, width=80)
    timeline.grid(row=len(fields)+3, column=0, columnspan=2, sticky="nsew")
    ...
    def draw_risk_graph():
        ...
        if points:
            x,y = points[-1]
            canvas.create_oval(x-3,y-3,x+3,y+3, outline="red", fill="red")
    ...
    def refresh():
        try:
            ...
            for host, info in loc_db.get("websites", {}).items():
                ...
                ip = info.get("ip")
                label = f"{host}: ip={ip}, region={region}, changes={cc}"
            ...
            for name, info in loc_db.get("games", {}).items():
                ...
                region = info.get("region")
                ip = info.get("ip")
                label = f"{name}: ip={ip}, region={region}, changes={cc}"
                loc_list.insert(tk.END, label)
...
class SentinelAPIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/status":
            ...
            self._send_json(threat_log[-150:])
            ...
            self._send_json(get_risk_history()[-150:])
        else:
...
def main():
    ...
    # initial correlation for environment choice
