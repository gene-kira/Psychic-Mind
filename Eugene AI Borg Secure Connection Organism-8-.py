#!/usr/bin/env python3
"""
Borg Brain v5 – Sentinel Organism

New in v5:
- Volatility memory analysis integration hook (real CLI call if available)
- Neo4j threat graph integration (optional)
- GPU-accelerated threat heatmap rendering (optional, cupy)
- Auto-training feedback loop from operator decisions (allow/block) to bias scoring
...
- ML anomaly detection:
  ...
  - Deep autoencoder (PyTorch, GPU-aware, if available)
...
- Reputation:
  - Local reputation DB
  - Optional VirusTotal / AbuseIPDB integration (API keys required)
- Process lineage tracking (psutil)
- ETW / kernel telemetry bridge (JSON protocol)
- Swarm mesh (UDP gossip) for distributed threat intel
- Memory forensics hooks
- Persistent allow/block lists (JSON)
- Operator cockpit with:
  - IP lists (Allowed / Blocked / Pending)
  - Timeline panel
...
# Optional deep learning (PyTorch autoencoder)
try:
    import torch
...
# Optional GPU array for heatmap
try:
    import cupy as cp
    CUPY_AVAILABLE = True
except ImportError:
...
# Optional matplotlib for saving heatmap PNG
try:
    import matplotlib
    matplotlib.use("Agg")
...
# Optional external integrations (you must fill these if you want real calls)
VT_API_KEY = os.environ.get("BORG_VT_API_KEY", "")
ABUSEIPDB_API_KEY = os.environ.get("BORG_ABUSEIPDB_API_KEY", "")
GEOIP2_DB_PATH = os.environ.get("BORG_GEOIP2_DB", "")  # e.g. "/path/to/GeoLite2-City.mmdb"

# Neo4j config (optional)
NEO4J_URI = os.environ.get("BORG_NEO4J_URI", "")
NEO4J_USER = os.environ.get("BORG_NEO4J_USER", "")
NEO4J_PASS = os.environ.get("BORG_NEO4J_PASS", "")

# Volatility config (optional)
VOLATILITY_PATH = os.environ.get("BORG_VOLATILITY_PATH", "volatility")  # CLI name or full path
...
# ---------------------------------------------------------------------------
# List manager – persistent allow/block + runtime pending + feedback stats
# ---------------------------------------------------------------------------
...
class ListManager:
    def __init__(self, logger):
        ...
        # Feedback stats for auto-training
        self.feedback = {
    ...
    def _load(self):
        ...
        try:
            ...
            for k in ("allow", "block"):
                for ip, v in fb.get(k, {}).items():
                    self.feedback[k][ip] = int(v)
    ...
    def _save(self):
        try:
            data = {
                ...
                "feedback": {
                    "allow": dict(self.feedback["allow"]),
                    "block": dict(self.feedback["block"]),
                },
            }
    ...
    def get_feedback_bias(self, ip: str) -> float:
        """
        Auto-training feedback bias:
        - If operator often allows an IP, reduce its effective threat.
        - If operator often blocks an IP, increase its effective threat.
        """
        a = self.feedback["allow"].get(ip, 0)
        b = self.feedback["block"].get(ip, 0)
        if a == 0 and b == 0:
            return 0.0
        # Simple heuristic: block votes add +1 per vote, allow votes subtract 0.5 per vote
        return b * 1.0 - a * 0.5

# ---------------------------------------------------------------------------
# Reputation manager (local + external stubs)
# ---------------------------------------------------------------------------
...
class ReputationManager:
    ...
    def query_external(self, ip: str) -> float:
        """
        Stub for VirusTotal / AbuseIPDB integration.
        You can implement real HTTP calls here using VT_API_KEY / ABUSEIPDB_API_KEY.
        For now, we just log and return existing score.
        """
        if not VT_API_KEY and not ABUSEIPDB_API_KEY:
            return self.get_score(ip)
        self._log(f"[{now_ts()}] [Reputation] External reputation query for {ip} (VT/AbuseIPDB stub).")
        return self.get_score(ip)

# ---------------------------------------------------------------------------
# GeoIP / ASN (heuristic + optional GeoIP2)
# ---------------------------------------------------------------------------
...
class GeoIPResolver:
    def __init__(self, logger):
        self._log = logger
        self.reader = None
        if GEOIP2_AVAILABLE and GEOIP2_DB_PATH and os.path.exists(GEOIP2_DB_PATH):
            try:
                self.reader = geoip2.database.Reader(GEOIP2_DB_PATH)
                self._log(f"[{now_ts()}] [GeoIP] Using GeoIP2 DB at {GEOIP2_DB_PATH}")
            except Exception as e:
                self._log(f"[{now_ts()}] [GeoIP] Failed to open GeoIP2 DB: {e}")
                self.reader = None
        else:
            if GEOIP2_AVAILABLE:
                self._log(f"[{now_ts()}] [GeoIP] GeoIP2 available but DB path not set or missing.")
            else:
                self._log(f"[{now_ts()}] [GeoIP] GeoIP2 not available, using heuristic only.")
    ...
    def lookup(self, ip: str):
        if self.reader:
            try:
                ...
                asn = "AS-UNK"
                scope = "public"
                return {"country": country, "asn": asn, "scope": scope}
            except Exception:
                pass
        try:
...
class QueenGuardian(QObject):
    ...
    def __init__(self, registry: ConnectionRegistry, policy: LockdownPolicy,
                 ...
                 lists: ListManager, reputation: ReputationManager,
                 geoip: GeoIPResolver, parent=None):
        super().__init__(parent)
        self._registry = registry
        self._policy = policy
    ...
    def _handle_task(self, host: str, port: int, expected_fingerprint: str):
        ...
        geo_info = self._geoip.lookup(combined_ips[0]) if combined_ips else None
        rep_score = 0.0
        for ip in combined_ips:
            rep_score = max(rep_score, self._reputation.query_external(ip))
...
# ---------------------------------------------------------------------------
# Anomaly (ML + deep + heuristic + feedback bias)
# ---------------------------------------------------------------------------
...
class QueenAnomaly(QObject):
    ...
    def _heuristic_score(self, conn: dict) -> float:
        ...
        score += rep / 3.0
        # Feedback bias from operator decisions
        if host:
            score += self._lists.get_feedback_bias(host) * 0.5
        return max(score, 0.0)

    def _build_features(self, snapshot):
        feats = []
        for c in snapshot:
            ...
            feats.append([float(port), float(secure), float(rep), float(kind)])
        return feats
    ...
    def _train_autoencoder(self, features):
        if not self._use_torch or len(features) < 20:
            return
        try:
            x = torch.tensor(features, dtype=torch.float32, device=self._device)
            input_dim = x.shape[1]
            self._autoencoder = SimpleAutoencoder(input_dim).to(self._device)
            opt = torch.optim.Adam(self._autoencoder.parameters(), lr=1e-3)
            loss_fn = nn.MSELoss()
            for _ in range(50):
                opt.zero_grad()
            ...
            self._log(f"[{now_ts()}] [Anomaly] Autoencoder trained on {len(features)} samples (device={self._device}).")
        except Exception as e:
            self._log(f"[{now_ts()}] [Anomaly] Autoencoder training failed: {e}")
            self._autoencoder = None
            self._ae_trained = False

    def _ae_scores(self, snapshot):
        if not self._use_torch or not self._ae_trained or self._autoencoder is None:
            return None
        try:
            features = self._build_features(snapshot)
            x = torch.tensor(features, dtype=torch.float32, device=self._device)
    ...
    def _loop(self):
        while not self._stop_event.is_set():
            ...
            features = self._build_features(snapshot)
            if self._use_torch and not self._ae_trained and len(features) >= 20:
                self._train_autoencoder(features)

            ml_scores = self._ml_scores(snapshot)
            ae_scores = self._ae_scores(snapshot)
            ...
            for idx, conn in enumerate(snapshot):
                ...
                if ae_scores is not None:
                    ae_component = float(ae_scores[idx]) * 2.0
                    base += ae_component
                score = max(base, 0.0)
                self._registry.set_anomaly_score(idx, score)
                if score >= 4.0:
                    self._registry.add_tag(idx, "high_anomaly")
                    self._log(f"[{now_ts()}] [Anomaly] High anomaly {score:.1f} for {conn.get('kind')} "
...
class QueenThreat(QObject):
    ...
    def _compute_threat_score(self, conn: dict) -> float:
        ...
        base += rep / 2.0
        if host:
            base += self._lists.get_feedback_bias(host)
        return base
...
class QueenSniffer(QObject):
    ...
    def _handle_packet(self, pkt):
        ...
        geo_info = self._geoip.lookup(dst)
        rep_score = self._reputation.query_external(dst)
...
class QueenBridge(QObject):
    ...
    def __init__(self, registry: ConnectionRegistry, policy: LockdownPolicy,
                 ...
                 reputation: ReputationManager, geoip: GeoIPResolver, parent=None):
        super().__init__(parent)
        self._registry = registry
        self._policy = policy
        self._firewall = firewall
    ...
    def _loop(self):
        ...
        try:
            ...
            srv.bind((host, port))
            srv.listen(5)
            self._log(f"[{now_ts()}] [Bridge] Listening on {host}:{port} for driver/service/ETW events.")
        except Exception as e:
    ...
    def _handle_client(self, conn, addr):
        try:
            ...
            if msg.get("event") == "new_connection":
                ...
                geo_info = self._geoip.lookup(rip)
                rep_score = self._reputation.query_external(rip)
            ...
            elif msg.get("event") == "etw_telemetry":
                self._log(f"[{now_ts()}] [Bridge] ETW telemetry event: {msg.get('detail', '')}")
                self._timeline(f"[{now_ts()}] ETW {msg.get('detail', '')}")
            else:
...
# ---------------------------------------------------------------------------
# Memory forensics hooks (Volatility integration)
# ---------------------------------------------------------------------------

class QueenForensics(QObject):
    log_message = Signal(str)
    timeline_event = Signal(str)
    ...
    def trigger_memory_scan(self, pid: int, reason: str):
        self._log(f"[{now_ts()}] [Forensics] Memory scan requested for PID {pid} (reason={reason}).")
        self._timeline(f"[{now_ts()}] FORENSICS PID {pid} ({reason})")

    def run_volatility(self, image_path: str, profile: str = ""):
        """
        Real Volatility integration (CLI-based).
        Requires VOLATILITY_PATH to point to a valid 'volatility' executable.
        """
        cmd = [VOLATILITY_PATH]
        if profile:
            cmd += ["--profile", profile]
        cmd += ["-f", image_path, "pslist"]
        try:
            self._log(f"[{now_ts()}] [Forensics] Running Volatility: {' '.join(cmd)}")
            out = subprocess.run(cmd, capture_output=True, text=True, timeout=120)
            if out.returncode != 0:
                self._log(f"[{now_ts()}] [Forensics] Volatility error: {out.stderr[:500]}")
            else:
                snippet = out.stdout[:1000]
                self._log(f"[{now_ts()}] [Forensics] Volatility output (truncated):\n{snippet}")
        except Exception as e:
            self._log(f"[{now_ts()}] [Forensics] Failed to run Volatility: {e}")

# ---------------------------------------------------------------------------
# Neo4j threat graph integration
# ---------------------------------------------------------------------------

class ThreatGraphNeo4j(QObject):
    log_message = Signal(str)

    def __init__(self, parent=None):
        super().__init__(parent)
        self.driver = None
        if NEO4J_AVAILABLE and NEO4J_URI and NEO4J_USER and NEO4J_PASS:
            try:
                self.driver = GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASS))
                self._log(f"[{now_ts()}] [Neo4j] Connected to Neo4j at {NEO4J_URI}")
            except Exception as e:
                self._log(f"[{now_ts()}] [Neo4j] Failed to connect: {e}")
                self.driver = None
        else:
            self._log(f"[{now_ts()}] [Neo4j] Neo4j not configured or driver missing.")
    ...
    def push_connection(self, process_name: str, pid: int, ip: str, threat_score: float):
        if not self.driver:
            return
        try:
            with self.driver.session() as session:
                session.execute_write(
                    ...
                    process_name, pid, ip, threat_score
                )
        except Exception as e:
            self._log(f"[{now_ts()}] [Neo4j] Failed to push connection: {e}")

    @staticmethod
    def _create_relationship(tx, process_name, pid, ip, threat_score):
        tx.run(
            """
            MERGE (p:Process {name: $name, pid: $pid})
            MERGE (h:Host {ip: $ip})
            MERGE (p)-[r:CONNECTS_TO]->(h)
            SET r.threat_score = $score, r.last_seen = timestamp()
            """,
...
class GPUHeatmapEngine:
    def __init__(self, logger):
        self._log = logger
        self.gpu = CUPY_AVAILABLE
        if self.gpu:
            self._log(f"[{now_ts()}] [HeatmapGPU] Using CuPy for GPU heatmap.")
        else:
            self._log(f"[{now_ts()}] [HeatmapGPU] CuPy not available, using CPU fallback.")
        if not MPL_AVAILABLE:
            self._log(f"[{now_ts()}] [HeatmapGPU] Matplotlib not available, PNG export disabled.")

    def render_heatmap_png(self, scores_by_ip: dict, path: str = "borg_heatmap.png"):
        if not MPL_AVAILABLE:
            self._log(f"[{now_ts()}] [HeatmapGPU] Cannot render heatmap – matplotlib missing.")
            return
        if not scores_by_ip:
            self._log(f"[{now_ts()}] [HeatmapGPU] No data for heatmap.")
            return
        values = list(scores_by_ip.values())
        n = len(values)
        size = int(max(4, (n ** 0.5)))
        # Pad values to square
        padded = values + [0.0] * (size * size - len(values))
        if self.gpu:
            arr = cp.array(padded, dtype=cp.float32).reshape(size, size)
            data = cp.asnumpy(arr)
        else:
            import numpy as np
            data = np.array(padded, dtype=float).reshape(size, size)
        plt.figure(figsize=(4, 4))
        plt.imshow(data, cmap="inferno")
        plt.colorbar(label="Threat Score")
        plt.title("Borg Threat Heatmap")
        plt.tight_layout()
        try:
            plt.savefig(path, dpi=150)
            self._log(f"[{now_ts()}] [HeatmapGPU] Saved heatmap PNG to {path}")
        except Exception as e:
            self._log(f"[{now_ts()}] [HeatmapGPU] Failed to save heatmap PNG: {e}")
        finally:
            plt.close()
...
class SecureCockpit(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Borg Brain v5 – Sentinel Cockpit")
        ...
        self.geoip = GeoIPResolver(self._lists_log)
        ...
        self.sniffer = QueenSniffer(self.registry, self.firewall, self.list_manager, self.reputation, self.geoip)
        self.bridge = QueenBridge(self.registry, self.policy, self.firewall, self.list_manager, self.reputation, self.geoip)
        self.forensics = QueenForensics()
        self.neo4j_graph = ThreatGraphNeo4j()
        self.gpu_heatmap = GPUHeatmapEngine(self._heatmap_log)
        ...
        self.forensics.log_message.connect(self.append_log)
        self.forensics.timeline_event.connect(self.append_timeline)

        self.neo4j_graph.log_message.connect(self.append_log)
        ...
        self._graph_timer.timeout.connect(self.refresh_threat_graph)
        self._graph_timer.start(7000)

        self._tree_timer = QTimer(self)
        self._tree_timer.timeout.connect(self.refresh_process_tree)
        self._tree_timer.start(8000)

        self._neo4j_timer = QTimer(self)
        self._neo4j_timer.timeout.connect(self.push_graph_to_neo4j)
        self._neo4j_timer.start(15000)
    ...
    def _build_ui(self):
        ...
        title_label = QLabel("BORG BRAIN v5 – SENTINEL COCKPIT")
        ...
        self.btn_export_heatmap = QPushButton("Export Heatmap PNG (GPU if available)")
        self.btn_export_heatmap.clicked.connect(self.on_export_heatmap_png)
        heatmap_layout.addWidget(heatmap_label)
        heatmap_layout.addWidget(self.heatmap_view)
        heatmap_layout.addWidget(self.btn_export_heatmap)
        tabs.addTab(heatmap_tab, "Threat Heatmap")

        # Threat graph tab (textual graph)
        graph_tab = QWidget()
        graph_layout = QVBoxLayout(graph_tab)
        graph_layout.setContentsMargins(10, 10, 10, 10)
        graph_layout.setSpacing(8)
        graph_label = QLabel("Threat Graph – Process → IP → Score")
        self.graph_view = QTextEdit()
        self.graph_view.setReadOnly(True)
        graph_layout.addWidget(graph_label)
        graph_layout.addWidget(self.graph_view)
        tabs.addTab(graph_tab, "Threat Graph")
        ...
        tree_label = QLabel("Process Tree – Lineage of Observed PIDs")
        self.tree_view = QTextEdit()
        self.tree_view.setReadOnly(True)
        tree_layout.addWidget(tree_label)
        tree_layout.addWidget(self.tree_view)
        tabs.addTab(tree_tab, "Process Tree")
    ...
    def refresh_threat_graph(self):
        ...
        for c in snapshot:
            ...
            if not ip:
                continue
            edges.append((proc, ip, score))
        lines = ["Process → IP [ThreatScore]", "---------------------------"]
        for proc, ip, score in sorted(edges, key=lambda e: e[2], reverse=True)[:50]:
            lines.append(f"{proc}  →  {ip}  [{score:.2f}]")
    ...
    def refresh_process_tree(self):
        ...
        for c in snapshot:
            ...
            if pid and lineage:
                seen[pid] = lineage
        lines = ["Observed Process Trees", "----------------------"]
        for pid, lineage in seen.items():
            lines.append(lineage)
        self.tree_view.setPlainText("\n".join(lines))

    @Slot()
    def on_export_heatmap_png(self):
        snapshot = self.registry.snapshot()
        scores_by_ip = defaultdict(float)
        for c in snapshot:
            ip = c.get("host")
            if not ip:
                continue
            scores_by_ip[ip] = max(scores_by_ip[ip], c.get("threat_score", 0.0))
        self.gpu_heatmap.render_heatmap_png(scores_by_ip, path="borg_heatmap.png")

    @Slot()
    def push_graph_to_neo4j(self):
        snapshot = self.registry.snapshot()
        for c in snapshot:
            proc = c.get("process_name") or "unknown"
            pid = c.get("pid") or 0
            ...
            if score < 5.0:
                continue
            self.neo4j_graph.push_connection(proc, pid, ip, score)
