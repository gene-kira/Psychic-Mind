...
REQUIRED_LIBS = ["pywin32", "psutil", "torch"]
...
# =========================
# LOW-LEVEL HOOKS (SAFE INTROSPECTION)
# =========================

def scan_processes():
    """
    Real process scan using psutil.
    Returns list of dicts: {pid, name, cpu, exe, suspicious}
    """
    results = []
    ...
    for p in psutil.process_iter(attrs=["pid", "name", "cpu_percent", "exe"]):
        ...
        exe = info.get("exe") or ""
        suspicious = False
        # simple heuristic: high CPU and non-system path
...
def listen_etw_stub():
    ...
    Real ETW requires specialized libs; here we simulate an empty list.
    """
    return []

def kernel_sensors_stub():
    """
    Kernel sensor placeholder.
    In a real system this might read safe counters or logs, not hook the kernel.
    """
    return {
        "context_switches": 0,
        "syscalls": 0,
...
def compute_anomaly_score_cpu(features, lockdown, autonomous):
    """
    Simple CPU-based scoring with non-linear weighting.
    """
    ...
    # non-linear bump for many outbound connections
...
def compute_anomaly_score_gpu(features, lockdown, autonomous):
    """
    Optional GPU/NPU scoring using PyTorch.
    This is a tiny linear model just to demonstrate acceleration.
    """
    ...
    # simple learned-like weights (hand-tuned)
    ...
    score = (x * w).sum() + b
    score = score + torch.sqrt(torch.clamp(x[1], min=0)) * 5.0  # outbound sqrt bump
...
# =========================
# THREAT NARRATIVE + SWARM CONSENSUS (SAFE, HIGH-LEVEL)
# =========================

def build_threat_narrative(timeline, anomaly_score):
    """
    Build a simple narrative from timeline + score.
    """
    if not timeline:
    ...
    if anomaly_score > 80:
        summary = "High-risk sequence detected; potential attack chain forming."
    elif anomaly_score > 40:
        summary = "Elevated activity; monitor closely."
    else:
        summary = "Normal background activity."
    return {"summary": summary, "steps": steps}

def swarm_consensus(nodes_scores):
    """
    Simple swarm consensus: average + max.
    """
    if not nodes_scores:
...
# =========================
# MINI-SENTINEL HOOKS (NOW USING REAL DATA + NARRATIVE + SWARM)
# =========================

def hook_collect_timeline(lockdown, autonomous,
                          proc_list, net_list, reg_list, etw_events, kernel_data, anomaly_score):
...
def hook_collect_threat_matrix(lockdown, autonomous, anomaly_score):
    ...
    consensus = swarm_consensus(nodes)
    # embed consensus as a pseudo-node
    nodes.append({"node": "Consensus", "score": consensus["consensus_score"]})
    return nodes
...
def hook_collect_node_sync(lockdown, autonomous):
    ...
    if lockdown:
        status = "lockdown"
    return [
        {"node": "LocalNode", "status": status}
    ]

def hook_collect_logs(lockdown, autonomous, anomaly_score):
    now = datetime.utcnow().isoformat() + "Z"
...
def hook_collect_persona(lockdown, autonomous, anomaly_score):
    if anomaly_score > 80:
        mode = "Hunter"
...
def hook_emit_events(lockdown, autonomous, anomaly_score):
    now = datetime.utcnow().isoformat() + "Z"
    ...
    def run(self):
        while self.running:
            ...
            timeline = hook_collect_timeline(lockdown, autonomous,
                                             ...
                                             etw_events, kernel_data, anomaly_score)
            ...
            narrative = build_threat_narrative(timeline, anomaly_score)
