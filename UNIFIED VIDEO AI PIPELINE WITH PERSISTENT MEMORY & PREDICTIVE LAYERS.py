"""
UNIFIED VIDEO AI PIPELINE (RESILIENT, PREDICTIVE, PERSISTENT)

Features:
- Autoloader with GPU awareness
...
def detect_cuda():
    try:
        ...
        print(f"[GPU] Torch not available or error: {e}. Using CPU.")
        return "cpu"
...
class YOLOv8Detector(BaseDetector):
    ...
    def _fallback_to_cpu(self):
        if self.device == "cuda":
            print("[GPU WATCHDOG] CUDA error detected. Falling back to CPU.")
            self.device = "cpu"
            self._build_model()
    ...
    def detect(self, frame):
        try:
            ...
            msg = str(e)
            print(f"[DETECTOR] YOLO detect error: {msg}")
