...
+ Behavioral State Classifier (CALM/FOCUSED/ALTERED/SUPPRESSED)
+ Missing-detail Anomalies
+ Predictive "data physics" style stress/entropy
+ Abstract Network / Remote-Control Presence Detection (no IP logging)
+ Behavioral Threat Scan (non-destructive, no identifiers)
...
class POVUnifiedSecurityCore:
    def __init__(self, root, node_id="NODE_LOCAL", role="WORKSTATION"):
        ...
        # High-stress tracking for "missing recovery"
        self.high_stress_start = None

        # Integrity / self-check state
    ...
    def threat_scan_behavioral(self):
        """
        Non-destructive threat scan:
        - Counts listening ports (no IP/port details).
        - Counts processes (no names, no PIDs).
        - Flags unusual density as behavioral anomalies.
        """
        try:
            try:
                conns = psutil.net_connections(kind="inet")
    ...
    def self_check_integrity(self):
        ...
        - Ensures key components are callable/present.
        - Does NOT modify code or rewrite itself.
        """
        try:
    ...
    def build_codex_status(self, stress, entropy, behavioral_state, anomaly_flags, alert_text, actions_text):
        if stress < 0.3 and entropy < 0.3:
            health_state = "OK"
            threat_level = "LOW"
            confidence = 0.9
    ...
    def log_attacker_pattern(self, stress, entropy, anomaly_flags):
        ...
        if "NET_FOREIGN" in anomaly_flags:
            pattern_parts.append("NET_FOREIGN")
        ...
        if "MANY_LISTENING_PORTS" in anomaly_flags:
            pattern_parts.append("MANY_LISTENING_PORTS")
        if "MANY_PROCESSES" in anomaly_flags:
            pattern_parts.append("MANY_PROCESSES")
        if "MISSING_DISK_ACTIVITY" in anomaly_flags:
            pattern_parts.append("MISSING_DISK_ACTIVITY")
        if "MISSING_ENTROPY_CORRELATION" in anomaly_flags:
            pattern_parts.append("MISSING_ENTROPY_CORRELATION")
        if "MISSING_STRESS_CORRELATION" in anomaly_flags:
            pattern_parts.append("MISSING_STRESS_CORRELATION")
        if "MISSING_RECOVERY" in anomaly_flags:
            pattern_parts.append("MISSING_RECOVERY")
