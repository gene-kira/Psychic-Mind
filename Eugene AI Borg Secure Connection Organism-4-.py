#!/usr/bin/env python3
"""
AI Borg Secure Connection Organism – Tiered Queens + Sniffer + Anomaly + Swarm + Bridge
Single unified file.
...
- PySide6 cockpit UI
- Queens:
    * QueenGuardian: Global TLS Guardian (all Borg connections go through here)
    ...
    * QueenSniffer: Wireshark-style passive packet capture (Scapy)
    * QueenAnomaly: Simple anomaly scoring on observed flows
    * QueenSwarm: Stub for peer sync / swarm mesh
    * QueenBridge: Stub server for driver/service bridge (future kernel interceptor)
...
- Autonomous behavior:
    ...
    * Anomaly engine scores flows and tags suspicious ones
...
# Optional: Scapy for packet sniffing
try:
...
# Optional: psutil for process attribution
try:
    import psutil
...
# ---------------------------------------------------------------------------
# Connection registry (for scanner, resolver, lockdown, sniffer, anomaly)
# ---------------------------------------------------------------------------
...
class ConnectionRegistry:
    ...
    Stores:
        - kind: "active" (Borg-initiated), "sniff" (passive), "bridge" (driver/service)
        - host, port
        - ips
        - secure (True/False/None)
        - reason
        - fingerprint
        - process_name, pid (best-effort)
        - anomaly_score
...
# ---------------------------------------------------------------------------
# Process attribution helper (best-effort, user-mode)
# ---------------------------------------------------------------------------

def find_process_for_connection(remote_ip, remote_port):
    """
    Best-effort mapping of remote_ip:remote_port to a process using psutil.
    This is not perfect, but gives some attribution.
    """
    ...
    def _loop(self):
        while not self._stop_event.is_set():
            ...
            for idx, conn in enumerate(snapshot):
                # Only enforce on active Borg connections, not passive sniffed flows
...
# ---------------------------------------------------------------------------
# QueenAnomaly – simple anomaly scoring on flows
# ---------------------------------------------------------------------------
...
class QueenAnomaly(QObject):
    ...
    def _score_connection(self, conn: dict) -> float:
        ...
        # Unknown or non-secure active connection
        if conn.get("kind") == "active":
        ...
        # Sniffed flows to unusual ports
        ...
        if conn.get("process_name") is None and conn.get("kind") != "sniff":
            score += 1.0
...
# ---------------------------------------------------------------------------
# QueenSwarm – stub for swarm mesh / peer sync
# ---------------------------------------------------------------------------
...
class QueenSwarm(QObject):
    ...
    def __init__(self, registry: ConnectionRegistry, parent=None):
        super().__init__(parent)
        self._registry = registry
        # Stub: in a real swarm, you’d sync threat intel with peers.
    ...
    def export_threats(self):
        """
        Example: export high-anomaly connections as JSON for peers.
        """
        snapshot = self._registry.snapshot()
        threats = []
        for c in snapshot:
            if c.get("anomaly_score", 0.0) >= 4.0:
                threats.append({
                    "host": c.get("host"),
                    "port": c.get("port"),
                    "reason": c.get("reason"),
        ...
        return json.dumps({"threats": threats}, indent=2)


# ---------------------------------------------------------------------------
# QueenSniffer – Wireshark-style passive packet capture
# ---------------------------------------------------------------------------
...
class QueenSniffer(QObject):
    ...
    def _handle_packet(self, pkt):
        ...
        # Process attribution (best-effort for outbound)
        proc_name, pid = (None, None)
        if src.startswith("192.") or src.startswith("10.") or src.startswith("172."):
            # Likely local; try map remote side
            proc_name, pid = find_process_for_connection(dst, dport or 0)

        # Feed into registry as passive observation
        self._registry.add_connection({
            "kind": "sniff",
            "host": dst,
            "port": dport or 0,
    ...
    def _loop(self):
        self._log("[Sniffer] Starting packet capture (Wireshark-style).")
        try:
...
# ---------------------------------------------------------------------------
# QueenBridge – stub server for driver/service bridge
# ---------------------------------------------------------------------------
...
class QueenBridge(QObject):
    ...
    def _loop(self):
        """
        Simple TCP JSON protocol on localhost:55555 for future driver/service.
        ...
        Currently: logs and stores, but does not enforce at kernel level.
        """
        host = "127.0.0.1"
        port = 55555
        try:
    ...
    def _handle_client(self, conn, addr):
        try:
            ...
            try:
                ...
                self._log("[Bridge] Received invalid JSON from client.")
                return
            ...
            if msg.get("event") == "new_connection":
                ...
                # Simple policy: if lockdown enabled and port not allowed → block
                decision = "allow"
                reason = "ok"
                if self._policy.is_enabled() and rport not in self._policy.allowed_ports:
                    decision = "block"
                ...
                self._log("[Bridge] Unknown event type from client.")
        except Exception as e:
...
class SecureCockpit(QMainWindow):
    def __init__(self):
        ...
        self.anomaly = QueenAnomaly(self.registry)
        self.swarm = QueenSwarm(self.registry)
        self.sniffer = QueenSniffer(self.registry)
        self.bridge = QueenBridge(self.registry, self.policy)
    ...
    def _build_ui(self):
        ...
        log_label = QLabel("Event Log (Guardians + Queens + Sniffer + Bridge):")
        self.log_view = QTextEdit()
        self.log_view.setReadOnly(True)

        self.status_summary = QLabel("Connections: 0 | Secure: 0 | Insecure: 0 | Observed: 0 | High Anomaly: 0")
    ...
    def refresh_status_summary(self):
        ...
        self.status_summary.setText(
            f"Connections: {total} | Secure: {secure} | Insecure: {insecure} | Observed: {observed} | High Anomaly: {high_anom}"
        )
