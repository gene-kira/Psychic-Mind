...
- File & Process Telemetry organs (cross-platform)
- Qt/PySide6 cockpit: Atlas, Peers, Prediction, Mode, Backoff, Files, Processes
"""

# ============================================================
#  ELEVATION (Windows) + AUTOLOADER
# ============================================================

import os, sys, platform, ctypes, importlib, subprocess
...
DEFAULT_TRUST_BUNDLE = "borg_trust_bundle_v4.json"
...
def create_default_trust_bundle(path: str):
    ...
    bundle = {
        ...
        "settings": {
            ...
            "max_peers": 32,
            "swarm_key": swarm_key,
            "predictor_path": "borg_predictor_v4.json",
            "crypto_mode": "auto",
            "status_port": 8053,
            "file_watch_paths": [],
        }
...
def sign_update(node_id: str, private_key_hex: str, update_payload: dict) -> dict:
    body = {"node_id": node_id, "timestamp": time.time(), "payload": update_payload}
...
def verify_update(peer_registry: PeerRegistry, reputation: PeerReputation, body: dict) -> Optional[dict]:
    ...
    if not peer or not reputation.is_trusted(node_id):
        return None
    msg = json.dumps({"node_id": node_id, "timestamp": ts, "payload": payload}, sort_keys=True).encode("utf-8")
...
class PredictiveDomainTracker:
    def __init__(self, path="borg_predictor_v4.json", max_history: int = 5000):
        self.path = path
    ...
    def _save(self):
        try:
            ...
            with open(self.path, "w", encoding="utf-8") as f:
                json.dump({"counts_global": cg, "counts_by_hour": ch, "counts_by_dow": cd}, f)
        except Exception:
            pass
...
class QueenRouter:
    def __init__(self, node_id: str, private_key_hex: str,
                 ...
                 predictor_path: str = "borg_predictor_v4.json",
        ...
        with self._lock:
            ...
            for c in candidates:
                ...
                rc = RouteCandidate(
                    ...
                    last_latency_ms=latency, last_checked=last_checked,
                    healthy=healthy, source=source
                )
...
# ============================================================
#  UI AUTOMATION WATCHER (OPTIONAL)
# ============================================================
...
# ============================================================
#  FILE TELEMETRY ORGAN (CROSS-PLATFORM)
# ============================================================
...
class _FileEventHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        if not event.is_directory:
            self.parent._record_event(event.src_path, "created")
    def on_modified(self, event):
        if not event.is_directory:
            self.parent._record_event(event.src_path, "modified")

class FileTelemetryOrgan:
    def __init__(self, paths: List[str], max_events: int = 1000):
        self.paths = paths
    ...
    def stop(self):
        ...
        if self._observer:
            try:
                ...
                pass
    def _record_event(self, path: str, kind: str):
        try:
            st = os.stat(path)
            size = st.st_size
            mtime = st.st_mtime
...
# ============================================================
#  PROCESS TELEMETRY ORGAN (CROSS-PLATFORM)
# ============================================================

class ProcessTelemetryOrgan:
    ...
    def _loop(self):
        while self._running:
            procs = []
            try:
                for p in psutil.process_iter(["pid", "ppid", "name", "exe", "cmdline", "connections"]):
                    try:
                        info = p.info
...
class StatusHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/atlas":
            ...
            files = self.server.organism.get("files")
            data = files.snapshot() if files else []
        elif self.path == "/processes":
            procs = self.server.organism.get("processes")
            data = procs.snapshot() if procs else []
        else:
...
def run_organism():
    ...
    queen = QueenRouter(
        ...
        predictor_path=bundle["settings"].get("predictor_path", "borg_predictor_v4.json"),
    ...
    dns_server.start()
    file_paths = bundle["settings"].get("file_watch_paths") or []
    files_organ = FileTelemetryOrgan(file_paths, max_events=1000)
    files_organ.start()
    procs_organ = ProcessTelemetryOrgan(interval_sec=3)
    procs_organ.start()
    organism = {
        "queen": queen,
        "peer_registry": bundle["peer_registry"],
...
def run_cockpit():
    ...
    class FilesPanel(QWidget):
        def __init__(self, parent=None):
            ...
            layout = QVBoxLayout(self)
            self.table = QTableWidget(0, 6)
            self.table.setHorizontalHeaderLabels(["Path", "Kind", "Size", "Hash", "Modified", "Seen"])
            self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
            self.table.verticalHeader().setVisible(False)
            self.table.setShowGrid(True)
            self.table.setSelectionMode(self.table.NoSelection)
            self.table.setEditTriggers(self.table.NoEditTriggers)
            layout.addWidget(self.table)
        def update_data(self, files: list):
            if files is None:
                self.table.setRowCount(0)
                return
            files = sorted(files, key=lambda e: e.get("ts", 0), reverse=True)
            self.table.setRowCount(len(files))
            for row, e in enumerate(files):
                path = e.get("path") or "-"
                kind = e.get("kind") or "-"
                size = e.get("size")
                h = e.get("hash") or "-"
                ...
                mtime_str = time.ctime(mtime) if mtime else "-"
                ts_str = time.ctime(ts) if ts else "-"
                self.table.setItem(row, 0, color_item(path, QColor(200, 200, 200)))
                self.table.setItem(row, 1, color_item(kind, QColor(180, 180, 220)))
                self.table.setItem(row, 2, color_item(size_str, QColor(180, 180, 180)))
                self.table.setItem(row, 3, color_item(h[:16] + "…" if len(h) > 16 else h, QColor(160, 220, 160)))
                self.table.setItem(row, 4, color_item(mtime_str, QColor(160, 160, 160)))
                self.table.setItem(row, 5, color_item(ts_str, QColor(160, 160, 160)))
    ...
    class ProcessesPanel(QWidget):
        def __init__(self, parent=None):
            ...
            layout = QVBoxLayout(self)
            self.table = QTableWidget(0, 6)
            self.table.setHorizontalHeaderLabels(["PID", "PPID", "Name", "Exe", "Cmdline", "Remotes"])
            self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
            self.table.verticalHeader().setVisible(False)
            self.table.setShowGrid(True)
            self.table.setSelectionMode(self.table.NoSelection)
            self.table.setEditTriggers(self.table.NoEditTriggers)
            layout.addWidget(self.table)
        def update_data(self, procs: list):
            if procs is None:
                self.table.setRowCount(0)
                return
            procs = sorted(procs, key=lambda p: p.get("pid") or 0)
            ...
            for row, p in enumerate(procs):
                ...
                remotes = ", ".join(p.get("remotes") or [])
                self.table.setItem(row, 0, color_item(str(pid), QColor(200, 200, 200)))
                self.table.setItem(row, 1, color_item(str(ppid), QColor(160, 160, 160)))
                self.table.setItem(row, 2, color_item(name, QColor(200, 200, 200)))
                self.table.setItem(row, 3, color_item(exe, QColor(160, 200, 220)))
                self.table.setItem(row, 4, color_item(cmd, QColor(180, 180, 180)))
                self.table.setItem(row, 5, color_item(remotes, QColor(200, 180, 140)))

    class CockpitWindow(QWidget):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("Borg Defensive Cockpit v4")
            ...
            top_layout.addWidget(atlas_box, 3)
            top_layout.addWidget(peers_box, 3)
            top_layout.addWidget(pred_box, 3)
            top_layout.addWidget(mode_box, 2)
            files_box = make_group("File Telemetry")
            files_layout = QVBoxLayout(files_box)
            self.files_panel = FilesPanel()
            files_layout.addWidget(self.files_panel)
            procs_box = make_group("Process Telemetry")
            procs_layout = QVBoxLayout(procs_box)
            self.procs_panel = ProcessesPanel()
            procs_layout.addWidget(self.procs_panel)
            bottom_layout.addWidget(files_box, 3)
            bottom_layout.addWidget(procs_box, 3)
            ...
            self.timer_files = QTimer(self)
            self.timer_files.timeout.connect(self.refresh_files)
            self.timer_files.start(4000)
            self.timer_procs = QTimer(self)
            self.timer_procs.timeout.connect(self.refresh_processes)
        ...
        def refresh_files(self):
            data = safe_get("/files")
            self.files_panel.update_data(data or [])
        def refresh_processes(self):
            data = safe_get("/processes")
            self.procs_panel.update_data(data or [])
