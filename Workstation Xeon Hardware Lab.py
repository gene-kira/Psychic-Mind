...
"""
Workstation / Xeon Hardware Lab v3.0 (Unified)
...
- Timeline visualization (event log)
...
BOARD_DB: Dict[str, Dict[str, Any]] = {
    "ASUS WS C422 PRO": {
        ...
        "vendor": "ASUSTeK",
        "product": "WS C422 PRO",
        "vrm_current_limit": 220,  # amps (heuristic)
...
class RuleEngine:
    def evaluate(
        ...
        # Chipset heuristic
        supported = cpu.get("supported_chipsets", [])
        chipset = board.get("chipset")
        if supported and chipset and chipset not in supported:
            result["score"] -= 20
            result["warnings"].append("Chipset not in CPU's preferred list (heuristic)")
        ...
        # VRM modeling (very rough heuristic)
...
class Simulator:
    ...
    def run(
        ...
        result = self.engine.evaluate(cpu, board, gpu, mem)
...
class HardwareLab(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Workstation / Xeon Hardware Lab v3.0")
    ...
    def showEvent(self, event: QtGui.QShowEvent) -> None:
        ...
        # Ensure all widgets exist before any refresh calls
    ...
    def run_simulation(self):
        ...
        report = self.sim.run(cpu_name, board_name, gpu_name, mem_name)
        self.report_box.setText(json.dumps(report, indent=4))

        advice = recommend(self.db.cpu_db[cpu_name], self.workload_select.currentText())
        ...
        self._log_event(
            ...
            f"MEM={mem_name}, Score={report['score']}"
        )
        self.refresh_inventory_tab()
