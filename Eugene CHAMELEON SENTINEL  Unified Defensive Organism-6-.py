#!/usr/bin/env python3
"""
CHAMELEON SENTINEL T11 — Unified Defensive Organism
...
- Threat Narrative Engine (event chains + summaries)
- Entity Profiles (websites/games: stability, trust, volatility)
- Threat Prediction (short-horizon risk/posture forecast)
- Tkinter Tactical Cockpit GUI with themes:
    ...
    - Advanced worker swarm visualization
...
def save_reboot_memory(data: Dict[str, Any]):
    data = dict(data)
    data["threat_log"] = threat_log[-150:]
...
class ThreatNarrativeEngine:
    """
    Builds short narrative summaries from recent events and correlation.
    """
    def __init__(self):
        self.last_summary = ""
    ...
    def build_summary(self, corr: Dict[str, Any]) -> str:
        ...
        recent = threat_log[-40:]
        auth_fail = any(e["kind"] == "auth" and "Invalid" in e["detail"] for e in recent)
        loc_warn = any(e["kind"] == "location" and e["level"] == "warn" for e in recent)
        state_alert = any(e["kind"] == "state" and e["level"] == "alert" for e in recent)

        parts = [f"Posture {posture} ({idx:.1f})."]

        if factors:
            parts.append("Drivers: " + ", ".join(factors) + ".")

        if auth_fail:
            parts.append("Recent authentication failures detected.")
        if loc_warn:
            parts.append("Location changes observed for sites/games.")
        if state_alert:
            parts.append("Guardian altered state was triggered recently.")

        if not (auth_fail or loc_warn or state_alert):
...
# ============================================================
#  THREAT PREDICTION (SHORT HORIZON)
# ============================================================

class ThreatPredictor:
    """
    Simple short-horizon prediction based on recent risk/threat trends.
    """
    def predict(self) -> Dict[str, Any]:
        hist = get_risk_history()[-40:]
        if len(hist) < 3:
        ...
        predicted_index = max(0.0, min(idxs[-1] + di * 0.5, 100.0))
...
# ============================================================
#  LOCATION GUARDIAN + ENTITY PROFILES
# ============================================================
...
# ============================================================
#  GUI TACTICAL COCKPIT (THEMES + ADVANCED SWARM VIS)
# ============================================================
...
def start_gui():
    ...
    root = tk.Tk()
    root.title("Chameleon Sentinel T11 Cockpit")
    ...
    lbl_narrative = ttk.Label(narrative_frame, text="No narrative yet.")
    ...
    # RIGHT: heatmap + risk graph + swarm + location
    ...
    canvas = tk.Canvas(graph_frame, height=150, bg="black", highlightthickness=0)
    canvas.grid(row=0, column=0, sticky="nsew")

    # Worker swarm visualization (advanced)
    ...
    def draw_swarm():
        ...
        for i, worker in enumerate(workers):
            angle = 2 * 3.14159 * i / n
            wx = cx + int(radius * 0.8 * (0.9 * (i % 2) + 0.1) * (1 if i % 4 < 2 else -1))
            wy = cy + int(radius * 0.8 * (1 if i % 3 == 0 else -1))
            color = theme["accent"]
...
class SentinelAPIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/status":
            ...
            self._send_json(threat_log[-200:])
            ...
            self._send_json(get_risk_history()[-200:])
        else:
