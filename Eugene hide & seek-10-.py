# =========================
# AUTO-ELEVATION + BOOTSTRAP LOGGING
# =========================
...
ELEVATION_LOG_FILE = "sentinel_elevation.log"
SILENT_ELEVATE = os.environ.get("SENTINEL_SILENT_ELEVATE", "0") == "1"

def _init_bootstrap_logging():
    try:
        logging.basicConfig(
            filename=ELEVATION_LOG_FILE,
            level=logging.INFO,
            format="%(asctime)s [BOOT] %(levelname)s %(message)s"
        )
        logging.info("Bootstrap logging initialized.")
    except Exception:
        # Last-resort: ignore logging failures
        pass

_init_bootstrap_logging()

def _log_boot(msg, level="info"):
    try:
        if level == "error":
            logging.error(msg)
        elif level == "warning":
...
def ensure_admin():
    """
    Ensure the process is running with administrative privileges.
    - Uses ShellExecuteW with 'runas' verb.
    - Supports both script and frozen EXE.
    - Logs to file + optional console (silent mode via SENTINEL_SILENT_ELEVATE=1).
    """
    try:
        if ctypes.windll.shell32.IsUserAnAdmin():
            _log_boot("[Codex Sentinel] Already running with administrative privileges.")
            return

        _log_boot("[Codex Sentinel] Elevation required. Relaunching with admin rights...")

        # Build parameters
        params = " ".join([f'"{arg}"' for arg in sys.argv[1:]])

        # If frozen (PyInstaller/EXE), elevate the EXE directly
        if getattr(sys, "frozen", False):
            exe_path = os.path.abspath(sys.executable)
            _log_boot(f"[Codex Sentinel] Detected frozen executable: {exe_path}")
            ...
            # Normal script: elevate Python with script path
            script = os.path.abspath(sys.argv[0])
            _log_boot(f"[Codex Sentinel] Elevating script: {script}")
            ctypes.windll.shell32.ShellExecuteW(
                None,
                "runas",
                sys.executable,
        ...
        _log_boot(f"[Codex Sentinel] Elevation failed: {e}", level="error")
        sys.exit(1)

ensure_admin()

# =========================
# ORIGINAL ORGANISM IMPORTS AND CONFIG
# =========================
...
def swarm_consensus(nodes_scores):
    ...
    # behavioral negotiation: majority weighted vote
    lockdown_votes = []
    auto_votes = []
...
class PersonaRL:
    ...
    def update(self, risk, altered_state, anomaly_score, chain_findings, chosen_mode):
        ...
        if s not in self.q:
            self.q[s] = {m: 0.0 for m in self.MODES}
        # heuristic reward: high anomaly + chains => Hunter/Guardian good, Observer bad
