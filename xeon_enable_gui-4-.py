...
# -*- coding: utf-8 -*-
"""
Firmware Policy Organ: Xeon-enable command generator cockpit (v3)

- No direct NVRAM/firmware writes.
- Smarter: CPU/board detection via cpuinfo, WMI (Windows), dmidecode (Linux/macOS), fingerprint scoring.
- Operator-grade: dark cockpit, status bar, log, method preview, danger meter, matrix panel, profile builder.
- Autonomous: validation, basic profile diffing, profile import/export, suspicious-offset warnings.
- More complete: multi-method schema, Xeon compatibility matrix, board fingerprints.
- Integrated: class-based organ with hooks (external config, logging, status callbacks) for embedding.
"""
...
class FirmwarePolicyOrgan:
    """
    Integration hooks:
      - external_config: pass a dict to override internal config.
      - on_log: callable(str) to receive log lines.
      - on_status: callable(str) to receive status updates.
    """

    def __init__(self, root=None, external_config=None, on_log=None, on_status=None):
        self._own_root = False
        if root is None:
            root = tk.Tk()
            self._own_root = True
        self.root = root
        self.root.title("Firmware Policy Organ – Xeon Enable Cockpit (v3)")
    ...
    def _build_layout(self):
        ...
        self.method_combo.bind("<<ComboboxSelected>>", lambda e: self._update_method_preview())
