#!/usr/bin/env python3
"""
neurofabric_tier7_unified_all_upgrades.py

Tier-7 NeuroFabric organism with:

- Tiered storage (NVMe/Optane-aware) + hybrid auto-blacklist (WinError-safe)
- Auto-port FastAPI backend (no 8080 conflicts)
- Tkinter Tactical Cockpit (always-on, stable)
- Optional PySide6 GPU-ish cockpit (if installed)
...
- Autoencoder anomaly model (PyTorch) + real training loop scaffold
...
- NPU acceleration scaffold (Intel/Qualcomm-style hook)
- Heat-based migration daemon (KV + vector hot/cold)
...
# Optional PyTorch Geometric (real GNN)
try:
...
# ---------------------------------------------------------------------------
# GNN-based attack chain reasoning (NetworkX + optional PyG)
# ---------------------------------------------------------------------------
...
class NPUAccelerator:
    def __init__(self):
        self.available = False  # hook for real NPU APIs
...
class NeuroFabricCockpitTk:
    def __init__(self,
        ...
        self.root.title("NeuroFabric Tactical Cockpit (Tk, Tier-7)")
...
# ---------------------------------------------------------------------------
# Optional PySide6 cockpit (more visual)
# ---------------------------------------------------------------------------
...
class NeuroFabricCockpitQt:
    def __init__(self, stats_worker: StatsWorker):
        ...
        self.window.setWindowTitle("NeuroFabric Cockpit (Qt, Tier-7)")
