#!/usr/bin/env python3
# unified_glyph_vault_systembound.py
#
# Concept demo ONLY:
# - Glyph codec
# - RSA + AES-GCM crypto
# - Simple "autoloader" for crypto backend
# - System-bound wrapper (pseudo machine binding)
# - TLS-like handshake
# - Local vault
...
# -----------------------------
# System-bound key provider (pseudo)
# -----------------------------

class SystemKeyProvider:
    """
    Pseudo machine-bound key:
    - Uses OS name + hostname + a local secret file.
    - This is NOT real TPM/DPAPI security, just a demo of the idea.
    """

    def __init__(self, label: str = "glyph_vault_demo"):
        self.label = label
        self.secret_path = os.path.join(
            os.path.expanduser("~"), f".{self.label}_machine_secret"
        )

    def _ensure_local_secret(self) -> bytes:
    ...
    def get_wrapping_key(self) -> bytes:
        ...
        local_secret = self._ensure_local_secret()
        h = hashlib.sha256()
        h.update(os_name + b"|" + host + b"|" + local_secret)
        return h.digest()  # 32 bytes


# -----------------------------
# System-bound wrapper
# -----------------------------

class SystemBoundWrapper:
    def __init__(self, sys_keys: SystemKeyProvider):
        self.sys_keys = sys_keys
    ...
    def wrap(self, plaintext: bytes, meta: dict) -> bytes:
        ...
        wrapping_key = self.sys_keys.get_wrapping_key()
        aes_wrap = AESGCM(wrapping_key)
        w_nonce = os.urandom(12)
        wrapped_key = aes_wrap.encrypt(w_nonce, master_key, b"")

        obj = {
            "version": 1,
            "algo": "AES-256-GCM",
            "wrapped_key": (w_nonce + wrapped_key).hex(),
    ...
    def unwrap(self, blob: bytes) -> bytes:
        ...
        wk = bytes.fromhex(obj["wrapped_key"])
        w_nonce, w_ct = wk[:12], wk[12:]
        aes_wrap = AESGCM(wrapping_key)
        master_key = aes_wrap.decrypt(w_nonce, w_ct, b"")
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Unified Glyph Vault (Concept Demo)")
        ...
        self.sys_keys = SystemKeyProvider()
        self.wrapper = SystemBoundWrapper(self.sys_keys)
        ...
        tk.Button(frm_mid2, text="Wrap Vault (This Machine)", command=self.wrap_vault).pack(side=tk.LEFT, padx=5)
        tk.Button(frm_mid2, text="Unwrap Vault", command=self.unwrap_vault).pack(side=tk.LEFT, padx=5)
        ...
        self.log_msg("[Info] This is a concept demo. Do NOT use for real secrets.")
    ...
    def init_server(self):
        self.server = Server()
        self.client = Client()
        self.log_msg("[Handshake] Server and Client initialized with fresh keys/randoms.")
    ...
    def run_handshake(self):
        ...
        self.log_msg("[Handshake] Session keys derived (client & server).")
    ...
    def wrap_vault(self):
        if self.encrypted_vault_blob is None:
            ...
            return
        meta = {
            "note": "wrapped_to_this_machine_demo",
            "os": platform.system(),
            "host": platform.node(),
        }
        self.wrapped_vault_blob = self.wrapper.wrap(self.encrypted_vault_blob, meta)
        self.log_msg(f"[Wrapper] Vault wrapped to this machine ({len(self.wrapped_vault_blob)} bytes).")

    def unwrap_vault(self):
        if self.wrapped_vault_blob is None:
            messagebox.showwarning("Warning", "No wrapped vault yet")
            return
        try:
            self.encrypted_vault_blob = self.wrapper.unwrap(self.wrapped_vault_blob)
        except Exception as e:
            messagebox.showerror("Error", f"Unwrap failed: {e}")
            return
        self.log_msg("[Wrapper] Unwrapped vault back to encrypted blob. You can now decrypt vault.")
