...
# - Universal autoloader for external libs (fail silently)
# =========================

# =========================
# UNIVERSAL AUTOLOADER (Fail Silently Mode)
# =========================
import importlib
import subprocess
import sys
...
def autoload(module_name, package_name=None):
    ...
    - module_name: import path (e.g., "PIL.Image", "torchvision.transforms")
    - package_name: pip package name if different (e.g., "opencv-python")
    - Fail silently: returns None if install or import fails
    """
    try:
        return importlib.import_module(module_name)
    except ImportError:
        pkg = package_name if package_name else module_name.split(".")[0]
        print(f"[Autoloader] Installing missing package: {pkg}")
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", pkg])
        except Exception as e:
            print(f"[Autoloader] Installation failed for {pkg}: {e}")
            return None

        try:
            return importlib.import_module(module_name)
        except Exception as e:
            print(f"[Autoloader] Import failed after install: {module_name} ({e})")
            return None
...
# =========================
# EXTERNAL LIBS VIA AUTOLOADER
# =========================
np = autoload("numpy")
requests = autoload("requests")

PIL_Image = autoload("PIL.Image", "Pillow")
cv2 = autoload("cv2", "opencv-python")

torch = autoload("torch")
if torch is None:
    raise RuntimeError("torch is required for USET-X.")
nn = torch.nn
optim = torch.optim

psutil = autoload("psutil")

pymongo = autoload("pymongo")
MongoClient = pymongo.MongoClient if pymongo else None
...
matplotlib = autoload("matplotlib")
plt = autoload("matplotlib.pyplot", "matplotlib")
if matplotlib is None or plt is None:
    raise RuntimeError("matplotlib is required for the operator console.")
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg  # used in Part 2

# Tkinter is stdlib, import normally (not via autoload)
...
class USETX:
    ...
    def detect_objects(self, bytes_data):
        # Simple pseudo-object detection for stability
...
class AnalystAgent:
    ...
    def analyze_recent(self, limit=50):
        ...
        if not complexities or np is None:
...
class USETXConsole(tk.Tk):
    ...
    def plot_evolution(self, evolution):
        ...
        self.ax.spines["left"].set_color("#cccccc")
        self.ax.spines["right"].set_color("#cccccc")
        if not evolution or np is None:
            ...
            for d in meta.get("meta_docs", []):
                ...
                if d.get("type") == "analysis_summary":
                    lines.append(
                        ...
                        f"avg_complexity={d.get('avg_complexity')}, "
                        f"anomaly={d.get('anomaly_score')}"
                    )
