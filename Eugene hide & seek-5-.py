...
# =========================
# ALTERED STATES ENGINE (PREDICTIVE / MISSING DETAIL GUESSER)
# =========================

class AlteredStatesEngine:
    """
    High-level, safe "data physics" engine:
    - tracks anomaly score history
    - looks for sudden jumps/drops (state shifts)
    - flags when expected signals are missing
    """
...
# =========================
# THREAT NARRATIVE + SWARM CONSENSUS
# =========================
...
def hook_collect_threat_matrix(lockdown, autonomous, anomaly_score):
    nodes = [{"node": "LocalNode", "score": anomaly_score}]
...
def hook_collect_logs(lockdown, autonomous, anomaly_score, chain_findings, altered_state):
    now = datetime.utcnow().isoformat() + "Z"
        ...
        while self.running:
            try:
                ...
                while self.running:
                    ...
                    # Event text is written by OrganismLoop; here we just sleep
...
class OrganismLoop(threading.Thread):
    ...
    def run(self):
        while self.running:
            ...
            # synthesize SecEvents for chain engine
            ...
            logs     = hook_collect_logs(lockdown, autonomous, anomaly_score, chain_findings, altered_state)
            ...
            # also emit events to OS pipe if available
            if PIPE_AVAILABLE:
                try:
