#!/usr/bin/env python3
"""
AI Borg Secure Connection Organism (Full Unified File + Packet Sniffer)
...
- QueenSniffer:
    * Wireshark-style passive packet capture using Scapy
    * Feeds observed flows into the Borg registry and cockpit log
...
# ---------------------------------------------------------------------------
# Connection registry (for scanner, resolver, lockdown, sniffer)
# ---------------------------------------------------------------------------

class ConnectionRegistry:
    """
    Central registry of all connection attempts and states.
    Stores host, port, IPs, security status, reasons, fingerprints, kind.
    kind: "active" (Borg-initiated) or "sniff" (passive observed).
    """
    def __init__(self):
...
class SecureCockpit(QMainWindow):
    ...
    def _build_ui(self):
        ...
        log_label = QLabel("Event Log (Guardians + Sniffer):")
        self.log_view = QTextEdit()
        self.log_view.setReadOnly(True)

        self.status_summary = QLabel("Connections: 0 | Secure: 0 | Insecure: 0 | Observed: 0")
    ...
    def refresh_status_summary(self):
        ...
        observed = sum(1 for c in snapshot if c.get("kind") == "sniff")
        self.status_summary.setText(
            f"Connections: {total} | Secure: {secure} | Insecure: {insecure} | Observed: {observed}"
        )
