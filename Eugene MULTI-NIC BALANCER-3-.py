...
# -*- coding: utf-8 -*-
"""
network_organism_hud_altered_states.py
...
- PySide6 tactical HUD cockpit with Learning + Physics panels
"""
...
class NetworkHAL:
    ...
    def apply_gaming_qos(self, game_process_names, high_prio_nic):
        """
        Windows 11:
          - Bias high_prio_nic via metric
          - Optional DSCP policies via PowerShell
        Other OS: hooks only.
        """
...
class DataPhysicsEngine:
    """
    Models each NIC as a pipe:
      - pressure: how "full" / stressed the pipe is (load + health)
      - turbulence: instability (derived from health score)
      - flow: total Mbps
    """
    def compute(self, nic_health: dict, throughput: dict):
        ...
        for nic, health in nic_health.items():
            ...
            # Lower health score = better; invert for pressure
            # Clamp to avoid negative
            pressure = max(0.0, 100.0 - min(health, 100.0)) + flow_mbps
            # Turbulence: how "angry" the line is; higher health score => more turbulence
...
# =========================
#  NETWORK BRAIN (WITH LEARNING + ALTERED STATES)
# =========================
...
class NetworkBrain:
    def __init__(self, node_id=None, role=ROLE_WORKER,
        ...
        self.manual_mode_override = False  # HUD override flag
    ...
    def set_consciousness_mode(self, mode: ConsciousnessMode):
        ...
        self.manual_mode_override = True  # HUD override
    ...
    def _routing_control_thread(self):
        while not self.stop_event.is_set():
            # Auto consciousness only if HUD hasn't overridden
            ...
            # Apply gaming QoS when in gaming mode
...
# =========================
#  PYSIDE6 TACTICAL HUD COCKPIT (WITH LEARNING + PHYSICS + MODE SWITCH)
# =========================
...
if HAVE_QT:
    class NetworkCockpit(QtWidgets.QWidget):
        ...
        def _build_ui(self):
            ...
            # Consciousness control bar
            control_bar = QtWidgets.QHBoxLayout()
            self.mode_display = QtWidgets.QLabel()
            control_bar.addWidget(self.mode_display)
            ...
            left_layout.addWidget(self.peers_view)
            ...
            self.learning_label = QtWidgets.QLabel("LEARNING STATUS (per hour, per NIC)")
            right_layout.addWidget(self.learning_label)

            self.learning_table = QtWidgets.QTableWidget()
            ...
            right_layout.addWidget(self.learning_table)
        ...
        def refresh(self):
            ...
            self.mode_display.setText(
                f"CONSCIOUSNESS: {mode_conscious.upper()} :: CPU {cpu:.1f}% :: MEM {mem:.1f}%"
            )
            ...
            for row, nic in enumerate(sorted(nic_health.keys())):
                tx_mbps = throughput.get(nic, {}).get("tx_bps", 0.0) / 1e6
                rx_mbps = throughput.get(nic, {}).get("rx_bps", 0.0) / 1e6
                phys = physics.get(nic, {})
                pressure = phys.get("pressure", 0.0)
                turbulence = phys.get("turbulence", 0.0)
                flow_mbps = phys.get("flow_mbps", 0.0)
                ...
                self.table.setItem(row, 6, QtWidgets.QTableWidgetItem(f"{pressure:.1f}"))
                self.table.setItem(row, 7, QtWidgets.QTableWidgetItem(f"{turbulence:.1f}"))
                self.table.setItem(row, 8, QtWidgets.QTableWidgetItem(f"{flow_mbps:.2f}"))
                self.table.setItem(row, 9, QtWidgets.QTableWidgetItem(f"{cpu:.1f}"))
                self.table.setItem(row, 10, QtWidgets.QTableWidgetItem(f"{mem:.1f}"))
                self.table.setItem(row, 11, QtWidgets.QTableWidgetItem(role))
            ...
            self.learning_table.setRowCount(len(rows))
            for i, (hour, nic, samples, avg_health, avg_tx, avg_rx, success_ratio) in enumerate(rows):
                self.learning_table.setItem(i, 0, QtWidgets.QTableWidgetItem(f"{hour:02d}"))
                self.learning_table.setItem(i, 1, QtWidgets.QTableWidgetItem(nic))
                self.learning_table.setItem(i, 2, QtWidgets.QTableWidgetItem(str(samples)))
                self.learning_table.setItem(i, 3, QtWidgets.QTableWidgetItem(f"{avg_health:.1f}"))
                self.learning_table.setItem(i, 4, QtWidgets.QTableWidgetItem(f"{avg_tx:.2f}"))
                self.learning_table.setItem(i, 5, QtWidgets.QTableWidgetItem(f"{avg_rx:.2f}"))
                self.learning_table.setItem(i, 6, QtWidgets.QTableWidgetItem(f"{success_ratio:.2f}"))
