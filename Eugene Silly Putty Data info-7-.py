#!/usr/bin/env python3
# Full Borg Organism – Next-Level Autonomous ASI Core
# Upgraded: Prediction, Learning, Defense, Autonomy, GUI, Swarm, Aliveness
# Hybrid Reboot Memory (SMB + Local Fallback) + Episodic Memory + Cross-Organ Fusion
# Upgrades: Probabilistic, temporal, multi-node AttackChainEngine + async autonomous loop
...
# =========================
# Attack Chain Engine – Upgraded (temporal, probabilistic, multi-node)
# =========================

class AttackChainEngine:
    """
    Upgraded engine:
    - Temporal ordering: sequences must appear in order within a time window.
    - Probabilistic scoring: combines event probabilities, timing, and pattern weights.
    - Multi-node correlation: boosts score when pattern appears across multiple nodes.
    """
    ...
    def _score_pattern_for_node(
        ...
        # sliding search for ordered sequence
    ...
    def detect(self) -> List[Tuple[str, float, Set[str]]]:
        ...
        for pattern in self.patterns:
            ...
            for n in nodes:
                s, w = self._score_pattern_for_node(n, pattern)
                if s > 0.0:
                    node_scores[n] = s
                    if w:
                        node_windows[n] = w
            ...
            # multi-node correlation: boost if multiple nodes show pattern
...
class AdaptiveCodexMutation:
    ...
    def load_from_state(self, state: Dict[str, Any]):
        self.retention_seconds = float(state.get("retention_seconds", self.retention_seconds))
        self.ghost_sensitive = bool(state.get("ghost_sensitive", self.ghost_sensitive))
        self.phantom_node_enabled = bool(state.get("phantom_node_enabled", self.phantom_node_enabled))
...
class SelfRewritingAgent:
    ...
    def step(self, risk: float, health: float, ghost_sync: bool):
        scale = self.learning_rate * (0.5 + 0.5 * risk)
        if ghost_sync:
            scale *= 2.0
    ...
    def load_from_state(self, state: Dict[str, Any]):
        w = state.get("weights")
        if isinstance(w, list) and len(w) == len(self.agent_weights):
            self.agent_weights = self.cp.array(w, dtype=float)
...
class BrainCore:
    ...
    def __init__(self, chain_engine: AttackChainEngine, learning_rate: float = 0.05):
        ...
        self.internal_dialogue: List[str] = deque(maxlen=10)
    ...
    def _multi_engine_vote(self, base_risk: float, variance: float, trend: float, turbulence: float, baseline_dev: float, organ_vector: Dict[str, float]) -> float:
        engines = {}
...
def _record_episode(self, label: str):
        ep = {
            "label": label,
            "risk": self.risk_score,
            "health": self.collective_health,
...
def _auto_goal_shift(self):
        hour = datetime.now().hour
        if self.mission == "AUTO":
            if 0 <= hour < 6:
                self.mission = "LEARN"
            elif 6 <= hour < 18:
                self.mission = "OPTIMIZE"
            else:
                self.mission = "STABILITY"

        if self.risk_score > 0.7:
            self.mission = "PROTECT"
        elif self.collective_health < 0.4:
            self.mission = "STABILITY"

def _self_experiment(self):
        if self._tick_count % 120 != 0:
            return
        old_thr = self.autonomy.threshold_risk
        delta = random.uniform(-0.2, 0.2)
        self.autonomy.threshold_risk = max(1.0, min(3.0, self.autonomy.threshold_risk + delta))
        self._log_internal(f"Experiment: threshold_risk {old_thr:.2f} -> {self.autonomy.threshold_risk:.2f}")

def _log_internal(self, msg: str):
        self.internal_dialogue.append(f"[{time.strftime('%H:%M:%S')}] {msg}")

def _cross_organ_vector(self, organs: List[BaseOrgan]) -> Dict[str, float]:
        vec = {}
        for o in organs:
            vec[o.name] = max(0.0, min(1.0, 0.5 * o.risk + 0.5 * (1.0 - o.health)))
        return vec

def tick(self, putty: SillyPuttyDataGatherer, organs: List[BaseOrgan], swarm_view: Dict[str, Any]):
        self._tick_count += 1
        p = putty.report()

        organ_vec = self._cross_organ_vector(organs)
        organ_avg = sum(organ_vec.values()) / len(organ_vec) if organ_vec else 0.0
        swarm_risk = swarm_view.get("swarm_risk", 0.0)

        base_risk = (
            0.25 * p["anomaly_density"] +
            0.15 * p["pressure"] +
            0.15 * p["turbulence"] +
            0.25 * organ_avg +
            0.2 * swarm_risk
        ...
        best_guess = self._multi_engine_vote(base_risk, variance, trend, p["turbulence"], baseline_dev, organ_vec)
        ...
        self._auto_goal_shift()
        self._self_experiment()

        if self.risk_score > 0.8:
            self._record_episode("pre_overload")
        if self.collective_health > 0.8 and self.risk_score < 0.3:
            self._record_episode("stability")
        if self.meta_state == "Hyper-Flow" and self.collective_health > 0.7:
            self._record_episode("beast_mode_win")
        ...
        if self.risk_score > 0.7:
            self._log_internal("I sense danger; tightening posture and prioritizing protection.")
        elif self.opportunity_score > 0.6:
            self._log_internal("This looks like a good learning window; relaxing some constraints.")
...
if GUI_AVAILABLE:
    class BorgCockpit(QWidget):
        def __init__(self,
            ...
            self.setWindowTitle("Borg Organism Cockpit – Full Brain v2")
...
def autonomous_loop(autoloader: BorgAutoloader,
    ...
    while True:
        ...
        swarm_consensus.submit_votes("node-001", node_votes)
...
if GUI_AVAILABLE:
    class BorgCockpit(QWidget):
        ...
        def _tick(self):
            ...
            self.label_chains.setText(
                "Chains: " + ", ".join([f"{c[0]}({c[1]})[{','.join(c[2])}]" for c in chains]) if chains else "Chains: none"
            )
            self.label_autonomy.setText(
                f"Autonomy - appetite={autonomy['appetite']} thr={autonomy['threshold_risk']} hor={autonomy['horizon']} damp={autonomy['dampening']} cache={autonomy['cache_aggressiveness']} threads={autonomy['thread_expansion']} lr={autonomy['learning_rate']}"
            )
...
# =========================
# Autonomous Daemon Loop (async, upgraded)
# =========================

async def autonomous_loop_async(autoloader: BorgAutoloader,
                                putty: SillyPuttyDataGatherer,
                                organs: List[BaseOrgan],
                                self_integrity: SelfIntegrityOrgan,
                                queen: BorgQueen,
    ...
    while True:
        ...
        # queen assimilation
        report = putty.report()
        queen.assimilate(node_id, report)

        # RT queen events
        events_for_rt = [{"entity": f"{node_id}_flow_{i}", "score": report["anomaly_density"] + i * 0.01} for i in range(3)]
        rt_queen.update(node_id, events_for_rt)

        # AttackChainEngine events (temporal + probabilistic)
        # simulate some security events based on anomaly density / pressure
        anomaly = report["anomaly_density"]
        pressure = report["pressure"]
        base_p = max(0.1, min(0.9, anomaly + pressure * 0.2))

        # simple synthetic stream
        chain_engine.add_event(node_id, "proc_spawn", {"src": "system"}, probability=base_p)
        if anomaly > 0.3:
            chain_engine.add_event(node_id, "powershell", {"src": "user"}, probability=base_p * 0.9)
        if anomaly > 0.4:
            chain_engine.add_event(node_id, "net_connect", {"dst": "remote"}, probability=base_p * 0.8)
        if anomaly > 0.5:
            chain_engine.add_event(node_id, "file_mod", {"path": "C:\\temp\\x"}, probability=base_p * 0.7)
        if anomaly > 0.6:
            chain_engine.add_event(node_id, "proc_start", {"exe": "weird.exe"}, probability=base_p * 0.8)
        if anomaly > 0.7:
            chain_engine.add_event(node_id, "net_conn", {"dst": "cnc"}, probability=base_p * 0.9)
        ...
        swarm_consensus.submit_votes(node_id, node_votes)
        consensus = swarm_consensus.compute_consensus()
        swarm_view = {
            "swarm_risk": consensus.get("risk", 0.0),
        }

        # environment based on chains
        if chains:
            brain.set_environment("DANGER")
        else:
            brain.set_environment("CALM")
        ...
        if now - last_snapshot >= snapshot_interval:
            snapshot = {
                ...
                "queen": queen.status(),
                "rt_queen_risk": rt_queen.global_risk(),
                "chains": [(c[0], c[1], list(c[2])) for c in chains],
                "situational": brain.situational_snapshot(),
                "predictive": brain.predictive_snapshot(),
                "codex": brain.codex_snapshot(),
                "agent": brain.agent_snapshot(),
...
# =========================
# Load learned state helper
# =========================

def load_learned_state_if_exists(brain: BrainCore, reboot_mgr: RebootMemoryManager):
    state = reboot_mgr.load_state()
    if not isinstance(state, dict):
        return
    bl = state.get("brain_learning", {})
    if isinstance(bl, dict):
        brain.load_learning_state(bl)
...
def main():
    ...
    reboot_mgr = RebootMemoryManager(
        primary_smb=r"\\192.168.1.50\borg",
        fallback_local=r"C:\BorgMemory",
    )
    ...
    brain = BrainCore(chain_engine, learning_rate=0.05)
    load_learned_state_if_exists(brain, reboot_mgr)

    try:
        if GUI_AVAILABLE:
            # GUI path: run async loop in a background thread, GUI in main thread
            def _run_async_loop():
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                loop.run_until_complete(
                    autonomous_loop_async(
                        ...
                        snapshot_path="brain_snapshot.json",
                        interval=0.5,
                    )
                )

            bg_thread = threading.Thread(target=_run_async_loop, daemon=True)
            bg_thread.start()
            ...
            # headless path: run async loop directly
            asyncio.run(
                autonomous_loop_async(
                    autoloader=autoloader,
                    putty=putty,
