"""
BORG-OS MEMORY BACKBONE ORGAN (v2.1 – Distributed, Privilege-Aware, Telemetry-Fed, Visual Overlays)
...
class BorgMemoryOrgan:
    ...
    def _inject_keepalive(self):
        # Inject low-priority neutral packets to keep flows alive
...
class MemoryVisualizer:
    ...
    Overlays:
    - LED persistence mode (cells fade instead of instantly going dark)
    - Health heartbeat LED (top-right indicator)
    - Reroute flash LED (top-left indicator when misroutes occur)
    - Telemetry pulse overlay when system metrics injected
    """
    ...
    def update(self):
        ...
        # Health heartbeat LED
        # Simple pulse based on time and forget_prob
        health = max(0.0, 1.0 - self.organ.policy.forget_prob)
        # blink: on/off every ~1s
        blink = int(self.organ.policy.time) % 2 == 0
        if blink:
            # color from green (healthy) to red (unhealthy)
