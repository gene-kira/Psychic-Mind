# =========================
# MIGHTY MOUSE – PREDICTIVE UPGRADE (UNIFIED FILE)
# =========================
...
class MemoryStore:
    def __init__(self):
        self.data = {
            "web": {
                ...
                "samples": 0,
                "trusted_ips": {},
                "domain_stats": {},   # drift, mismatch counts, last_status
            },
    ...
    def update_global_threat(self, value):
        g = self.data["global"]
        g["threat_history"].append((time.time(), value))
        if len(g["threat_history"]) > 200:
            g["threat_history"].pop(0)
        g["last_global_threat"] = value

        # Simple trend: compare last 5 values
        if len(g["threat_history"]) >= 5:
            recent = [v for _, v in g["threat_history"][-5:]]
...
# =========================
# WEB BRAIN (WITH MULTI-IP TRUST + DRIFT)
# =========================
...
class WebBrain:
    ...
    def _update_domain_drift(self, domain, status):
    ...
    def _check_ip_trust(self, domain, ip_str):
        ...
        learned.append(ip_str)
        self.memory.save()
        return ("MISMATCH", f"{domain} → {ip_str} differs from trusted IP list")
    ...
    def evaluate(self):
        ...
        # Drift tracking
        if domain:
            self._update_domain_drift(domain, status)
            drift_info = self.domain_stats.get(domain, {})
            mismatch_count = drift_info.get("mismatch_count", 0)
            if mismatch_count >= 3:
                score += 20
                self.reasons.append(f"Domain drift: {mismatch_count} mismatches observed")
...
class FilesystemBrain:
    ...
    def update_hover(self, path):
        ...
        if path:
            ...
            self.memory.update_avg("fs", self.score)
        else:
            self.score = 0
...
class GameCognition:
    ...
    def _update_timeline_and_prediction(self):
        ...
        player = self.minimap.get("player", {"x": 0, "y": 0})
        # Base score: count + proximity weighting
        base_score = 0
        for e in enemies:
            dx = e.get("x", 0) - player["x"]
            dy = e.get("y", 0) - player["y"]
            dist = max(1.0, sqrt(dx*dx + dy*dy))
            # closer enemies contribute more
            contrib = max(0, 40 - min(40, dist / 5.0))
            base_score += contrib
        ...
        if len(self.danger_history) > 50:
            self.danger_history.pop(0)

        self.memory.record_danger_peak(base_score)

        # Predict future danger using simple linear trend
        if len(self.danger_history) >= 5:
            recent = self.danger_history[-5:]
            times = list(range(len(recent)))
            n = len(times)
            sum_x = sum(times)
            sum_y = sum(recent)
            sum_xy = sum(x*y for x, y in zip(times, recent))
            ...
            horizon = 5.0
            predicted = recent[-1] + slope * horizon
            predicted = max(0, min(100, predicted))
            self.memory.set_future_danger(int(predicted))

            if predicted >= 70 and recent[-1] < 70:
                self.hud.push("predict", f"Danger spike expected in ~5s (trend rising)")
...
# Add missing method: richer danger score already used above, but keep API
def game_get_danger_score(self):
    if not self.minimap:
        return 0
    # Use last computed base score if available
    if self.danger_history:
        return self.danger_history[-1]
    enemies = self.minimap.get("enemies", [])
    return min(100, len(enemies) * 10)

GameCognition.get_danger_score = game_get_danger_score
        ...
        if mode == "game":
            if threat_level >= 60:
                ...
                if cautious > aggressive:
                    ...
                    profile = "combat"
        elif mode in ("web", "filesystem", "network"):
            if threat_level >= 50:
                profile = "analysis"
            else:
                profile = "stealth"
...
class CursorEngine:
    ...
    def set_state(self, profile, threat_level, future_danger, global_threat):
        combined = max(threat_level, future_danger, global_threat)
        if profile == "stealth":
            if combined < 40:
...
class DirectXOverlayBridge:
    ...
    def send_state(self, game_cog, mode, profile, threat_level, future_danger, global_threat):
        if not self.enabled:
            return
        state = {
            "mode": mode,
...
class NetworkAnomalyMonitor:
    ...
    def __init__(self, memory, hud, orchestrator):
        ...
        net.setdefault("anomaly_count", 0)
    ...
    def scan_once(self):
        ...
        for c in conns:
            ...
            net["anomaly_count"] = net.get("anomaly_count", 0) + 1
...
class Orchestrator:
    ...
    def compute_global_threat(self, web_score, fs_score, game_score, net_anomaly_count):
        # Weighted combination
        global_threat = (
            0.30 * web_score +
            0.20 * fs_score +
            0.40 * game_score +
            0.10 * min(100, net_anomaly_count * 5)
        )
        global_threat = int(max(0, min(100, global_threat)))
        self.memory.update_global_threat(global_threat)
    ...
    def tick(self):
        ...
        if self.mode == "filesystem":
            fs_score = self.fs.score
            self.bus.publish("fs_score", fs_score)
        elif self.mode == "web":
            web_score = self.web.evaluate()
            self.bus.publish("web_score", web_score)
        elif self.mode == "game":
            game_score = self.game.get_danger_score()
            self.bus.publish("game_danger", game_score)
        else:
            # still evaluate web/game occasionally for global threat
            if self.web.latest_url:
                web_score = self.web.evaluate()
            if self.game.minimap:
                game_score = self.game.get_danger_score()

        net_anomaly_count = self.memory.data["network"].get("anomaly_count", 0)
        future_danger = self.memory.data["game"]["future_danger_prediction"]

        global_threat = self.compute_global_threat(web_score, fs_score, game_score, net_anomaly_count)
        effective_threat = max(web_score, fs_score, game_score, future_danger, global_threat)

        profile = self.profiles.autoselect_profile(self.mode, effective_threat)

        self.cursor.set_state(profile, max(web_score, fs_score, game_score), future_danger, global_threat)
        self.overlay_bridge.send_state(self.game, self.mode, profile, max(web_score, fs_score, game_score), future_danger, global_threat)
...
class CockpitGUI(QWidget):
    def __init__(self, orch, hud, web, fs, game, profiles, memory):
        ...
        self.setWindowTitle("Mighty Mouse – Predictive Tactical HUD (Trust + Network + Global Threat)")
        ...
        self.global_threat_label = QLabel("Global Threat: 0 (STABLE)")
        main_layout.addWidget(self.status_label)
        main_layout.addWidget(self.mode_label)
        main_layout.addWidget(self.profile_label)
        main_layout.addWidget(self.global_threat_label)
        ...
        self.net_anomaly_label = QLabel("Network Anomalies: 0")
        main_layout.addWidget(self.net_cone_label)
        main_layout.addWidget(self.net_cone_view)
        main_layout.addWidget(self.net_anomaly_label)
    ...
    def update_gui(self):
        ...
        global_data = self.memory.data["global"]
        global_threat = global_data.get("last_global_threat", 0)
        trend = global_data.get("trend", "STABLE")
        self.global_threat_label.setText(f"Global Threat: {global_threat} ({trend})")
        ...
        self.net_anomaly_label.setText(f"Network Anomalies: {net.get('anomaly_count', 0)}")
...
def main():
    ...
    orch.log("[SYSTEM] Mighty Mouse (Predictive) fully initialized")
    sys.exit(app.exec_())
