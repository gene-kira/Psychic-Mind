...
class LearnedPolicy:
    ...
    MODEL_PATH = Path.home() / ".borg_queen_policy_tier10_1.json"
    ...
    def __init__(self, history_len: int = 5, lr: float = 0.01):
        ...
        # prediction_error_cpu, prediction_error_mem,
        # telemetry_gap_norm, missing_flag,
        # mode_one_hot (4),
        # water_flow (2): cpu_flow, mem_flow
        self.dim = history_len * 2 + 3 + 3 + 4 + 2
        self.weights = {a: [0.0] * self.dim for a in self.ACTIONS}

        # prediction head: predicts next cpu, mem from same features
    ...
    def _features(
        ...
        # simple "water data physics": flow = last - previous
        if len(cpu_seq) >= 2:
            cpu_flow = (cpu_seq[-1] - cpu_seq[-2]) / 100.0
            mem_flow = (mem_seq[-1] - mem_seq[-2]) / 100.0
        else:
            cpu_flow = 0.0
            mem_flow = 0.0
...
# =========================
# 11. Prediction error overlay (confidence waveform)
# =========================
...
# =========================
# 12. PySide6 cockpit (mindstream + visual load controls + confidence overlay)
# =========================
...
class CockpitWindow(QtWidgets.QMainWindow):
    def __init__(self, queen: QueenCore, stress: StressManager):
        ...
        self.setWindowTitle("Borg Tier-10.1 Cockpit – Queen Mindstream")
        ...
        # Thin mode indicator bar (no full background tint)
    ...
    def refresh_from_state(self):
        ...
        # Mode bar color only (no full background tint)
        if mode == "DREAMING":
