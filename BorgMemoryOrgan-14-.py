"""
BORG-OS MEMORY BACKBONE ORGAN (v10.0 – Adaptive AI HUD + Encrypted Swarm + Persistence)

Key upgrades vs v9.0:
- Adaptive AI HUD:
  - Calm Mode: minimal vital signs + single compact graph + status line
  - Alert Mode: expands to show threat/anomaly details and lineage
  - Auto-switch based on threat, anomaly, and user interaction
- AI Timing Manager:
  - Dynamically adjusts GUI refresh rate based on load, threat, anomaly, and interaction
  - Skips non-essential redraws under heavy load
- Simpler, faster GUI:
  - Fewer widgets, less redraw, more responsive mouse interaction
- All previous features retained:
  - Memory tiles, Alzheimer-like degradation, water physics, personality, altered states
  - Real telemetry organs (CPU/RAM, disk, process, network, GPU, etc.)
  - Anomaly detection organ
  - Encrypted swarm networking (AES-GCM / ChaCha20-Poly1305 + HMAC)
...
class AITimingManager:
    """
    AI-driven GUI pacing:
    - Uses CPU, threat, anomaly, last GUI time, and user interaction
    - Decides calm vs alert mode and refresh interval
    """
    def __init__(self):
    ...
    def decide_mode(self, organ: BorgMemoryOrgan) -> str:
        ...
        if threat > 0.6 or anomaly > 0.6:
            self.mode = "alert"
        elif idle > 30 and threat < 0.3 and anomaly < 0.3:
            self.mode = "calm"
        # if user is interacting frequently, allow alert mode to persist
        return self.mode

    def gui_delay_ms(self, organ: BorgMemoryOrgan, base_ms: int = 80) -> int:
        ...
        delay = int(max(50, min(delay, 300)))
        return delay
...
class AdaptiveAIHUD:
    ...
    - Alert: adds threat/anomaly details + lineage
    """
    def __init__(self, organ: BorgMemoryOrgan, timing: AITimingManager, base_tick_ms=80):
        if tkinter is None:
        ...
        # Bind mouse events to record interaction (for AI timing)
    ...
    def _status_text(self, organ: BorgMemoryOrgan, mode: str) -> str:
        ...
        if t < 0.3 and a < 0.3 and cpu < 60:
            return "Status: System Stable"
        if t > 0.7 or a > 0.7:
            return "Status: ALERT – Threat/Anomaly High"
        if cpu > 80:
            return "Status: High Load"
        if mode == "calm":
            return "Status: Calm Monitoring"
        return "Status: Active Monitoring"

    def _update(self):
        t0 = time.time()
        o = self.organ
        mode = self.timing.decide_mode(o)  # calm or alert
        ...
        # Single compact graph: mix CPU/MEM/Threat into one line (threat scaled)
        combined = []
...
def main():
    organ = BorgMemoryOrgan(
        organ_id="memory_backbone_v10_0",
    ...
    if tkinter is not None:
        ...
        hud = AdaptiveAIHUD(organ, timing, base_tick_ms=80)
        hud.run()
    else:
        while True:
            time.sleep(1.0)
