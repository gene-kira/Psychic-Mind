...
class UIAutomationOrgan(threading.Thread):
    """
    Watches the active window and infers a coarse 'intent' signal:
    - markets / charts
    - news / media
    - maps / satellite
    - other
...
class PredictiveOrgan:
    """
    Multi-factor, UI-aware escalation predictor.
    Consumes:
      - stress history
      - sentiment history
      - headline counts
    ...
    def update(self, stress, sentiment_score, headline_count, ui_snapshot):
        ...
        if len(self.stress_history) > 300:
            self.stress_history = self.stress_history[-300:]
            self.sentiment_history = self.sentiment_history[-300:]
            self.headline_history = self.headline_history[-300:]
            self.ui_intent_history = self.ui_intent_history[-300:]

        predicted_stress = predict_future_stress_linear(self.stress_history, steps_ahead=6)

        # Escalation probability: combine normalized factors
        base = predicted_stress / 100.0

        # Sentiment: negative pushes probability up
        if sentiment_score < -0.2:
            base += 0.15
        elif sentiment_score > 0.2:
            base -= 0.1
        ...
        # UI intent: if user is on markets/news/maps, assume heightened attention
...
class HormuzCockpit(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.title_label = tk.Label(
            top,
            text="Global Chokepoint Impact Monitor (Predictive + UI-Aware)",
            fg="#00ffcc",
            bg="#101018",
    ...
    def _schedule_predictive_refresh(self):
        ...
        self.after(10000, self._schedule_predictive_refresh)
    ...
    def refresh_markets(self):
        ...
        pred = predict_future_stress_linear(self.stress_history, steps_ahead=6)
    ...
    def refresh_predictive(self):
        ...
        self.ml_label_label.config(
            text=f"ML Assessment: {pred['label']}"
        )
...
if __name__ == "__main__":
    log("=== Starting Global Chokepoint Cockpit (Predictive + UI-Aware) ===")
    app = HormuzCockpit()
    app.mainloop()
