"""
Unified Organism (Bernoulli + Distributed + Anomaly + HopGraph + GPU + ML + Consensus + Replay + Scripting + Autonomous Mode)
...
- AutonomousModeOrgan (behavior profiles, decision tree, personality, safety rails, override hierarchy)
- LifecycleOrgan
- build_organism()
"""
...
class AutonomousModeOrgan:
    """
    Meta-organ that:
    - Watches flow metrics, anomalies, policy state, consensus view
    - Applies behavior profiles (aggressive/cautious/adaptive)
    - Uses a decision tree to adjust workers, thresholds, and node state
    - Enforces safety rails
    - Respects override hierarchy:
        1) Operator commands
        2) Safety rails
        3) Autonomous decisions
        4) Policy engine defaults
    """
    ...
    def _safety_rails_allow(self, action_kind: str, target: str) -> bool:
        if self._actions_in_last_minute() >= self.max_actions_per_min:
            self.logger.log("WARN", "Autonomous safety rail: too many actions per minute",
                            max=self.max_actions_per_min)
            return False

        if action_kind == "SEAL_NODE" and self.require_consensus_for_seal:
            if self._last_consensus:
                state = self._last_consensus.get(target, "OPEN")
                if state != "SEALED":
                    self.logger.log("WARN", "Autonomous safety rail: consensus does not agree to seal",
                                    node=target, consensus_state=state)
                    return False
        ...
        if action_kind == "SEAL_NODE" and len(sealed_nodes) >= max(1, len(node_state) - 1):
            self.logger.log("WARN", "Autonomous safety rail: refusing to seal last open node", target=target)
            return False

        return True

    def _apply_autonomous_policy_action(self, kind: str, target: str, reason: str):
        if not self._safety_rails_allow(kind, target):
            return
        self.bus.publish(FlowEvent("operator_command", {
            "cmd": kind,
            "target": target,
    ...
    def _decision_tree_on_anomaly(self, anomaly: Dict[str, Any]):
        ...
        if self._last_metrics:
            ...
            if avg_pressure > 0.95 and current_workers < max_workers:
                ...
                self._record_action("increase_workers_on_anomaly")
            elif avg_pressure < 0.2 and current_workers > min_workers:
                self.workers.adjust_workers(current_workers - 1)
                self._record_action("decrease_workers_on_low_pressure")

    def _decision_tree_periodic(self):
        ...
        for node, health in node_health.items():
            if health >= profile.seal_threshold:
                self._apply_autonomous_policy_action("SEAL_NODE", node,
                                                     "health above seal threshold (auto)")
            elif health >= profile.quarantine_threshold:
                self._apply_autonomous_policy_action("RESET_NODE_HEALTH", node,
                                                     "health above quarantine threshold (auto)")

    def start(self):
        self.thread = Thread(target=self._loop, daemon=True)
        self.thread.start()
        self.logger.log("INFO", "AutonomousModeOrgan started",
...
def build_organism(config_path: Optional[str] = None):
    ...
    auto_mode = AutonomousModeOrgan(cfg, policy, flow_metrics, bus, logger, workers, rings)
