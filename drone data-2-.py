...
class SystemTelemetry:
    ...
    def snapshot(self) -> Dict:
        ...
        net = (net_io.bytes_sent + net_io.bytes_recv) / (1024 * 1024)  # MB total
    ...
    def to_swarm_state(self, tele: Dict) -> Dict:
        # Map system load → priority / congestion / risk
...
class SwarmCockpit:
    ...
    def _loop(self):
        ...
        self.status_label.config(
            text=f"Status: Mood={self.swarm.emotion.mood.value}, Consensus={self.swarm.consensus.decision}, CPU={tele['cpu']:.1f}%, RAM={tele['ram']:.1f}%"
        )
...
def main():
    ...
    ui_driver = UIDriver(enabled=False)  # keep False in test unless you really want UIAutomation
