...
# =========================
# GPU / NPU Acceleration Helper
# =========================

class AccelBackend:
    def __init__(self):
        self.gpu_available = cp is not None
        self.npu_available = torch is not None and torch.cuda.is_available()
        self.backend = "cpu"
    ...
    def to_array(self, data: List[float]):
        if self.backend == "gpu" and self.gpu_available:
            return cp.asarray(data, dtype=cp.float32)
        if self.backend == "npu" and self.npu_available:
            return torch.tensor(data, dtype=torch.float32, device="cuda")
        return data  # plain Python list

    def mean(self, arr):
        if self.backend == "gpu" and self.gpu_available:
            return float(cp.mean(arr))
        if self.backend == "npu" and self.npu_available:
...
# =========================
# Attack Chain Engine + Narrative
# =========================

class AttackChainEngine:
    """
    Temporal, probabilistic, multi-node attack chain engine.
    """

    def __init__(self, window: int = 120, max_gap: int = 60):
        self.window = window
        self.max_gap = max_gap
        self.events: deque = deque()  # each: {"ts","node","type","data","p"}

        self.patterns = [
            {"name": "LOLBIN_ATTACK", "sequence": ["proc_spawn", "powershell", "net_connect"], "base_score": 0.9},
            {"name": "PROCESS_STORM", "sequence": ["proc_spawn", "proc_spawn", "proc_spawn", "net_connect"], "base_score": 0.8},
            {"name": "PERSISTENCE_EXFIL", "sequence": ["file_mod", "net_connect"], "base_score": 0.85},
            {"name": "FULL_ATTACK_CHAIN", "sequence": ["proc_start", "net_conn", "file_mod"], "base_score": 0.95},
            {"name": "SPAWN_STORM", "sequence": ["proc_start"] * 10, "base_score": 0.8},
            {"name": "LOLBIN_BEACON", "sequence": ["net_conn", "powershell"], "base_score": 0.9},
        ]
...
class AttackNarrativeEngine:
    """
    Builds a lightweight graph-based narrative from events and chains.
    """

    def __init__(self):
        self.graph: Dict[str, Dict[str, Any]] = {}  # entity -> {"type":..., "edges":[(target, rel, ts)]}
        self.last_narratives: List[Dict[str, Any]] = []

    def _ensure_node(self, entity: str, etype: str = "unknown"):
        if entity not in self.graph:
            self.graph[entity] = {"type": etype, "edges": []}

    def add_relation(self, src: str, dst: str, rel: str):
        ts = time.time()
        self._ensure_node(src)
        self._ensure_node(dst)
        self.graph[src]["edges"].append((dst, rel, ts))

    def ingest_chain(self, node_id: str, chain_name: str, score: float, events: List[Dict[str, Any]]):
        # Very simple narrative: link processes, IPs, files if present
        entities = set()
        for e in events:
            meta = e.get("data", {})
            proc = meta.get("proc")
        ...
        for i in range(len(entities) - 1):
            self.add_relation(entities[i], entities[i + 1], f"chain:{chain_name}")
        narrative = {
            "ts": time.time(),
            "node": node_id,
            "chain": chain_name,
        ...
        self.last_narratives.append(narrative)
        if len(self.last_narratives) > 100:
            self.last_narratives = self.last_narratives[-100:]

    def snapshot(self) -> Dict[str, Any]:
        return {
            "nodes": len(self.graph),
            "last_narratives": self.last_narratives[-10:],
        }
...
# =========================
# ML Anomaly Engine (IsolationForest)
# =========================

class MLAnomalyEngine:
    """
    Wraps IsolationForest for per-tick anomaly scoring.
    Uses GPU/NPU arrays for feature aggregation when available.
    """

    def __init__(self, accel: AccelBackend, max_samples: int = 512):
        self.accel = accel
        self.max_samples = max_samples
        self.features_buffer: List[List[float]] = []
        self.model = None
        self.trained = False
        self.last_train = 0.0
        self.train_interval = 300.0  # seconds
    ...
    def add_sample(self, feat: List[float]):
        self.features_buffer.append(feat)
        if len(self.features_buffer) > self.max_samples:
            self.features_buffer = self.features_buffer[-self.max_samples :]
    ...
    def maybe_train(self):
        ...
        if now - self.last_train < self.train_interval:
            return
        if len(self.features_buffer) < 50:
            return
        self._ensure_model()
        X = self.features_buffer
        try:
            self.model.fit(X)
            self.trained = True
            self.last_train = now
    ...
    def score(self, feat: List[float]) -> float:
        ...
        """
        self.add_sample(feat)
        self.maybe_train()
        if not self.trained or self.model is None:
            # fallback: simple distance from mean using accel backend
            if not self.features_buffer:
                return 0.5
            means = []
            for i in range(len(feat)):
                col = [row[i] for row in self.features_buffer]
                arr = self.accel.to_array(col)
                means.append(self.accel.mean(arr))
            dist = sum(abs(feat[i] - means[i]) for i in range(len(feat))) / max(len(feat), 1)
            return max(0.0, min(1.0, dist))
        try:
            s = -self.model.score_samples([feat])[0]  # higher = more anomalous
            # normalize roughly
...
class WindowsETWListener:
    def __init__(self):
        ...
        if platform.system().lower().startswith("win"):
            try:
                # Placeholder for real ETW integration (e.g., etw, pywintrace)
                # self.real_provider = SomeETWWrapper(...)
                self.available = True
            except Exception:
                self.available = False
    ...
    def poll_events(self) -> List[Dict[str, Any]]:
        ...
        events = []
        now = time.time()
        # If real provider exists, pull from it; otherwise synthetic
        if self.real_provider is not None:
            # events = self.real_provider.poll()
            pass
        else:
...
class PcapListener:
    """
    Stub for packet capture. In a real deployment, wire to pyshark/scapy/npcap.
    """
    ...
    def __init__(self, iface: Optional[str] = None):
        ...
        self.available = False
        try:
            # import pyshark or scapy here if desired
            # self.available = True
            self.available = False
        except Exception:
            self.available = False

    def poll_flows(self) -> List[Dict[str, Any]]:
        if not self.available:
            return []
        # Placeholder: return parsed flows
        return []
...
class TelemetryCollector:
    ...
    def collect_psutil_metrics(self) -> Dict[str, float]:
        ...
        disk = (disk_io.read_bytes + disk_io.write_bytes) if disk_io else 0
        net_io = psutil.net_io_counters()
        net = (net_io.bytes_sent + net_io.bytes_recv) if net_io else 0
        return {
            "cpu": cpu / 100.0,
            "mem": mem / 100.0,
    ...
    def feed_attack_chain(self, chain_engine: AttackChainEngine):
        # psutil-based process/network events
        ...
        # pcap flows
        for f in self.pcap.poll_flows():
            chain_engine.add_event(self.node_id, "net_conn", f, probability=0.6)


# =========================
# Swarm Sync over HTTP + UDP Mesh
# =========================
    ...
    def push_state(self, state: Dict[str, Any]):
        ...
        if self.udp_sock is not None:
            ...
            for peer in self.peers:
                ...
                try:
                    self.udp_sock.sendto(payload, (host, int(port)))
                except Exception:
                    continue
...
class BrainCore:
    ...
    def __init__(self,
        ...
        self.predictions = {"1s": 0.2, "5s": 0.2, "30s": 0.2, "120s": 0.2}
        ...
        self.ml_engine = MLAnomalyEngine(self.accel)
        self.narrative = AttackNarrativeEngine()
    ...
    def tick(self,
        ...
        ml_score = self.ml_engine.score(feat)
...
class BorgAutoloader:
    ...
    def autoload(self):
        libs = ["numpy", "psutil", "PySide6", "sklearn", "cupy", "torch"]
        for lib in libs:
            try:
                importlib.import_module(lib)
                self.loaded.append(lib)
            except Exception as e:
                self.failed[lib] = str(e)
...
class AutoloaderWatchdog(threading.Thread):
    def __init__(self, autoloader: BorgAutoloader, interval: float = 60.0):
...
class RebootMemoryManager:
    def __init__(self, smb_path: Optional[str] = None, local_path: str = "reboot_memory.json"):
    ...
    def _write_with_integrity(self, path: str, data: Dict[str, Any]):
        tmp = path + ".tmp"
    ...
    def _read_with_integrity(self, path: str) -> Optional[Dict[str, Any]]:
        try:
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
            with open(path + ".sha256", "r", encoding="utf-8") as f:
                expected = f.read().strip()
            actual = hashlib.sha256(json.dumps(data, sort_keys=True).encode("utf-8")).hexdigest()
            if expected != actual:
    ...
    def save_state(self, state: Dict[str, Any]):
        try:
            self._write_with_integrity(self.local_path, state)
            if self.smb_path:
                self._write_with_integrity(self.smb_path, state)
    ...
    def load_state(self) -> Optional[Dict[str, Any]]:
        paths = [p for p in [self.smb_path, self.local_path] if p]
        for p in paths:
            data = self._read_with_integrity(p)
            if data is not None:
    ...
    def snapshot(self) -> Dict[str, Any]:
        return {
            "primary_smb": self.smb_path,
            "fallback_local": self.local_path,
...
if GUI_AVAILABLE:
    class BorgCockpit(QWidget):
        def __init__(self,
            ...
            dialogue_group = QGroupBox("Internal Dialogue / Meta-State Timeline")
        ...
        def _tick(self):
            ...
            self.sa_label_state.setText(
                f"Mission: {sa['mission']} | Env: {sa['environment']} | MetaState: {sa['meta_state']} | Regime: {sa['regime']} | Emotion: {sa['emotion']} | RiskTol: {sa['risk_tolerance']} | OppBias: {sa['opportunity_bias']}"
            )
...
def autonomous_loop(autoloader: BorgAutoloader,
    ...
    while True:
        ...
        if chains:
            # naive: use all events in window as context
            events_list = list(chain_engine.events)
            for cname, cscore, nodes in chains:
                for n in nodes:
                    node_events = [e for e in events_list if e["node"] == n]
                    brain.narrative.ingest_chain(n, cname, cscore, node_events)
        ...
        brain.tick(putty, organs, swarm_view)
        ...
        if now - last_snapshot >= snapshot_interval:
            snapshot = {
                ...
                "narrative": brain.narrative.snapshot(),
            }
            try:
        ...
        # Adaptive interval based on CPU load
        cpu_load = metrics.get("cpu", 0.0)
        # more load -> longer sleep
        dynamic_interval = interval + cpu_load * 0.5
...
def main():
    ...
    node_id = "node-001"
    swarm_peers: List[str] = []  # e.g. ["192.168.1.20:8080", "192.168.1.21:8080"]
    brain = BrainCore(chain_engine, learning_rate=0.05, node_id=node_id, swarm_peers=swarm_peers)
    ...
    if GUI_AVAILABLE:
        ...
        loop_thread = threading.Thread(
            ...
            args=(autoloader, putty, organs, self_integrity, queen, rt_queen,
                  swarm_consensus, chain_engine, brain, reboot_mgr, telemetry),
            daemon=True,
        )
        loop_thread.start()
