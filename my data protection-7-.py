...
"""
GUARDIAN SENTINEL - TIER-2+ DEFENSIVE ORGANISM (WITH REAL ETW AGENT + ML SCHEMA)
-------------------------------------------------------------------------------

This single unified file contains:

1) Guardian Sentinel node (GUI + swarm + ML + ETW/WinDivert clients + CLI)
2) Real ETW Agent (separate mode) that:
   - Subscribes to PowerShell and Security logs via PowerShell
   - Streams normalized JSON events over TCP to Guardian
3) ML Training Schema utilities:
   - Feature schema for shell events
   - Log-to-dataset exporter for training ONNX/Torch models

MODES:
  - Default (no args): run Guardian Sentinel node
...
class MLTrainingSchema:
    ...
    def export_from_log(cls, log_path: str, out_csv: str):
        """
        Reads decrypted JSON lines (already decrypted) and exports shell events
        into a CSV suitable for ML training.
        """
...
class EncryptedLogger:
    ...
    def decrypt_to_plain(self, out_path: str):
        """
        Utility: decrypts encrypted log into a plain JSONL file for ML export.
        """
...
class ETWAgentService:
    """
    Real ETW agent (separate mode):
      - Uses PowerShell Get-WinEvent to tail PowerShell and Security logs
      - Normalizes events into JSON lines
      - Streams over TCP to connected Guardian clients
    """
    ...
    def _powershell_tail_loop(self):
        """
        Spawns a PowerShell process that continuously emits JSON for relevant events.
        We then broadcast those lines to all connected clients.
        """
        ps_script = r"""
