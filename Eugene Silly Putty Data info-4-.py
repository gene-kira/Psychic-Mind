#!/usr/bin/env python3
# Full Borg Organism – Autonomous Brain with Always-On GUI + Background Engine
# All upgrades merged into a single file.
...
if GUI_AVAILABLE:
    class BorgCockpit(QWidget):
        def __init__(self,
            ...
            pi_group = QGroupBox("Predictive Intelligence + Codex / Agent / Cipher / Reasoning / Escalations")
...
# =========================
# Autonomous Daemon Loop (background thread)
# =========================
...
def autonomous_loop(autoloader: BorgAutoloader,
                    ...
                    learned_path: str = "brain_learned_state.json",
                    interval: float = 0.5):
    last_snapshot = time.time()
    snapshot_interval = 10.0  # seconds
    last_learned_save = time.time()
    learned_interval = 30.0   # seconds
    ...
    while True:
        ...
        if now - last_learned_save >= learned_interval:
            learned_state = brain.export_learning_state()
            try:
                with open(learned_path, "w", encoding="utf-8") as f:
                    json.dump(learned_state, f, indent=2)
            except Exception:
                pass
...
def load_learned_state_if_exists(brain: BrainCore, learned_path: str = "brain_learned_state.json"):
    ...
    try:
        ...
        if isinstance(state, dict):
            brain.load_learning_state(state)
    except Exception:
        pass
...
def main():
    ...
    load_learned_state_if_exists(brain, "brain_learned_state.json")
    ...
    try:
        if GUI_AVAILABLE:
            ...
            autonomous_loop(
                ...
                learned_path="brain_learned_state.json",
                interval=0.5,
            )
    finally:
        watchdog.stop()
