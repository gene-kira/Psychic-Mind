"""
UNIFIED VIDEO AI PIPELINE + CLICKABLE HIGH-VIS HUD + REBOOTCORE DRIVE POPUP (CUDA-OPTIONAL)
WITH STARTUP SAVE-LOCATION PROMPT, ANONYMOUS SMB SUPPORT, AND NON-BLOCKING FOLDER PICKER

Features:
- As before, plus:
  - On startup: ask where to store RebootCore memory
  - Supports local drives, SMB UNC paths, and local folders
  - Auto-saves on reboot to chosen location
  - Can change location later via centered popup
"""
...
def prompt_smb_path(blocking_console_fallback: bool = True) -> Optional[str]:
    """
    Anonymous SMB: just ask for a UNC path like \\SERVER\\Share\\Folder
    """
    if TK_AVAILABLE:
        try:
            root = tk.Tk()
...
# ============================================================
# STARTUP SAVE-LOCATION PROMPT (CONSOLE + TK)
# ============================================================

def choose_initial_save_location():
    """
    Runs once at startup, before video loop.
    Lets user choose:
      - Local drive
      - SMB UNC path
      - Local folder
    ...
    # After changing base dir, reload memory from new location (if exists)
...
if __name__ == "__main__":
    # Ask on startup where to save, including SMB
