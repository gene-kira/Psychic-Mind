...
class BorgMemoryOrgan:
    ...
    def _consolidate_long_term_memory(self):
        # Move some ghosts into long-term memory
    ...
    def _update_threat_level(self):
        # Threat from telemetry
        cpu_threat = max(0.0, (self.last_cpu - 70) / 30.0)  # >70% CPU
        mem_threat = max(0.0, (self.last_mem - 80) / 20.0)  # >80% MEM

        # Threat from entropy (high forget/misroute)
        entropy = (self.policy.forget_prob + self.policy.misroute_prob) / 2.0

        # Combine
        self.threat_level = max(0.0, min(1.0, 0.5 * max(cpu_threat, mem_threat) + 0.5 * entropy))

    def _learn_from_health(self):
        # Health ~ inverse of forget_prob
        ...
        # Simple learning:
        # If health is low and threat is low -> reduce forget_prob
        # If threat is high -> increase misroute_prob slightly (paranoid scanning)
...
class MemoryVisualizer:
    """
    Visual grid:
    - Tiles as rows, cells as rectangles
    - Color = energy, text = value
    - LED persistence mode
    - Ghost halo overlay
    ...
    def __init__(self, organ: BorgMemoryOrgan, root=None, cell_size=40, tick_ms=300, persistence_sec=5.0):
        ...
        # Telemetry overlay (full grid pulse)
        ...
        # Health heartbeat LED (top-right)
...
class ThreatDetectionOrgan:
    """
    Monitors health and threat level from a remote backbone.
    """
