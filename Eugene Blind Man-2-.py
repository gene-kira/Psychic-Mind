#!/usr/bin/env python3
# OmniQueen Swarm Guard - unified sentinel, queen + nodes
...
def load_config():
    if not os.path.exists(CONFIG_PATH):
        default = {
            ...
            "process_watch": {
                "enabled": True,
                "suspicious_keywords": ["miner", "rat", "keylog", "hack", "bot", "crypt", "stealer"],
                "rolling_window": 600
            },
            "net_watch": {
                "enabled": True,
...
class BorgMemory:
    ...
    def score_process(self, name, cmdline, suspicious_keywords):
        ...
        if self.proc_count_stats and self.proc_count_stats["count"] > 5:
            ...
            if sigma > 0:
                env_boost = min(0.4, sigma / (self.ai_cfg.get("sigma_multiplier", 2.5) * 8.0))
        ...
        pressure_boost = min(0.4, pressure * 2.5)
...
def firewall_block_ip(ip):
    ...
    try:
        if system == "windows":
            ...
            # simple iptables rule; adapt to nftables/ufw if desired
            cmd = ["iptables", "-A", "INPUT", "-s", ip, "-j", "DROP"]
        elif system == "darwin":
            # macOS: pf is more complex; log only for now
...
def main():
    ...
    t_queen = None
    if role == "queen":
        t_queen = threading.Thread(target=queen_loop, args=(cfg, stop_event), daemon=True)
        t_queen.start()
