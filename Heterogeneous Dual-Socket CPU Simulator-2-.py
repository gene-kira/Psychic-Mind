...
# -*- coding: utf-8 -*-
"""
Heterogeneous Dual-Socket CPU Simulator v2
...
def assign_workloads(state: SimulationState) -> str:
    ...
    for w in state.workloads:
        ...
        lines.append(
            f"[{state.policy}] Workload '{w.name}' → {chosen.name} "
...
def compute_socket_utilization(state: SimulationState, socket_index: int) -> float:
    """
    Compute a simple utilization metric per socket:
...
class DualSocketSimulator(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Heterogeneous Dual-Socket CPU Simulator – Industrial Cockpit")
    ...
    def _build_ui(self):
        ...
        # Gauges
        gauge_group = QtWidgets.QGroupBox("Per-Socket Utilization")
    ...
    def _on_policy_changed(self, idx: int):
        ...
        # Re-run assignment if workloads already exist
        if self.state.workloads:
            explanation = assign_workloads(self.state)
            self.workload_model.refresh()
            self.explanation_text.setPlainText(explanation)
            self.load_text.setPlainText(summarize_socket_load(self.state))
