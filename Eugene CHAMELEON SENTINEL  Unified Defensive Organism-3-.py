...
- Tkinter Tactical Cockpit GUI with Threat Timeline
...
def log_event(kind: str, detail: str, level: str = "info"):
    """
    Append an event to the threat timeline.
    kind: short category (e.g. 'risk', 'mode', 'auth')
...
def start_gui():
    ...
    sep.grid(row=len(fields), column=0, columnspan=2, sticky="ew", pady=(10, 5))

    lbl_timeline = ttk.Label(frm, text="Threat timeline:")
    lbl_timeline.grid(row=len(fields)+1, column=0, columnspan=2, sticky="w")

    timeline = tk.Listbox(frm, height=12, width=90)
